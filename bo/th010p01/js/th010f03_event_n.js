...
function onLoad() {
	...
	// 出力シール
	if (urlPara["StdFocusedItemID"] == "Syutsuryoku_seal" 
		|| urlPara["StdFocusedItemID"] == "Layout")
	{
		// 項目設定
		var ItemNM;
		if (urlPara["StdFocusedItemID"] == "Syutsuryoku_seal") {
			ItemNM = "Syutsuryoku_seal"
		}
		else {
			ItemNM = "Layout"
		}
		...
		// フォーカス設定
		for (i = 0; i < getAdvControlFromItemID(ItemNM).length; i++) {
			if (getAdvControlFromItemID(ItemNM)[i].checked == true) {
				getAdvControlFromItemID(ItemNM)[i].focus();
				break;
			}
		}
	}
