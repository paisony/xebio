...
using com.xebio.bo.Tj120p01.Facade;
...
namespace com.xebio.bo.Tj120p01.Page
{
  /// <summary>
  /// Tj120f01のコードビハインドです。
  /// </summary>
  public partial class Tj120f01Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Tj120f01Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Tj120f01Facade().DoLoad(facadeContext);
								...
								ControlHeaderStoreCls.InitialSetHeaderStore<Tj120f01Form>(loginInfVO, tj120f01Form);
								// 一覧画面共通処理 ----------

								if (string.IsNullOrEmpty(tj120f01Form.Modeno))
								{
									// アコーディオンなし
									AccordionUtil.SetAccordionKbn(pageContext, BoSystemConstant.ACCORDION_ST_NONE);
									// モードNoを照会に設定
									tj120f01Form.Modeno = BoSystemConstant.MODE_REF.ToString();
				...
				if (base.GetPageContext() != null)
				{
					...
					Tj120f01Form f01VO = (Tj120f01Form)fvm.GetProgramVO(base.GetPageContext().CommandInfo.ProgramId).GetFormVO(Tj120p01Constant.FORMID_01);
		...
		protected virtual void OnBTNSEARCH_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Tj120f01Facade().DoBTNSEARCH_FRM(facadeContext);
				...
				// モードを設定 
				ModeUtil.SetMode(pageContext, ModeCls.GetModeNm(((Tj120f01Form)pageContext.GetFormVO()).Stkmodeno));
		...
		protected virtual void OnBTNZENSTK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj120f01Facade().DoBTNZENSTK_FRM(facadeContext);
		...
		protected virtual void OnBTNZENKJO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj120f01Facade().DoBTNZENKJO_FRM(facadeContext);
		...
		protected virtual void OnBTNPRINT_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj120f01Facade().DoBTNPRINT_FRM(facadeContext);

				// PDFファイル名を取得
				pdfNm = (string)facadeContext.GetUserObject(Tj120p01Constant.FCDUO_RRT_FLNM);
			...
			string serverPath = string.Format("{0}{1}{2}",
											FilePathManager.GetOutFilePath(Tj120p01Constant.PGID),
											Path.DirectorySeparatorChar,
											pdfNm
											);
			// クライアントファイル名
			string clientNm = string.Format("{0}.{1}",
											BoSystemReport.CreateFileName(BoSystemConstant.REPORTNM_TANAOROSIJUFUKULIST_V, 2),
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj120f01Facade().DoPGR_PGN(facadeContext, pageindex);
		...
		protected virtual void OnM1ROWNO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				// 次画面のフォームVOをファサードに設定
				FormVOManager fvm = new FormVOManager(Session);
				facadeContext.SetUserObject(Tj120p01Constant.FCDUO_NEXTVO, fvm.GetProgramVO(facadeContext.CommandInfo.ProgramId).GetFormVO(Tj120p01Constant.FORMID_02));

				new Tj120f01Facade().DoM1ROWNO_FRM(facadeContext);
		...
		protected virtual void OnBTNENTER_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				new Tj120f01Facade().DoBTNENTER_FRM(facadeContext);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					//{
						...
						//FormVOを取得する
						Tj120f01Form tj120f01Form = (Tj120f01Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(tj120f01Form);
			
						//明細部データを表示する
						RenderList(tj120f01Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(tj120f01Form, pageContext.FormInfo, formResource, lang);
					//}
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="tj120f01Form">画面FormVO</param>
		private void ShowListPageInfo(Tj120f01Form tj120f01Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(tj120f01Form.GetList("M1"));
		...
		private void RenderList(
			Tj120f01Form tj120f01Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(tj120f01Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(tj120f01Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = tj120f01Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Tj120f01M1Form tj120f01M1Form = (Tj120f01M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1face_no"),
					DataFormatUtil.GetFormatItem(tj120f01M1Form.M1face_no,formInfo["M1face_no"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tana_dan"),
					DataFormatUtil.GetFormatItem(tj120f01M1Form.M1tana_dan,formInfo["M1tana_dan"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kai_su"),
					DataFormatUtil.GetFormatItem(tj120f01M1Form.M1kai_su,formInfo["M1kai_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_su"),
					DataFormatUtil.GetFormatItem(tj120f01M1Form.M1scan_su,formInfo["M1scan_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1nyuryokutan_nm"),
					DataFormatUtil.GetFormatItem(tj120f01M1Form.M1nyuryokutan_nm,formInfo["M1nyuryokutan_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1nyuryoku_ymd"),
					DataFormatUtil.GetFormatItem(tj120f01M1Form.M1nyuryoku_ymd,formInfo["M1nyuryoku_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(tj120f01M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(tj120f01M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(tj120f01M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));
				ControlUtil.SetControlValue(Searchcnt,
					DataFormatUtil.GetFormatItem(tj120f01Form.Searchcnt, formInfo["Searchcnt"]));

				if(!base.CheckUseSelfCustomize())
				{
					// ボタンのValueにNoを設定
					((System.Web.UI.HtmlControls.HtmlInputButton)M1.Items[index].FindControl("M1rowno")).Value =
					tj120f01M1Form.Dictionary[Tj120p01Constant.DIC_M1ROWNO].ToString();
					// 明細背景色の設定
		...
		/// </summary>
		/// <param name="tj120f01Form">画面FormVO</param>
		private void RenderM1Pager(Tj120f01Form tj120f01Form)
		{
			Pgr.VirtualItemCount = tj120f01Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = tj120f01Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = tj120f01Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(tj120f01Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(tj120f01Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Modeno,
				DataFormatUtil.GetFormatItem(tj120f01Form.Modeno,formInfo["Modeno"]));
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(tj120f01Form.Stkmodeno,formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Face_no_from,
				DataFormatUtil.GetFormatItem(tj120f01Form.Face_no_from,formInfo["Face_no_from"]));
			ControlUtil.SetControlValue(Face_no_to,
				DataFormatUtil.GetFormatItem(tj120f01Form.Face_no_to,formInfo["Face_no_to"]));
			ControlUtil.SetControlValue(Nyuryokutan_cd,
				DataFormatUtil.GetFormatItem(tj120f01Form.Nyuryokutan_cd,formInfo["Nyuryokutan_cd"]));
			ControlUtil.SetControlValue(Nyuryokutan_nm,
				DataFormatUtil.GetFormatItem(tj120f01Form.Nyuryokutan_nm,formInfo["Nyuryokutan_nm"]));
			ControlUtil.SetControlValue(Nyuryoku_ymd_from,
				DataFormatUtil.GetFormatItem(tj120f01Form.Nyuryoku_ymd_from,formInfo["Nyuryoku_ymd_from"]));
			ControlUtil.SetControlValue(Nyuryoku_ymd_to,
				DataFormatUtil.GetFormatItem(tj120f01Form.Nyuryoku_ymd_to,formInfo["Nyuryoku_ymd_to"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Tj120f01Form)base.GetPageContext().GetFormVO());
			...
			Tj120f01Form formVo = (Tj120f01Form)base.GetPageContext().GetFormVO();
			//	全選択ボタン	[選択モードNO]が「取消」以外の場合、Disabledとする。
			//	全解除ボタン	[選択モードNO]が「取消」以外の場合、Disabledとする。
			//	Ｍ１フェイス№	[選択モードNO]が「修正」以外の場合、Disabledとする。
			//	Ｍ１棚段		[選択モードNO]が「修正」以外の場合、Disabledとする。
			//	印刷ボタン		[選択モードNO]が「照会」以外の場合、Disabledとする。
			//	確定ボタン		[選択モードNO]が「照会」の場合、Disabledとする。
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Tj120f01_Titlebar", lang);
				header.FormName = formResource.GetString("Tj120f01_FormCaption", lang);
			}
		}
		#endregion
		#endregion
