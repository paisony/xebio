...
function onLoad() {
	...
	if (mode == c_modeapply || mode == c_modesinseimaedel || mode == c_modesinseigodel || mode == c_moderef) {
...
// 業務ロジック↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
		...
		case "Btnback".toUpperCase():
			...
			// 新規作成、申請前修正モードの場合
			if (mode == c_insert || mode == c_modesinseimaeupd) {
...
// 業務ロジック↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
		case "Sinseiriyu_kb".toUpperCase():	// 申請理由
			...
			break;
		case "Kamoku_cd".toUpperCase():	// 科目コード
			// 名称取得部品を起動
...
function calcRow(lineNo) {
	...
	// Ｍ１売価金額
	var baikaKin = getAdvControlFromItemID("M1gokeibaika_kin", lineNo);
