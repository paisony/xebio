...
function onSubmit() {
	...
	switch (AdvGB_LastClickItemNm.toUpperCase()) {
		...
		case "Btnback".toUpperCase():
			// モードが「訂正」の場合、メッセージを出力
			if (getAdvControlFromItemID(clm_StkMode).value == c_modeteisei
...
function calcRow(lineNo) {
	...
	if (su.value == null || su.value == "") {
		getAdvControlFromItemID("M1teisei_suryo", lineNo).value = formatComma(kakuteisu.value);
		su.value = kakuteisu.value;
	}
