...
namespace com.xebio.bo.Tb090p01.Page
{
  ...
  /// Tb090f01のコードビハインドです。
  /// </summary>
  public partial class Tb090f01Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Tb090f01Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Tb090f01Facade().DoLoad(facadeContext);

								#region 共通ヘッダ処理

								// 一覧画面共通処理 ----------
								LoginInfoVO loginInfVO = LoginInfoUtil.GetLoginInfo();
								Tb090f01Form tb090f01Form = (Tb090f01Form)facadeContext.FormVO;
								ControlHeaderStoreCls.InitialSetHeaderStore<Tb090f01Form>(loginInfVO, tb090f01Form);
								// 一覧画面共通処理 ----------

								// 初期表示時のモードとアコーディオン状態を設定
								if (string.IsNullOrEmpty(tb090f01Form.Modeno))
								{
									// アコーディオンなし
									AccordionUtil.SetAccordionKbn(pageContext, BoSystemConstant.ACCORDION_ST_NONE);
									// モードNoを訂正に設定
									tb090f01Form.Modeno = BoSystemConstant.MODE_TEISEI;
									TabUtil.SetTabNumber(pageContext, BoSystemConstant.MODE_TEISEI);
								}
				...
				if (base.GetPageContext() != null)
				{
					...
					Tb090f01Form f01VO = (Tb090f01Form)fvm.GetProgramVO(base.GetPageContext().CommandInfo.ProgramId).GetFormVO(Tb090p01Constant.FORMID_01);
					TabUtil.SetTabNumber(base.GetPageContext(), f01VO.Modeno);
				}

				// 単一ファイルダウンロード処理
				if (SessionInfoUtil.GetPgObject(base.GetPageContext().CommandInfo.ProgramId, Tb090p01Constant.SESSION_KEY_DOWNLOAD_INFO, Session) != null)
				{
					// ダウンロード情報取得
					DLConditionVO dlVO = SessionInfoUtil.GetPgObject(base.GetPageContext().CommandInfo.ProgramId, Tb090p01Constant.SESSION_KEY_DOWNLOAD_INFO, Session) as DLConditionVO;

					// セッション削除
					SessionInfoUtil.RemovePgObject(Tb090p01Constant.SESSION_KEY_DOWNLOAD_INFO, base.GetPageContext());
		...
		protected virtual void OnBTNSEARCH_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Tb090f01Facade().DoBTNSEARCH_FRM(facadeContext);
				...
				// モードを設定 
				ModeUtil.SetMode(pageContext, ModeCls.GetModeNm(((Tb090f01Form)pageContext.GetFormVO()).Stkmodeno));
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tb090f01Facade().DoPGR_PGN(facadeContext, pageindex);
		...
		protected virtual void OnM1MOTODENPYO_BANGO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				// 次画面のフォームVOをファサードに設定
				FormVOManager fvm = new FormVOManager(Session);
				facadeContext.SetUserObject(Tb090p01Constant.FCDUO_NEXTVO, fvm.GetProgramVO(facadeContext.CommandInfo.ProgramId).GetFormVO(Tb090p01Constant.FORMID_02));

				new Tb090f01Facade().DoM1MOTODENPYO_BANGO_FRM(facadeContext);
				
				// 次画面のフォームビーンを設定
				fvm.SetFormVO(Tb090p01Constant.PGID, Tb090p01Constant.FORMID_02, (Tb090f02Form)facadeContext.GetUserObject(Tb090p01Constant.FCDUO_NEXTVO));
		...
		protected virtual void OnBTNENTER_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tb090f01Facade().DoBTNENTER_FRM(facadeContext);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					//{
						...
						//FormVOを取得する
						Tb090f01Form tb090f01Form = (Tb090f01Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(tb090f01Form);
			
						//明細部データを表示する
						RenderList(tb090f01Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(tb090f01Form, pageContext.FormInfo, formResource, lang);
					//}
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="tb090f01Form">画面FormVO</param>
		private void ShowListPageInfo(Tb090f01Form tb090f01Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(tb090f01Form.GetList("M1"));
		...
		private void RenderList(
			Tb090f01Form tb090f01Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(tb090f01Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(tb090f01Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = tb090f01Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Tb090f01M1Form tb090f01M1Form = (Tb090f01M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1rowno,formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumon_cd_bo"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1bumon_cd_bo,formInfo["M1bumon_cd_bo"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumonkana_nm"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1bumonkana_nm,formInfo["M1bumonkana_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1siiresaki_cd"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1siiresaki_cd,formInfo["M1siiresaki_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1siiresaki_ryaku_nm"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1siiresaki_ryaku_nm,formInfo["M1siiresaki_ryaku_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1nyukayotei_ymd"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1nyukayotei_ymd,formInfo["M1nyukayotei_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1aka_denpyo_bango"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1aka_denpyo_bango,formInfo["M1aka_denpyo_bango"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kuro_denpyo_bango"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1kuro_denpyo_bango,formInfo["M1kuro_denpyo_bango"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kensu"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1kensu,formInfo["M1kensu"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1teisei_suryo"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1teisei_suryo,formInfo["M1teisei_suryo"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1genkakin"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1genkakin,formInfo["M1genkakin"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1siire_kakutei_ymd"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1siire_kakutei_ymd,formInfo["M1siire_kakutei_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kakuteitan_nm"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1kakuteitan_nm,formInfo["M1kakuteitan_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kakutei_sb_nm"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1kakutei_sb_nm,formInfo["M1kakutei_sb_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kyakucyu"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1kyakucyu,formInfo["M1kyakucyu"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1negaki"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1negaki,formInfo["M1negaki"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(tb090f01M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));

				if(!base.CheckUseSelfCustomize()){
					// ボタンのValueに元伝票番号を設定
					((System.Web.UI.HtmlControls.HtmlInputButton)M1.Items[index].FindControl("M1motodenpyo_bango")).Value =
						tb090f01M1Form.Dictionary[Tb090p01Constant.DIC_M1MOTODENPYO_BANGO].ToString();
			...
			//M1明細部標題を設定する。
				...
				// (M1.HeaderRow.FindControl("M1kakutei_sb_nm") as Label).Text = 
					// DataFormatUtil.GetFormatListCaption(formResource.GetString("M1kakutei_sb_nm", lang), base.GetPageContext().FormInfo["M1kakutei_sb_nm"]);
		...
		/// </summary>
		/// <param name="tb090f01Form">画面FormVO</param>
		private void RenderM1Pager(Tb090f01Form tb090f01Form)
		{
			Pgr.VirtualItemCount = tb090f01Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = tb090f01Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = tb090f01Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(tb090f01Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(tb090f01Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Modeno,
				DataFormatUtil.GetFormatItem(tb090f01Form.Modeno,formInfo["Modeno"]));
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(tb090f01Form.Stkmodeno,formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Motodenpyo_bango_from,
				DataFormatUtil.GetFormatItem(tb090f01Form.Motodenpyo_bango_from,formInfo["Motodenpyo_bango_from"]));
			ControlUtil.SetControlValue(Motodenpyo_bango_to,
				DataFormatUtil.GetFormatItem(tb090f01Form.Motodenpyo_bango_to,formInfo["Motodenpyo_bango_to"]));
			ControlUtil.SetControlValue(Nyukayotei_ymd_from,
				DataFormatUtil.GetFormatItem(tb090f01Form.Nyukayotei_ymd_from,formInfo["Nyukayotei_ymd_from"]));
			ControlUtil.SetControlValue(Nyukayotei_ymd_to,
				DataFormatUtil.GetFormatItem(tb090f01Form.Nyukayotei_ymd_to,formInfo["Nyukayotei_ymd_to"]));
			ControlUtil.SetControlValue(Siire_kakutei_ymd_from,
				DataFormatUtil.GetFormatItem(tb090f01Form.Siire_kakutei_ymd_from,formInfo["Siire_kakutei_ymd_from"]));
			ControlUtil.SetControlValue(Siire_kakutei_ymd_to,
				DataFormatUtil.GetFormatItem(tb090f01Form.Siire_kakutei_ymd_to,formInfo["Siire_kakutei_ymd_to"]));
			ControlUtil.SetControlValue(Siiresaki_cd,
				DataFormatUtil.GetFormatItem(tb090f01Form.Siiresaki_cd,formInfo["Siiresaki_cd"]));
			ControlUtil.SetControlValue(Siiresaki_ryaku_nm,
				DataFormatUtil.GetFormatItem(tb090f01Form.Siiresaki_ryaku_nm,formInfo["Siiresaki_ryaku_nm"]));
			ControlUtil.SetControlValue(Bumon_cd_from,
				DataFormatUtil.GetFormatItem(tb090f01Form.Bumon_cd_from,formInfo["Bumon_cd_from"]));
			ControlUtil.SetControlValue(Bumon_nm_from,
				DataFormatUtil.GetFormatItem(tb090f01Form.Bumon_nm_from,formInfo["Bumon_nm_from"]));
			ControlUtil.SetControlValue(Bumon_cd_to,
				DataFormatUtil.GetFormatItem(tb090f01Form.Bumon_cd_to,formInfo["Bumon_cd_to"]));
			ControlUtil.SetControlValue(Bumon_nm_to,
				DataFormatUtil.GetFormatItem(tb090f01Form.Bumon_nm_to,formInfo["Bumon_nm_to"]));
			ControlUtil.SetControlValue(Old_jisya_hbn,
				DataFormatUtil.GetFormatItem(tb090f01Form.Old_jisya_hbn,formInfo["Old_jisya_hbn"]));
			ControlUtil.SetControlValue(Maker_hbn,
				DataFormatUtil.GetFormatItem(tb090f01Form.Maker_hbn,formInfo["Maker_hbn"]));
			ControlUtil.SetControlValue(Scan_cd,
				DataFormatUtil.GetFormatItem(tb090f01Form.Scan_cd,formInfo["Scan_cd"]));
			ControlUtil.SetControlValue(Kakutei_sb,
				DataFormatUtil.GetFormatItem(tb090f01Form.Kakutei_sb,formInfo["Kakutei_sb"]));
			ControlUtil.SetControlValue(Searchcnt,
				DataFormatUtil.GetFormatItem(tb090f01Form.Searchcnt,formInfo["Searchcnt"]));
		...
		protected override void SetItems()
		{
			...
			if (!IsPostBack)
			{
				Kakutei_sb.Items.Insert(0, new ListItem("", BoSystemConstant.DROPDOWNLIST_MISENTAKU));
			}
		}
		#endregion
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Tb090f01Form)base.GetPageContext().GetFormVO());
			...
			Tb090f01Form formVo = (Tb090f01Form)base.GetPageContext().GetFormVO();
		...
		private void SetCardCaption(FormResource formResource, string lang)
		{
			...
			ControlUtil.SetControlValue(Kakutei_sb_lbl, 
				DataFormatUtil.GetFormatCaption(formResource.GetString("Kakutei_sb", lang), base.GetPageContext().FormInfo["Kakutei_sb"]));
				DataFormatUtil.SetMustColorCaption(Kakutei_sb_lbl, base.GetPageContext().FormInfo["Kakutei_sb"]);
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Tb090f01_Titlebar", lang);
				header.FormName = formResource.GetString("Tb090f01_FormCaption", lang);
			}
		}
		#endregion
		#endregion
