...
namespace com.xebio.bo.Tj030p01.Page
{
  ...
  /// Tj030f02のコードビハインドです。
  /// </summary>
  public partial class Tj030f02Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Tj030f02Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Tj030f02Facade().DoLoad(facadeContext);
								break;
						}
					}
					else
		...
		protected virtual void OnBTNBACK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Tj030f02Facade().DoBTNBACK_FRM(facadeContext);
			...
			// 選択された管理Noにフォーカス設定
			focusItem = "M1face_no";
			focusMno = (string)((Tj030f02Form)pageContext.GetFormVO(Tj030p01Constant.PGID, Tj030p01Constant.FORMID_02)).Dictionary[Tj030p01Constant.DIC_M1SELCETROWIDX];
		...
		protected virtual void OnBTNPRINT_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj030f02Facade().DoBTNPRINT_FRM(facadeContext);
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj030f02Facade().DoPGR_PGN(facadeContext, pageindex);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					if (!MessageDisplayUtil.HasError(pageContext))
					{
						...
						//FormVOを取得する
						Tj030f02Form tj030f02Form = (Tj030f02Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(tj030f02Form);
			
						//明細部データを表示する
						RenderList(tj030f02Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(tj030f02Form, pageContext.FormInfo, formResource, lang);
					}
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="tj030f02Form">画面FormVO</param>
		private void ShowListPageInfo(Tj030f02Form tj030f02Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(tj030f02Form.GetList("M1"));
		...
		private void RenderList(
			Tj030f02Form tj030f02Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(tj030f02Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(tj030f02Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = tj030f02Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Tj030f02M1Form tj030f02M1Form = (Tj030f02M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1rowno,formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumon_cd"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1bumon_cd,formInfo["M1bumon_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumonkana_nm"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1bumonkana_nm,formInfo["M1bumonkana_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hinsyu_ryaku_nm"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1hinsyu_ryaku_nm,formInfo["M1hinsyu_ryaku_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1burando_nm"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1burando_nm,formInfo["M1burando_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1jisya_hbn"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1jisya_hbn,formInfo["M1jisya_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1maker_hbn"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1maker_hbn,formInfo["M1maker_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1syonmk"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1syonmk,formInfo["M1syonmk"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1iro_nm"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1iro_nm,formInfo["M1iro_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1size_nm"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1size_nm,formInfo["M1size_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_cd"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1scan_cd,formInfo["M1scan_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_su"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1scan_su,formInfo["M1scan_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(tj030f02M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));
		...
		/// </summary>
		/// <param name="tj030f02Form">画面FormVO</param>
		private void RenderM1Pager(Tj030f02Form tj030f02Form)
		{
			Pgr.VirtualItemCount = tj030f02Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = tj030f02Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = tj030f02Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(tj030f02Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(tj030f02Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Modeno,
				DataFormatUtil.GetFormatItem(tj030f02Form.Modeno,formInfo["Modeno"]));
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(tj030f02Form.Stkmodeno,formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Face_no,
				DataFormatUtil.GetFormatItem(tj030f02Form.Face_no,formInfo["Face_no"]));
			ControlUtil.SetControlValue(Tana_dan,
				DataFormatUtil.GetFormatItem(tj030f02Form.Tana_dan,formInfo["Tana_dan"]));
			ControlUtil.SetControlValue(Kai_su,
				DataFormatUtil.GetFormatItem(tj030f02Form.Kai_su,formInfo["Kai_su"]));
			ControlUtil.SetControlValue(Tenpo_gyosya_nm,
				DataFormatUtil.GetFormatItem(tj030f02Form.Tenpo_gyosya_nm,formInfo["Tenpo_gyosya_nm"]));
			ControlUtil.SetControlValue(Tenpo_gyosya_kb,
				DataFormatUtil.GetFormatItem(tj030f02Form.Tenpo_gyosya_kb,formInfo["Tenpo_gyosya_kb"]));
			ControlUtil.SetControlValue(Nyuryokutan_cd,
				DataFormatUtil.GetFormatItem(tj030f02Form.Nyuryokutan_cd,formInfo["Nyuryokutan_cd"]));
			ControlUtil.SetControlValue(Nyuryokutan_nm,
				DataFormatUtil.GetFormatItem(tj030f02Form.Nyuryokutan_nm,formInfo["Nyuryokutan_nm"]));
			ControlUtil.SetControlValue(Nyuryoku_ymd,
				DataFormatUtil.GetFormatItem(tj030f02Form.Nyuryoku_ymd,formInfo["Nyuryoku_ymd"]));
			ControlUtil.SetControlValue(Sosin_ymd,
				DataFormatUtil.GetFormatItem(tj030f02Form.Sosin_ymd,formInfo["Sosin_ymd"]));
			ControlUtil.SetControlValue(Gokeiscan_su,
				DataFormatUtil.GetFormatItem(tj030f02Form.Gokeiscan_su,formInfo["Gokeiscan_su"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Tj030f02Form)base.GetPageContext().GetFormVO());
			...
			#region 画面表示制御
			Tj030f02Form formVo = (Tj030f02Form)base.GetPageContext().GetFormVO();
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Tj030f02_Titlebar", lang);
				header.FormName = formResource.GetString("Tj030f02_FormCaption", lang);
			}
		}
		#endregion
		#endregion
