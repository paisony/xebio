...
using com.xebio.bo.Tj030p01.Facade;
...
namespace com.xebio.bo.Tj030p01.Page
{
  /// <summary>
  /// Tj030f01のコードビハインドです。
  /// </summary>
  public partial class Tj030f01Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Tj030f01Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Tj030f01Facade().DoLoad(facadeContext);
								...
								ControlHeaderStoreCls.InitialSetHeaderStore<Tj030f01Form>(loginInfVO, tj030f01Form);
								// 一覧画面共通処理 ----------

								if (string.IsNullOrEmpty(tj030f01Form.Modeno))
								{
									// アコーディオンなし
									AccordionUtil.SetAccordionKbn(pageContext, BoSystemConstant.ACCORDION_ST_NONE);
									// モードNoを照会に設定
									tj030f01Form.Modeno = BoSystemConstant.MODE_REF.ToString();
				...
				if (base.GetPageContext() != null)
				{
					...
					Tj030f01Form f01VO = (Tj030f01Form)fvm.GetProgramVO(base.GetPageContext().CommandInfo.ProgramId).GetFormVO(Tj030p01Constant.FORMID_01);
		...
		protected virtual void OnBTNSEARCH_FRM(object sender, System.EventArgs e)
		{
			...
			if (!base.DoValidate(sender))
			{
				...
				// 明細を非表示にする
				//string cScript = "detailHide();";
				//ClientScript.RegisterStartupScript(this.GetType(), "key", cScript, true);
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Tj030f01Facade().DoBTNSEARCH_FRM(facadeContext);
				...
				// モードを設定 
				ModeUtil.SetMode(pageContext, ModeCls.GetModeNm(((Tj030f01Form)pageContext.GetFormVO()).Stkmodeno));
		...
		protected virtual void OnBTNZENSTK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj030f01Facade().DoBTNZENSTK_FRM(facadeContext);
		...
		protected virtual void OnBTNZENKJO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj030f01Facade().DoBTNZENKJO_FRM(facadeContext);
		...
		protected virtual void OnBTNPRINT_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj030f01Facade().DoBTNPRINT_FRM(facadeContext);

				// PDFファイル名を取得
				pdfNm = (string)facadeContext.GetUserObject(Tj030p01Constant.FCDUO_RRT_FLNM);
			...
			string serverPath = string.Format("{0}{1}{2}",
											FilePathManager.GetOutFilePath(Tj030p01Constant.PGID),
											Path.DirectorySeparatorChar,
											pdfNm
											);
			// クライアントファイル名
			string clientNm = string.Format("{0}.{1}",
											BoSystemReport.CreateFileName(BoSystemConstant.REPORTNM_TANAOROSIITIRANHYO_V,2),
		...
		protected virtual void OnBTNCSV_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj030f01Facade().DoBTNCSV_FRM(facadeContext);
				...
				// CSVファイル名を取得
				csvNm = (string)facadeContext.GetUserObject(Tj030p01Constant.FCDUO_CSV_FLNM);
			...
			// クライアントファイル名
			string clientNm = string.Format("{0}{1}",
											BoSystemReport.CreateFileName(BoSystemConstant.CSVNM_TANA_V,2),
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj030f01Facade().DoPGR_PGN(facadeContext, pageindex);
		...
		protected virtual void OnM1FACE_NO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				// 次画面のフォームVOをファサードに設定
				FormVOManager fvm = new FormVOManager(Session);
				facadeContext.SetUserObject(Tj030p01Constant.FCDUO_NEXTVO, fvm.GetProgramVO(facadeContext.CommandInfo.ProgramId).GetFormVO(Tj030p01Constant.FORMID_02));

				new Tj030f01Facade().DoM1FACE_NO_FRM(facadeContext);

				// 次画面のフォームビーンを設定
				fvm.SetFormVO(Tj030p01Constant.PGID, Tj030p01Constant.FORMID_02, (Tj030f02Form)facadeContext.GetUserObject(Tj030p01Constant.FCDUO_NEXTVO));
		...
		protected virtual void OnBTNENTER_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj030f01Facade().DoBTNENTER_FRM(facadeContext);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					//{
						...
						//FormVOを取得する
						Tj030f01Form tj030f01Form = (Tj030f01Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(tj030f01Form);
			
						//明細部データを表示する
						RenderList(tj030f01Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(tj030f01Form, pageContext.FormInfo, formResource, lang);
					//}
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="tj030f01Form">画面FormVO</param>
		private void ShowListPageInfo(Tj030f01Form tj030f01Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(tj030f01Form.GetList("M1"));
		...
		private void RenderList(
			Tj030f01Form tj030f01Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(tj030f01Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(tj030f01Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = tj030f01Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Tj030f01M1Form tj030f01M1Form = (Tj030f01M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1rowno,formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tana_dan"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1tana_dan,formInfo["M1tana_dan"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kai_su"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1kai_su,formInfo["M1kai_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_su"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1scan_su,formInfo["M1scan_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1nyuryokutan_nm"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1nyuryokutan_nm,formInfo["M1nyuryokutan_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1nyuryoku_ymd"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1nyuryoku_ymd,formInfo["M1nyuryoku_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1sosin_ymd"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1sosin_ymd,formInfo["M1sosin_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1gyosya"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1gyosya,formInfo["M1gyosya"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(tj030f01M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));

				if(!base.CheckUseSelfCustomize()){
					((System.Web.UI.HtmlControls.HtmlInputButton)M1.Items[index].FindControl("M1face_no")).Value =
						tj030f01M1Form.Dictionary[Tj030p01Constant.DIC_M1FACE_NO].ToString();
					// 明細背景色の設定
		...
		/// </summary>
		/// <param name="tj030f01Form">画面FormVO</param>
		private void RenderM1Pager(Tj030f01Form tj030f01Form)
		{
			Pgr.VirtualItemCount = tj030f01Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = tj030f01Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = tj030f01Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(tj030f01Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(tj030f01Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Modeno,
				DataFormatUtil.GetFormatItem(tj030f01Form.Modeno,formInfo["Modeno"]));
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(tj030f01Form.Stkmodeno,formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Face_no_from,
				DataFormatUtil.GetFormatItem(tj030f01Form.Face_no_from,formInfo["Face_no_from"]));
			ControlUtil.SetControlValue(Face_no_to,
				DataFormatUtil.GetFormatItem(tj030f01Form.Face_no_to,formInfo["Face_no_to"]));
			ControlUtil.SetControlValue(Tana_dan_from,
				DataFormatUtil.GetFormatItem(tj030f01Form.Tana_dan_from,formInfo["Tana_dan_from"]));
			ControlUtil.SetControlValue(Tana_dan_to,
				DataFormatUtil.GetFormatItem(tj030f01Form.Tana_dan_to,formInfo["Tana_dan_to"]));
			ControlUtil.SetControlValue(Tenpo_gyosya_kb,
				DataFormatUtil.GetFormatItem(tj030f01Form.Tenpo_gyosya_kb,formInfo["Tenpo_gyosya_kb"]));
			ControlUtil.SetControlValue(Nyuryoku_ymd_from,
				DataFormatUtil.GetFormatItem(tj030f01Form.Nyuryoku_ymd_from,formInfo["Nyuryoku_ymd_from"]));
			ControlUtil.SetControlValue(Nyuryoku_ymd_to,
				DataFormatUtil.GetFormatItem(tj030f01Form.Nyuryoku_ymd_to,formInfo["Nyuryoku_ymd_to"]));
			ControlUtil.SetControlValue(Sosin_ymd_from,
				DataFormatUtil.GetFormatItem(tj030f01Form.Sosin_ymd_from,formInfo["Sosin_ymd_from"]));
			ControlUtil.SetControlValue(Sosin_ymd_to,
				DataFormatUtil.GetFormatItem(tj030f01Form.Sosin_ymd_to,formInfo["Sosin_ymd_to"]));
			ControlUtil.SetControlValue(Nyuryokutan_cd,
				DataFormatUtil.GetFormatItem(tj030f01Form.Nyuryokutan_cd,formInfo["Nyuryokutan_cd"]));
			ControlUtil.SetControlValue(Nyuryokutan_nm,
				DataFormatUtil.GetFormatItem(tj030f01Form.Nyuryokutan_nm,formInfo["Nyuryokutan_nm"]));
			ControlUtil.SetControlValue(Old_jisya_hbn,
				DataFormatUtil.GetFormatItem(tj030f01Form.Old_jisya_hbn,formInfo["Old_jisya_hbn"]));
			ControlUtil.SetControlValue(Old_jisya_hbn2,
				DataFormatUtil.GetFormatItem(tj030f01Form.Old_jisya_hbn2,formInfo["Old_jisya_hbn2"]));
			ControlUtil.SetControlValue(Old_jisya_hbn3,
				DataFormatUtil.GetFormatItem(tj030f01Form.Old_jisya_hbn3,formInfo["Old_jisya_hbn3"]));
			ControlUtil.SetControlValue(Old_jisya_hbn4,
				DataFormatUtil.GetFormatItem(tj030f01Form.Old_jisya_hbn4,formInfo["Old_jisya_hbn4"]));
			ControlUtil.SetControlValue(Old_jisya_hbn5,
				DataFormatUtil.GetFormatItem(tj030f01Form.Old_jisya_hbn5,formInfo["Old_jisya_hbn5"]));
			ControlUtil.SetControlValue(Sosin_jyotai,
				DataFormatUtil.GetFormatItem(tj030f01Form.Sosin_jyotai,formInfo["Sosin_jyotai"]));
			ControlUtil.SetControlValue(Scan_cd,
				DataFormatUtil.GetFormatItem(tj030f01Form.Scan_cd,formInfo["Scan_cd"]));
			ControlUtil.SetControlValue(Scan_cd2,
				DataFormatUtil.GetFormatItem(tj030f01Form.Scan_cd2,formInfo["Scan_cd2"]));
			ControlUtil.SetControlValue(Scan_cd3,
				DataFormatUtil.GetFormatItem(tj030f01Form.Scan_cd3,formInfo["Scan_cd3"]));
			ControlUtil.SetControlValue(Scan_cd4,
				DataFormatUtil.GetFormatItem(tj030f01Form.Scan_cd4,formInfo["Scan_cd4"]));
			ControlUtil.SetControlValue(Scan_cd5,
				DataFormatUtil.GetFormatItem(tj030f01Form.Scan_cd5,formInfo["Scan_cd5"]));
			ControlUtil.SetControlValue(Searchcnt,
				DataFormatUtil.GetFormatItem(tj030f01Form.Searchcnt, formInfo["Searchcnt"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Tj030f01Form)base.GetPageContext().GetFormVO());
			...
			Tj030f01Form formVo = (Tj030f01Form)base.GetPageContext().GetFormVO();

			if (BoSystemString.Nvl(formVo.Stkmodeno).Equals(BoSystemConstant.MODE_REF))
			{
				// [選択モードNo]が「照会」の場合
				// 全選択ボタン、全解除ボタン、確定使用不可
			...
			if (BoSystemString.Nvl(formVo.Sosin_jyotai).Equals(ConditionSosin_jotai.VALUE_SOSIN_JOTAI1))
			{
				// [送信状態]が「未送信」の場合
				// 送信日FROM、送信日TOを使用不可にする
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Tj030f01_Titlebar", lang);
				header.FormName = formResource.GetString("Tj030f01_FormCaption", lang);
			}
		}
		#endregion
		#endregion
