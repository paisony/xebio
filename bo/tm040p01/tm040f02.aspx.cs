...
namespace com.xebio.bo.Tm040p01.Page
{
  ...
  /// Tm040f02のコードビハインドです。
  /// </summary>
  public partial class Tm040f02Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Tm040f02Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Tm040f02Facade().DoLoad(facadeContext);
								break;
						}
					}
					else
		...
		protected virtual void OnBTNBACK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Tm040f02Facade().DoBTNBACK_FRM(facadeContext);
			...
			// 業務ロジック↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
			Tm040f02Form currentForm = (Tm040f02Form)pageContext.GetFormVO(Tm040p01Constant.PGID, Tm040p01Constant.FORMID_02);
			...
			// 選択された色リンクにフォーカス設定
			focusItem = "M1Iro_nm";
			focusMno = (string)currentForm.Dictionary[Tm040p01Constant.DIC_SELECT_ROWIDX];
		...
		protected virtual void OnBTNENTER_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tm040f02Facade().DoBTNENTER_FRM(facadeContext);
				...
				// 業務ロジック↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
				// 発注マスタ情報をセッションに設定
				Session[OpenTm040p01Cls.KEY_SIZE_SEARCH_RESULT] = facadeContext.GetUserObject(Tm040p01Constant.FCDUO_HACHU_MST_INFO);

				// 画面終了スクリプト出力
				OpenTm040p01Cls.CloseTm040p01(Page, true);
				// 業務ロジック↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					//{
						...
						//FormVOを取得する
						Tm040f02Form tm040f02Form = (Tm040f02Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(tm040f02Form);
			
						//明細部データを表示する
						RenderList(tm040f02Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(tm040f02Form, pageContext.FormInfo, formResource, lang);
					//}
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="tm040f02Form">画面FormVO</param>
		private void ShowListPageInfo(Tm040f02Form tm040f02Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(tm040f02Form.GetList("M1"));
		...
		private void RenderList(
			Tm040f02Form tm040f02Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(tm040f02Form, formInfo, formResource, lang);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = tm040f02Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Tm040f02M1Form tm040f02M1Form = (Tm040f02M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(tm040f02M1Form.M1rowno,formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_cd"),
					DataFormatUtil.GetFormatItem(tm040f02M1Form.M1scan_cd,formInfo["M1scan_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1size_nm"),
					DataFormatUtil.GetFormatItem(tm040f02M1Form.M1size_nm,formInfo["M1size_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1lot_su"),
					DataFormatUtil.GetFormatItem(tm040f02M1Form.M1lot_su,formInfo["M1lot_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1haibunkano_su"),
					DataFormatUtil.GetFormatItem(tm040f02M1Form.M1haibunkano_su,formInfo["M1haibunkano_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1itemsu"),
					DataFormatUtil.GetFormatItem(tm040f02M1Form.M1itemsu,formInfo["M1itemsu"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(tm040f02M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(tm040f02M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(tm040f02M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(tm040f02Form.Stkmodeno,formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Old_jisya_hbn,
				DataFormatUtil.GetFormatItem(tm040f02Form.Old_jisya_hbn,formInfo["Old_jisya_hbn"]));
			ControlUtil.SetControlValue(Bumon_nm,
				DataFormatUtil.GetFormatItem(tm040f02Form.Bumon_nm,formInfo["Bumon_nm"]));
			ControlUtil.SetControlValue(Hinsyu_ryaku_nm,
				DataFormatUtil.GetFormatItem(tm040f02Form.Hinsyu_ryaku_nm,formInfo["Hinsyu_ryaku_nm"]));
			ControlUtil.SetControlValue(Burando_nm,
				DataFormatUtil.GetFormatItem(tm040f02Form.Burando_nm,formInfo["Burando_nm"]));
			ControlUtil.SetControlValue(Maker_hbn,
				DataFormatUtil.GetFormatItem(tm040f02Form.Maker_hbn,formInfo["Maker_hbn"]));
			ControlUtil.SetControlValue(Syonmk,
				DataFormatUtil.GetFormatItem(tm040f02Form.Syonmk,formInfo["Syonmk"]));
			ControlUtil.SetControlValue(Iro_nm,
				DataFormatUtil.GetFormatItem(tm040f02Form.Iro_nm,formInfo["Iro_nm"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Tm040f02Form)base.GetPageContext().GetFormVO());
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Tm040f02_Titlebar", lang);
				header.FormName = formResource.GetString("Tm040f02_FormCaption", lang);
			}
		}
		#endregion
		#endregion
