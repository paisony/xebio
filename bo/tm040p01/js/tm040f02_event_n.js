...
function onSubmit() {
	...
	switch (AdvGB_LastClickItemNm.toUpperCase()) {
		...
		// 業務ロジック↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
		case "Btnback".toUpperCase():	// 戻るボタン
			// 確認メッセージ表示
			var msg = getMessage("W107");
			var yes = function () {	// 「はい」の場合、再試行
				$("#Btnback")[0].click();
			}
			var no = function () {	// 「いいえ」の場合、処理なし
			}
			return boOpenInfoDialog(msg, yes, no);

			break;

		case "Btnenter".toUpperCase():	// 確定ボタン
			// 確認メッセージ表示
