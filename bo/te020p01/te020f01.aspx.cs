...
using com.xebio.bo.Te020p01.Facade;
...
namespace com.xebio.bo.Te020p01.Page
{
  /// <summary>
  /// Te020f01のコードビハインドです。
  /// </summary>
  public partial class Te020f01Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Te020f01Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Te020f01Facade().DoLoad(facadeContext);
								#region 共通ヘッダ処理
								// 一覧画面共通処理	----------
								LoginInfoVO	loginInfVO = LoginInfoUtil.GetLoginInfo();
								Te020f01Form Te020f01Form =	(Te020f01Form)facadeContext.FormVO;
								ControlHeaderStoreCls.InitialSetHeaderStore<Te020f01Form>(loginInfVO, Te020f01Form);
								// 一覧画面共通処理	----------
								//アコーディオンを開いたた状態で表示
				...
				// 単一ファイルダウンロード処理
				if (SessionInfoUtil.GetPgObject(base.GetPageContext().CommandInfo.ProgramId, Te020p01Constant.SESSION_KEY_DOWNLOAD_INFO, Session) != null)
				{
					// ダウンロード情報取得
					DLConditionVO dlVO = SessionInfoUtil.GetPgObject(base.GetPageContext().CommandInfo.ProgramId, Te020p01Constant.SESSION_KEY_DOWNLOAD_INFO, Session) as DLConditionVO;

					// セッション削除
					SessionInfoUtil.RemovePgObject(Te020p01Constant.SESSION_KEY_DOWNLOAD_INFO, base.GetPageContext());
		...
		protected virtual void OnBTNPAGEINS_MINSX(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Te020f01Facade().DoBTNPAGEINS_MINSX(facadeContext);
		...
		protected virtual void OnBTNSIZSTK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				// new Te020f01Facade().DoBTNSIZSTK_FRM(facadeContext);
				...
				if (Session[OpenTm040p01Cls.KEY_SIZE_SEARCH_RESULT] != null)
				{
					...
					facadeContext.SetUserObject(Te020p01Constant.KEY_SIZE_SEARCH_RESULT, Session[OpenTm040p01Cls.KEY_SIZE_SEARCH_RESULT]);

					new Te020f01Facade().DoBTNSIZSTK_FRM(facadeContext);

					// フォーカスインデックスを取得
					focusMno = (string)facadeContext.GetUserObject(Te020p01Constant.KEY_SIZE_FOCUS_INDEX);
				}
				else
				{
					...
					// フォームオブジェクト取得
					Te020f01Form formVo = (Te020f01Form)pageContext.GetFormVO();
					...
					searchHachuVO.Sijitencd	= formVo.Jyuryoten_cd;		// 指示店舗コード（移動出荷マニュアル、返品マニュアル用）
					#endregion

					// サイズ検索画面起動
					OpenTm040p01Cls.OpenTm040p01( Page
												, Tm040p01Constant.FORMID_TE020F01
												, Te020p01Util.GetRowCnt(formVo.GetList("M1"))
		...
		protected virtual void OnBTNROWDEL_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Te020f01Facade().DoBTNROWDEL_FRM(facadeContext);

				// フォーカス行をコードビハインドに戻す
				lastRow = (decimal)facadeContext.GetUserObject(Te020p01Constant.FCDUO_FOCUSROW);
		...
		protected virtual void OnBTNCSV_TORIKOMI_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				// new Te020f01Facade().DoBTNCSV_TORIKOMI_FRM(facadeContext);

				// セッションにCSV取込戻り値が設定されている場合
				if (Session[OpenTm050p01Cls.KEY_CSV_IMPORT_RESULT] != null)
				{
					// セッションからファサードコンテキストに設定
					facadeContext.SetUserObject(Te020p01Constant.KEY_CSV_IMPORT_RESULT, Session[OpenTm050p01Cls.KEY_CSV_IMPORT_RESULT]);
					new Te020f01Facade().DoBTNCSV_TORIKOMI_FRM(facadeContext);

					// フォーカス情報取得
					focusMno = facadeContext.GetUserObject(Te020p01Constant.KEY_CSV_FOCUS_INDEX).ToString();
				}
				else
				{
					...
					Te020f01Form formVo = (Te020f01Form)pageContext.GetFormVO();
					// CSV取込チェック情報取得
					CsvCheckInfoVO csvCheckInfoVO = Te020p01Util.GetCsvCheckVo(formVo);
					// CSV取込画面用にセッションに設定
					Session[OpenTm050p01Cls.KEY_CSV_CHECK_INFO] = csvCheckInfoVO;
					// 取込結果反映用にDictionaryに設定
					formVo.Dictionary[Te020p01Constant.KEY_CSV_CHECK_INFO] = csvCheckInfoVO;
					#endregion

					// CSV取込画面起動
					OpenTm050p01Cls.OpenTm050p01(Page
												, Tm050p01Constant.FORMID_TE020F01
												, Te020p01Util.GetRowCnt(formVo.GetList("M1"))
												, decimal.ToInt32(GetMaxCntCls.GetMaxCnt(facadeContext.CommandInfo.FormId.ToUpper(), "1"))
												, BoSystemConstant.CSVNM_MANUAL_IDO
												, Btncsv_torikomi.ID);
				}
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Te020f01Facade().DoPGR_PGN(facadeContext, pageindex);
		...
		protected virtual void OnBTNENTER_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				new Te020f01Facade().DoBTNENTER_FRM(facadeContext);
				...
				// PDFファイル名取得
				string pdfNm = facadeContext.GetUserObject(Te020p01Constant.FCDUO_PRT_FLNM) as string;

				// サーバファイルフルパス
				string serverPath = string.Format("{0}{1}{2}",
												FilePathManager.GetOutFilePath(Te020p01Constant.PGID),
				...
				SessionInfoUtil.SetPgObject(pageContext.CommandInfo.ProgramId, Te020p01Constant.SESSION_KEY_DOWNLOAD_INFO, dlvo, Session);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					// エラー時の画面表示対応 --------------------------------  end
						...
						//FormVOを取得する
						Te020f01Form te020f01Form = (Te020f01Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(te020f01Form);
			
						//明細部データを表示する
						RenderList(te020f01Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(te020f01Form, pageContext.FormInfo, formResource, lang);
					// エラー時の画面表示対応 --------------------------------  str
					//}
					// エラー時の画面表示対応 --------------------------------  end
						// 明細エラー背景色対応 ----- START
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="te020f01Form">画面FormVO</param>
		private void ShowListPageInfo(Te020f01Form te020f01Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(te020f01Form.GetList("M1"));
		...
		private void RenderList(
			Te020f01Form te020f01Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(te020f01Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(te020f01Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = te020f01Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Te020f01M1Form te020f01M1Form = (Te020f01M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1rowno,formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumon_cd"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1bumon_cd,formInfo["M1bumon_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumonkana_nm"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1bumonkana_nm,formInfo["M1bumonkana_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hinsyu_ryaku_nm"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1hinsyu_ryaku_nm,formInfo["M1hinsyu_ryaku_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1burando_nm"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1burando_nm,formInfo["M1burando_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1jisya_hbn"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1jisya_hbn,formInfo["M1jisya_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1maker_hbn"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1maker_hbn,formInfo["M1maker_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1syonmk"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1syonmk,formInfo["M1syonmk"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1iro_nm"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1iro_nm,formInfo["M1iro_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1size_nm"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1size_nm,formInfo["M1size_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_cd"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1scan_cd,formInfo["M1scan_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1syukka_su"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1syukka_su,formInfo["M1syukka_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1gen_tnk"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1gen_tnk,formInfo["M1gen_tnk"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1genka_kin"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1genka_kin,formInfo["M1genka_kin"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1syukka_su_hdn"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1syukka_su_hdn,formInfo["M1syukka_su_hdn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1genka_kin_hdn"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1genka_kin_hdn,formInfo["M1genka_kin_hdn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(te020f01M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));
		...
		/// </summary>
		/// <param name="te020f01Form">画面FormVO</param>
		private void RenderM1Pager(Te020f01Form te020f01Form)
		{
			Pgr.VirtualItemCount = te020f01Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = te020f01Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = te020f01Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(te020f01Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(te020f01Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Shukkariyu_kbn,
				DataFormatUtil.GetFormatItem(te020f01Form.Shukkariyu_kbn,formInfo["Shukkariyu_kbn"]));
			ControlUtil.SetControlValue(Siji_bango,
				DataFormatUtil.GetFormatItem(te020f01Form.Siji_bango,formInfo["Siji_bango"]));
			ControlUtil.SetControlValue(Kaisya_cd,
				DataFormatUtil.GetFormatItem(te020f01Form.Kaisya_cd,formInfo["Kaisya_cd"]));
			ControlUtil.SetControlValue(Kaisya_nm,
				DataFormatUtil.GetFormatItem(te020f01Form.Kaisya_nm,formInfo["Kaisya_nm"]));
			ControlUtil.SetControlValue(Jyuryoten_cd,
				DataFormatUtil.GetFormatItem(te020f01Form.Jyuryoten_cd,formInfo["Jyuryoten_cd"]));
			ControlUtil.SetControlValue(Juryoten_nm,
				DataFormatUtil.GetFormatItem(te020f01Form.Juryoten_nm,formInfo["Juryoten_nm"]));
			ControlUtil.SetControlValue(Syukka_ymd,
				DataFormatUtil.GetFormatItem(te020f01Form.Syukka_ymd,formInfo["Syukka_ymd"]));
			ControlUtil.SetControlValue(Syukkasuryo_gokei,
				DataFormatUtil.GetFormatItem(te020f01Form.Syukkasuryo_gokei,formInfo["Syukkasuryo_gokei"]));
			ControlUtil.SetControlValue(Genka_kin_gokei,
				DataFormatUtil.GetFormatItem(te020f01Form.Genka_kin_gokei,formInfo["Genka_kin_gokei"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Te020f01Form)base.GetPageContext().GetFormVO());
			...
			{
				// CSV取込ボタン使用可
				ControlCls.Disable(Btncsv_torikomi, false);
			}
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Te020f01_Titlebar", lang);
				header.FormName = formResource.GetString("Te020f01_FormCaption", lang);
			}
		}
		#endregion
		#endregion
