...
function onChange(ev) {
	...
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
		case "Head_tenpo_cd".toUpperCase():	// ヘッダ店舗コード
			...
			break;
		case "Shukkariyu_kbn".toUpperCase():	// 出荷理由
			// 出荷理由のドロップダウン表示制御
			ShukkaRiyuControl();
			break;
		case "Siji_bango".toUpperCase():	// 指示番号
			// フォーマット処理
			formatSijinoIdou(getAdvControlFromItemID("Siji_bango"));
			var sijiBango = getAdvControlFromItemID("Siji_bango");
			if (sijiBango.value.length > 0
				&& sijiBango.value.length < 16) {
				// 10桁は入力エラーとしているため、16桁まで0埋め
				sijiBango.value = LPadZero(sijiBango.value, 16);
			}

			// 名称取得部品
			V02020( document.forms.Te020f01.bocommon$logininfo_copcd	// 出荷会社コード
					,getAdvControlFromItemID("Head_tenpo_cd")			// 出荷店舗コード
					,getAdvControlFromItemID("Siji_bango")				// 移動指示番号
					,getAdvControlFromItemID("Kaisya_cd")				// 入荷会社コード
					,getAdvControlFromItemID("Kaisya_nm")				// 入荷会社名
					,getAdvControlFromItemID("Jyuryoten_cd")			// 入荷店舗コード
					,getAdvControlFromItemID("Juryoten_nm")			// 入荷店舗名
					,getAdvControlFromItemID("Siji_bango")				// 移動指示番号
			);
			break;
		...
		case "M1scan_cd".toUpperCase():
			...
			var condition = {
				...
				, "SYUKAKAISYA_CD": document.forms.Te020f01.bocommon$logininfo_copcd				// 出荷会社コード（移動出荷マニュアル)
		...
		case "Kaisya_cd".toUpperCase():	// 入荷会社コード
			if (ev.key == "Enter" && !ev.shiftKey) {
				...
				if (getAdvControlFromItemID("Jyuryoten_cd").value == "") {
					// 会社コードが未設定の場合
					// 次の項目にフォーカス
					getAdvControlFromItemID("M1scan_cd", 1).focus();
				}
			}
			break;
	default:
...
// 出荷理由の表示制御を行う
function ShukkaRiyuControl() {

	// 本部指示の場合
	if (getAdvControlFromItemID("Shukkariyu_kbn").value == 1) {
		// 指示番号を使用可
