...
function onLoad() {
	...
	if (!focusedElem || focusedElem.id == "") {
		// フォーカスが当たっている項目が存在しない場合

		// フォーカスさせるインデックスを取得
		var focusIndex = -1;
		var i = 0;
		$("#str-result-item-wrap .str-result-item-01").each(function () {
			var chohyoLink = $(this).find(".meisaiLink");

			if (chohyoLink && chohyoLink.length != 0
				&& chohyoLink[0].value != ""
				&& focusIndex < 0) {
				// 明細中に表示されているリンクがあれば、該当行をフォーカス対象とする。
				focusIndex = i;
			}
			i++;
		});
		...
		if (focusIndex >= 0) {
			// インデックスが設定されている場合
			getAdvControlFromItemID("M1tyohyo_nm", focusIndex).focus();
		}
	}
	// 業務ロジック↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
}
