...
namespace com.xebio.bo.Ta020p01.Page
{
  ...
  /// Ta020f02のコードビハインドです。
  /// </summary>
  public partial class Ta020f02Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Ta020f02Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Ta020f02Facade().DoLoad(facadeContext);
								break;
						}
					}
					else
		...
		protected virtual void OnBTNBACK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Ta020f02Facade().DoBTNBACK_FRM(facadeContext);
				...
				if (((Ta020f02Form)pageContext.GetFormVO()).Stkmodeno.Equals(BoSystemConstant.MODE_INSERT))
				{
			...
			if (!((Ta020f02Form)pageContext.GetFormVO()).Stkmodeno.Equals(BoSystemConstant.MODE_INSERT))
			{
				...
				focusMno = (string)((Ta020f02Form)pageContext.GetFormVO(Ta020p01Constant.PGID, Ta020p01Constant.FORMID_02)).Dictionary[Ta020p01Constant.DIC_M1SELCETROWIDX];
		...
		protected virtual void OnBTNZENSTK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta020f02Facade().DoBTNZENSTK_FRM(facadeContext);
		...
		protected virtual void OnBTNZENKJO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta020f02Facade().DoBTNZENKJO_FRM(facadeContext);
		...
		protected virtual void OnBTNROWINS_MADD(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta020f02Facade().DoBTNROWINS_MADD(facadeContext);
		...
		protected virtual void OnBTNPAGEINS_MINSX(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta020f02Facade().DoBTNPAGEINS_MINSX(facadeContext);
				//明細フォーカスセット用インデックスを取得します。
				index = facadeContext.GetUserObject(MdSystemConstant.DETAIL_FOCUS_INDEX) as string;				
		...
		protected virtual void OnBTNSIZSTK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				//new Ta020f02Facade().DoBTNSIZSTK_FRM(facadeContext);
				...
				if (Session[OpenTm040p01Cls.KEY_SIZE_SEARCH_RESULT] != null)
				{
					...
					facadeContext.SetUserObject(Ta020p01Constant.KEY_SIZE_SEARCH_RESULT, Session[OpenTm040p01Cls.KEY_SIZE_SEARCH_RESULT]);

					new Ta020f02Facade().DoBTNSIZSTK_FRM(facadeContext);

					// フォーカスインデックスを取得
					focusMno = (string)facadeContext.GetUserObject(Ta020p01Constant.KEY_SIZE_FOCUS_INDEX);
				}
				else
				{
					...
					OpenTm040p01Cls.OpenTm040p01( Page
												, Tm040p01Constant.FORMID_TA020F02
												, Ta020p01Util.GetRowCnt(formVo.GetList("M1"))
												, dMaxCnt
												, searchHachuVO
												, Btnsizstk.ID);
				}
		...
		protected virtual void OnBTNROWDEL_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta020f02Facade().DoBTNROWDEL_FRM(facadeContext);
				...
				// 明細フォーカスセット用インデックスを取得します。
				index = facadeContext.GetUserObject(Ta020p01Constant.FCDUO_FOCUSROW) as string;
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta020f02Facade().DoPGR_PGN(facadeContext, pageindex);
		...
		protected virtual void OnBTNENTER_DBU(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				new Ta020f02Facade().DoBTNENTER_DBU(facadeContext);
				...
				Ta020f01Form f01VO = (Ta020f01Form)fvm.GetProgramVO(facadeContext.CommandInfo.ProgramId).GetFormVO(Ta020p01Constant.FORMID_01);
				// 明細画面VO
				...
				// 次画面のフォームビーンを設定
				fvm.SetFormVO(Ta020p01Constant.PGID, Ta020p01Constant.FORMID_01, f01VO);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					// エラー時の画面表示対応 --------------------------------  end
						...
						//FormVOを取得する
						Ta020f02Form ta020f02Form = (Ta020f02Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(ta020f02Form);
			
						//明細部データを表示する
						RenderList(ta020f02Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(ta020f02Form, pageContext.FormInfo, formResource, lang);
					// エラー時の画面表示対応 --------------------------------  str
					//}
					// エラー時の画面表示対応 --------------------------------  end
						// 明細エラー背景色対応 ----- START
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="ta020f02Form">画面FormVO</param>
		private void ShowListPageInfo(Ta020f02Form ta020f02Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(ta020f02Form.GetList("M1"));
		...
		private void RenderList(
			Ta020f02Form ta020f02Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(ta020f02Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(ta020f02Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = ta020f02Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Ta020f02M1Form ta020f02M1Form = (Ta020f02M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1rowno,formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumonkana_nm"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1bumonkana_nm,formInfo["M1bumonkana_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hyoka_kb"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1hyoka_kb,formInfo["M1hyoka_kb"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kahi_nm"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1kahi_nm,formInfo["M1kahi_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tenzaiko_su"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1tenzaiko_su,formInfo["M1tenzaiko_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hinsyu_ryaku_nm"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1hinsyu_ryaku_nm,formInfo["M1hinsyu_ryaku_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1nyukayotei_su"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1nyukayotei_su,formInfo["M1nyukayotei_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1uriage_su"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1uriage_su,formInfo["M1uriage_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1burando_nm"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1burando_nm,formInfo["M1burando_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1jido_su"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1jido_su,formInfo["M1jido_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1jisya_hbn"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1jisya_hbn,formInfo["M1jisya_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1syohin_zokusei"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1syohin_zokusei,formInfo["M1syohin_zokusei"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1iro_nm"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1iro_nm,formInfo["M1iro_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1size_nm"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1size_nm,formInfo["M1size_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1maker_hbn"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1maker_hbn,formInfo["M1maker_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1syonmk"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1syonmk,formInfo["M1syonmk"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hatchu_msg"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1hatchu_msg,formInfo["M1hatchu_msg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_cd"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1scan_cd,formInfo["M1scan_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1irai_su"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1irai_su,formInfo["M1irai_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1genkakin"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1genkakin,formInfo["M1genkakin"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1irai_su_hdn"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1irai_su_hdn,formInfo["M1irai_su_hdn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1gen_tnk"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1gen_tnk,formInfo["M1gen_tnk"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1genkakin_hdn"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1genkakin_hdn,formInfo["M1genkakin_hdn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(ta020f02M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));
		...
		/// </summary>
		/// <param name="ta020f02Form">画面FormVO</param>
		private void RenderM1Pager(Ta020f02Form ta020f02Form)
		{
			Pgr.VirtualItemCount = ta020f02Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = ta020f02Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = ta020f02Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(ta020f02Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(ta020f02Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(ta020f02Form.Stkmodeno,formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Irai_ymd,
				DataFormatUtil.GetFormatItem(ta020f02Form.Irai_ymd,formInfo["Irai_ymd"]));
			ControlUtil.SetControlValue(Tantosya_cd,
				DataFormatUtil.GetFormatItem(ta020f02Form.Tantosya_cd,formInfo["Tantosya_cd"]));
			ControlUtil.SetControlValue(Hanbaiin_nm,
				DataFormatUtil.GetFormatItem(ta020f02Form.Hanbaiin_nm,formInfo["Hanbaiin_nm"]));
			ControlUtil.SetControlValue(Irairiyu_cd,
				DataFormatUtil.GetFormatItem(ta020f02Form.Irairiyu_cd,formInfo["Irairiyu_cd"]));
			ControlUtil.SetControlValue(Gokei_irai_su,
				DataFormatUtil.GetFormatItem(ta020f02Form.Gokei_irai_su,formInfo["Gokei_irai_su"]));
			ControlUtil.SetControlValue(Gokei_genkakin,
				DataFormatUtil.GetFormatItem(ta020f02Form.Gokei_genkakin,formInfo["Gokei_genkakin"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Ta020f02Form)base.GetPageContext().GetFormVO());
			...
			#region 画面表示制御
			Ta020f02Form formVo = (Ta020f02Form)base.GetPageContext().GetFormVO();
			...
			|| BoSystemString.Nvl(formVo.Stkmodeno).Equals(BoSystemConstant.MODE_UPD))
			{
				...
				{
					// 依頼理由コード使用可
					ControlCls.Disable(Irairiyu_cd, false);
					// 行追加ボタン非表示
				...
				else
				{
					...
					{
						// 依頼理由コード使用不可
						ControlCls.Disable(Irairiyu_cd, true);
						// 申請の場合、行追加ボタン使用不可
			...
			// 依頼理由を設定
			if (string.IsNullOrEmpty(Irairiyu_cd.Text) || Irairiyu_cd.Text == BoSystemConstant.DROPDOWNLIST_MISENTAKU)
			{
				Irairiyu_cd.SelectedIndex = 1;
			}
		...
		private void SetCardCaption(FormResource formResource, string lang)
		{
			...
			ControlUtil.SetControlValue(Irai_ymd_lbl, 
				DataFormatUtil.GetFormatCaption(formResource.GetString("Irai_ymd", lang), base.GetPageContext().FormInfo["Irai_ymd"]));
				DataFormatUtil.SetMustColorCaption(Irai_ymd_lbl, base.GetPageContext().FormInfo["Irai_ymd"]);
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Ta020f02_Titlebar", lang);
				header.FormName = formResource.GetString("Ta020f02_FormCaption", lang);
			}
		}
		#endregion
		#endregion
