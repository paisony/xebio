...
function onLoad() {
	...
	// 担当者From名称取得時のイベント
	$('#Hanbaiin_nm_from').bind('mdSetAfter', function () {
		// イベント取得時の処理
		fromToCopyLbl("Tantosya_cd", "Hanbaiin_nm");
	});
}
...
function onChange(ev) {
	...
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
		case "Head_tenpo_cd".toUpperCase():	// ヘッダ店舗コード
			...
			break;
		case "Tantosya_cd_from".toUpperCase():// 入力担当者コード
			// 名称取得部品を起動
			V02005(getAdvControlFromItemID("Tantosya_cd_from"), getAdvControlFromItemID("Hanbaiin_nm_from"), null);
			break;
		case "Tantosya_cd_to".toUpperCase():// 入力担当者コード
			// 名称取得部品を起動
			V02005(getAdvControlFromItemID("Tantosya_cd_to"), getAdvControlFromItemID("Hanbaiin_nm_to"), null);
			break;	
		case "M1kengen_kb".toUpperCase():	// 権限区分
			// 明細行番号を取得する
			var lineNo = getItemMNofromCtrl(eventTarget);
			// 操作ありの背景色に変更
			commitColorSet(lineNo);
			break;
		case "M1passwardsyokika".toUpperCase():	// パス初期化
			// 明細行番号を取得する
			var lineNo = getItemMNofromCtrl(eventTarget);
			// 操作ありの背景色に変更
			commitColorSet(lineNo);
...
function onBlur(ev) {
	...
	switch (eventTargetName.toUpperCase()) {
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
	case "Tantosya_cd_from".toUpperCase():	// 担当者FROM
		// FROMの値をTOへコピー
		fromToCopy("Tantosya_cd");
		fromToCopy("Hanbaiin_nm");
		break;
