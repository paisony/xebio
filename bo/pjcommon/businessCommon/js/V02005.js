 ...
 * @param idCTL1 {Object} - 検索条件の値を取得するコントロール1(担当者コード) Object
 * @param nameCTL {Object} - 名称を設定するコントロール(担当者名) Object
 * @param errCTL1 {Object} - コードが存在しない場合にエラー表示になるコントロール1 Object
 */
function V02005_MAIN(idCTL1, nameCTL, errCTL1, delFlg) {
...
//	onBlur_adv(idCTL1);
	...
	V02005_INNER(condition, result, readOnly);
}
function V02005(idCTL1, nameCTL, errCTL1) {
	V02005_MAIN(idCTL1, nameCTL, errCTL1, 1);
}

function V02005_INNER(condition, result, readOnly) {
	searchCommonInner_RO(condition, result, readOnly, "V02005.aspx");
}
