/**
 * V02023 摘要マスタ取得
 *   呼出し例： V02023(getAdvControlFromItemID("id1"), getAdvControlFromItemID("name"), getAdvControlFromItemID("errId1"));
 * @param idCTL1 {Object} - 検索条件の値を取得するコントロール1(摘要コード) Object
 * @param nameCTL {Object} - 名称を設定するコントロール(摘要名称) Object
 * @param errCTL1 {Object} - コードが存在しない場合にエラー表示になるコントロール1 Object
 */
function V02023_MAIN(idCTL1, nameCTL, errCTL1, delFlg) {
...
//	onBlur_adv(idCTL1);
	...
	V02023_INNER(condition, result, readOnly);
}
function V02023(idCTL1, nameCTL, errCTL1) {
	V02023_MAIN(idCTL1, nameCTL, errCTL1, 1);
}

function V02023_INNER(condition, result, readOnly) {
	searchCommonInner_RO(condition, result, readOnly, "V02023.aspx");
}
