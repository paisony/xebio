...
using com.xebio.bo.Td020p01.Facade;
...
namespace com.xebio.bo.Td020p01.Page
{
  /// <summary>
  /// Td020f01のコードビハインドです。
  /// </summary>
  public partial class Td020f01Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Td020f01Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Td020f01Facade().DoLoad(facadeContext);
								#region 共通ヘッダ処理
								// 一覧画面共通処理	----------
								LoginInfoVO	loginInfVO = LoginInfoUtil.GetLoginInfo();
								Td020f01Form Td020f01Form =	(Td020f01Form)facadeContext.FormVO;
								ControlHeaderStoreCls.InitialSetHeaderStore<Td020f01Form>(loginInfVO, Td020f01Form);
								// 一覧画面共通処理	----------
								//アコーディオンを開いたた状態で表示
				...
				// 単一ファイルダウンロード処理
				if (SessionInfoUtil.GetPgObject(base.GetPageContext().CommandInfo.ProgramId, Td020p01Constant.SESSION_KEY_DOWNLOAD_INFO, Session) != null)
				{
					// ダウンロード情報取得
					DLConditionVO dlVO = SessionInfoUtil.GetPgObject(base.GetPageContext().CommandInfo.ProgramId, Td020p01Constant.SESSION_KEY_DOWNLOAD_INFO, Session) as DLConditionVO;

					// セッション削除
					SessionInfoUtil.RemovePgObject(Td020p01Constant.SESSION_KEY_DOWNLOAD_INFO, base.GetPageContext());
		...
		protected virtual void OnBTNPAGEINS_MINSX(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Td020f01Facade().DoBTNPAGEINS_MINSX(facadeContext);
		...
		protected virtual void OnBTNSIZSTK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				// new Td020f01Facade().DoBTNSIZSTK_FRM(facadeContext);
				...
				if (Session[OpenTm040p01Cls.KEY_SIZE_SEARCH_RESULT] != null)
				{
					...
					facadeContext.SetUserObject(Td020p01Constant.KEY_SIZE_SEARCH_RESULT, Session[OpenTm040p01Cls.KEY_SIZE_SEARCH_RESULT]);

					new Td020f01Facade().DoBTNSIZSTK_FRM(facadeContext);

					// フォーカスインデックスを取得
					focusMno = (string)facadeContext.GetUserObject(Td020p01Constant.KEY_SIZE_FOCUS_INDEX);
				}
				else
				{
					...
					// フォームオブジェクト取得
					Td020f01Form formVo = (Td020f01Form)pageContext.GetFormVO();

					// 発注マスタ検索用情報設定
					SearchHachuVO searchHachuVO = new SearchHachuVO();
					searchHachuVO.Tencd = formVo.Head_tenpo_cd;			// 店舗コード
					searchHachuVO.Sijino = formVo.Siji_bango;			// 指示NO（移動出荷マニュアル、返品マニュアル用）
					searchHachuVO.Syukakaisyacd = string.Empty;			// 出荷会社コード（移動出荷マニュアル)
					searchHachuVO.Nyukakaisyacd = string.Empty;			// 入荷会社コード（移動出荷マニュアル)
					searchHachuVO.Sijitencd = formVo.Head_tenpo_cd;		// 指示店舗コード（移動出荷マニュアル、返品マニュアル用）
					#endregion

					// サイズ検索画面起動
					OpenTm040p01Cls.OpenTm040p01( Page
												, Tm040p01Constant.FORMID_TD020F01
												, Td020p01Util.GetRowCnt(formVo.GetList("M1"))
			...
			if (Session[OpenTm040p01Cls.KEY_SIZE_SEARCH_RESULT] != null)
			{
				...
				{
					// Ｍ１店舗コードにフォーカス設定
					SetFocusCls.SetFocus(queryList, "M1tenpo_cd", focusMno);
				}
				// 権限取得部品の戻り値が"FALSE"の場合
				else
				{
		...
		protected virtual void OnBTNROWDEL_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Td020f01Facade().DoBTNROWDEL_FRM(facadeContext);
				...
				// フォーカス行をコードビハインドに戻す
				lastRow = (decimal)facadeContext.GetUserObject(Td020p01Constant.FCDUO_FOCUSROW);
		...
		protected virtual void OnBTNCSV_TORIKOMI_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				// new Td020f01Facade().DoBTNCSV_TORIKOMI_FRM(facadeContext);
				
				// セッションにCSV取込戻り値が設定されている場合
				if (Session[OpenTm050p01Cls.KEY_CSV_IMPORT_RESULT] != null)
				{
					// セッションからファサードコンテキストに設定
					facadeContext.SetUserObject(Td020p01Constant.KEY_CSV_IMPORT_RESULT, Session[OpenTm050p01Cls.KEY_CSV_IMPORT_RESULT]);
					new Td020f01Facade().DoBTNCSV_TORIKOMI_FRM(facadeContext);

					// フォーカス情報取得
					focusMno = facadeContext.GetUserObject(Td020p01Constant.KEY_CSV_FOCUS_INDEX).ToString();
				}
				else
				{
					...
					Td020f01Form formVo = (Td020f01Form)pageContext.GetFormVO();
					// CSV取込チェック情報取得
					CsvCheckInfoVO csvCheckInfoVO = Td020p01Util.GetCsvCheckVo(formVo);
					// CSV取込画面用にセッションに設定
					Session[OpenTm050p01Cls.KEY_CSV_CHECK_INFO] = csvCheckInfoVO;
					// 取込結果反映用にDictionaryに設定
					formVo.Dictionary[Td020p01Constant.KEY_CSV_CHECK_INFO] = csvCheckInfoVO;
					#endregion

					// CSV取込画面起動
					OpenTm050p01Cls.OpenTm050p01(Page
												, Tm050p01Constant.FORMID_TD020F01
												, Td020p01Util.GetRowCnt(formVo.GetList("M1"))
												, decimal.ToInt32(GetMaxCntCls.GetMaxCnt(facadeContext.CommandInfo.FormId.ToUpper(), "1"))
												, BoSystemConstant.CSVNM_MANUAL_HENPIN
												, Btncsv_torikomi.ID);
				}
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Td020f01Facade().DoPGR_PGN(facadeContext, pageindex);
		...
		protected virtual void OnBTNENTER_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Td020f01Facade().DoBTNENTER_FRM(facadeContext);
				...
				// PDFファイル名取得
				string pdfNm = facadeContext.GetUserObject(Td020p01Constant.FCDUO_PRT_FLNM) as string;

				// サーバファイルフルパス
				string serverPath = string.Format("{0}{1}{2}",
												FilePathManager.GetOutFilePath(Td020p01Constant.PGID),
				...
				SessionInfoUtil.SetPgObject(pageContext.CommandInfo.ProgramId, Td020p01Constant.SESSION_KEY_DOWNLOAD_INFO, dlvo, Session);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					// エラー時の画面表示対応 --------------------------------  end
						...
						//FormVOを取得する
						Td020f01Form td020f01Form = (Td020f01Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(td020f01Form);
			
						//明細部データを表示する
						RenderList(td020f01Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(td020f01Form, pageContext.FormInfo, formResource, lang);
					// エラー時の画面表示対応 --------------------------------  str
					//}
					// エラー時の画面表示対応 --------------------------------  end
						// 明細エラー背景色対応 ----- START
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="td020f01Form">画面FormVO</param>
		private void ShowListPageInfo(Td020f01Form td020f01Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(td020f01Form.GetList("M1"));
		...
		private void RenderList(
			Td020f01Form td020f01Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(td020f01Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(td020f01Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = td020f01Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Td020f01M1Form td020f01M1Form = (Td020f01M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1rowno,formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tenpo_cd"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1tenpo_cd,formInfo["M1tenpo_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tenpo_nm"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1tenpo_nm,formInfo["M1tenpo_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumon_cd"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1bumon_cd,formInfo["M1bumon_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumonkana_nm"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1bumonkana_nm,formInfo["M1bumonkana_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hinsyu_ryaku_nm"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1hinsyu_ryaku_nm,formInfo["M1hinsyu_ryaku_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1burando_nm"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1burando_nm,formInfo["M1burando_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1jisya_hbn"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1jisya_hbn,formInfo["M1jisya_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1maker_hbn"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1maker_hbn,formInfo["M1maker_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1syonmk"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1syonmk,formInfo["M1syonmk"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1iro_nm"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1iro_nm,formInfo["M1iro_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1size_nm"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1size_nm,formInfo["M1size_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_cd"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1scan_cd,formInfo["M1scan_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1itemsu"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1itemsu,formInfo["M1itemsu"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1gen_tnk"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1gen_tnk,formInfo["M1gen_tnk"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1genkakin"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1genkakin,formInfo["M1genkakin"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1itemsu_hdn"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1itemsu_hdn,formInfo["M1itemsu_hdn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1genka_kin_hdn"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1genka_kin_hdn,formInfo["M1genka_kin_hdn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(td020f01M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));
			...
			//M1明細部標題を設定する。
				...
				// (M1.HeaderRow.FindControl("M1itemsu_hdn") as Label).Text = 
					// DataFormatUtil.GetFormatListCaption(formResource.GetString("M1itemsu_hdn", lang), base.GetPageContext().FormInfo["M1itemsu_hdn"]);
		...
		/// </summary>
		/// <param name="td020f01Form">画面FormVO</param>
		private void RenderM1Pager(Td020f01Form td020f01Form)
		{
			Pgr.VirtualItemCount = td020f01Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = td020f01Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = td020f01Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(td020f01Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(td020f01Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Henpin_riyu,
				DataFormatUtil.GetFormatItem(td020f01Form.Henpin_riyu,formInfo["Henpin_riyu"]));
			ControlUtil.SetControlValue(Siji_bango,
				DataFormatUtil.GetFormatItem(td020f01Form.Siji_bango,formInfo["Siji_bango"]));
			ControlUtil.SetControlValue(Siiresaki_cd,
				DataFormatUtil.GetFormatItem(td020f01Form.Siiresaki_cd,formInfo["Siiresaki_cd"]));
			ControlUtil.SetControlValue(Siiresaki_ryaku_nm,
				DataFormatUtil.GetFormatItem(td020f01Form.Siiresaki_ryaku_nm,formInfo["Siiresaki_ryaku_nm"]));
			ControlUtil.SetControlValue(Biko,
				DataFormatUtil.GetFormatItem(td020f01Form.Biko,formInfo["Biko"]));
			ControlUtil.SetControlValue(Gokei_suryo,
				DataFormatUtil.GetFormatItem(td020f01Form.Gokei_suryo,formInfo["Gokei_suryo"]));
			ControlUtil.SetControlValue(Genka_kin_gokei,
				DataFormatUtil.GetFormatItem(td020f01Form.Genka_kin_gokei,formInfo["Genka_kin_gokei"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Td020f01Form)base.GetPageContext().GetFormVO());
			...
			{
				// CSV取込ボタン表示
				ControlCls.Visible(Btncsv_torikomiArea, true);
			}
			// 権限取得部品の戻り値が"FALSE"の場合
			else
			{
			...
			for (int index = 0; index < M1.Items.Count; index++)
			{
				...
				{
					// 店舗コード使用可
					ControlCls.Disable(((MDTextBox)M1.Items[index].FindControl("M1tenpo_cd")), false);
					// 店舗ボタン使用可
					ControlCls.Disable(((HtmlInputButton)M1.Items[index].FindControl("M1btntenpocd")), false);
				}
				// 権限取得部品の戻り値が"FALSE"の場合
				else
				{
					// 店舗コード使用不可
					ControlCls.Disable(((MDTextBox)M1.Items[index].FindControl("M1tenpo_cd")), true);
					// 店舗ボタン使用不可
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Td020f01_Titlebar", lang);
				header.FormName = formResource.GetString("Td020f01_FormCaption", lang);
			}
		}
		#endregion
		#endregion
