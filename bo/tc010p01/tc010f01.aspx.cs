...
using com.xebio.bo.Tc010p01.Facade;
...
namespace com.xebio.bo.Tc010p01.Page
{
  /// <summary>
  /// Tc010f01のコードビハインドです。
  /// </summary>
  public partial class Tc010f01Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Tc010f01Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Tc010f01Facade().DoLoad(facadeContext);

								#region 共通ヘッダ処理

								// 一覧画面共通処理 ----------
								LoginInfoVO loginInfVO = LoginInfoUtil.GetLoginInfo();
								Tc010f01Form tb070f01Form = (Tc010f01Form)facadeContext.FormVO;
								ControlHeaderStoreCls.InitialSetHeaderStore<Tc010f01Form>(loginInfVO, tb070f01Form);
								// 一覧画面共通処理 ----------
		...
		protected virtual void OnBTNPRINT_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Tc010f01Facade().DoBTNPRINT_FRM(facadeContext);
				...
				// PDFファイル名を取得
				pdfNm = (string)facadeContext.GetUserObject(Tc010p01Constant.FCDUO_RRT_FLNM);
			...
			string serverPath = string.Format("{0}{1}{2}",
											FilePathManager.GetOutFilePath(Tc010p01Constant.PGID),
											Path.DirectorySeparatorChar,
											pdfNm
											);
			// クライアントファイル名
			string clientNm = string.Format("{0}.{1}",
											BoSystemReport.CreateFileName(BoSystemConstant.REPORTNM_KYAKUTYUNYUKALIST, 2),
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					if (!MessageDisplayUtil.HasError(pageContext))
					{
						...
						//FormVOを取得する
						Tc010f01Form tc010f01Form = (Tc010f01Form)pageContext.GetFormVO();

						//カード部データを表示する
						RenderCard(tc010f01Form, pageContext.FormInfo, formResource, lang);
					}
		...
		/// カードデータを表示する
		/// </summary>
		/// <param name="tc010f01Form">画面FormVO</param>
		/// <param name="formInfo">画面情報</param>
		/// <param name="formResource">画面リソース</param>
		/// <param name="lang">言語設定</param>
		private void RenderCard(
			Tc010f01Form tc010f01Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(tc010f01Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(tc010f01Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Denpyo_jyotai,
				DataFormatUtil.GetFormatItem(tc010f01Form.Denpyo_jyotai,formInfo["Denpyo_jyotai"]));
			ControlUtil.SetControlValue(Nyukayotei_ymd_from,
				DataFormatUtil.GetFormatItem(tc010f01Form.Nyukayotei_ymd_from,formInfo["Nyukayotei_ymd_from"]));
			ControlUtil.SetControlValue(Nyukayotei_ymd_to,
				DataFormatUtil.GetFormatItem(tc010f01Form.Nyukayotei_ymd_to,formInfo["Nyukayotei_ymd_to"]));
			ControlUtil.SetControlValue(Siire_kakutei_ymd_from,
				DataFormatUtil.GetFormatItem(tc010f01Form.Siire_kakutei_ymd_from,formInfo["Siire_kakutei_ymd_from"]));
			ControlUtil.SetControlValue(Siire_kakutei_ymd_to,
				DataFormatUtil.GetFormatItem(tc010f01Form.Siire_kakutei_ymd_to,formInfo["Siire_kakutei_ymd_to"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Tc010f01Form)base.GetPageContext().GetFormVO());
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Tc010f01_Titlebar", lang);
				header.FormName = formResource.GetString("Tc010f01_FormCaption", lang);
			}
		}
		#endregion
		#endregion
