...
function onChange(ev) {
	...
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
		...
		case "Kaishi_jyotai".toUpperCase():		// 開始状態

			// 営業日
			var eigyo_ymd = getAdvControlFromItemID("Eigyo_ymd_hdn").value;
			// 営業日 + 1
			var eigyo_ymd2 = getAdvControlFromItemID("Eigyo_ymd_hdn2").value;

			switch (getAdvControlFromItemID("Kaishi_jyotai").value) {
				// 開始済の場合
				case p_kaishi_jyotai1:
					getAdvControlFromItemID("Baihenkaisi_ymd_from").value = eigyo_ymd;
					getAdvControlFromItemID("Baihenkaisi_ymd_to").value   = eigyo_ymd;
					break;

				// 開始予定の場合
				case p_kaishi_jyotai2:
					getAdvControlFromItemID("Baihenkaisi_ymd_from").value = eigyo_ymd2;
					getAdvControlFromItemID("Baihenkaisi_ymd_to").value   = eigyo_ymd2;
					break;

				default:
					getAdvControlFromItemID("Baihenkaisi_ymd_from").value = "";
					getAdvControlFromItemID("Baihenkaisi_ymd_to").value   = "";
					break;
			}
...
function onBlur(ev) {
	...
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
		...
		case "Bumon_cd_from".toUpperCase():	// 部門コードFROM
			...
			fromToCopy("Bumon_cd");
			break;
		case "Baihenkaisi_ymd_from".toUpperCase():	// 開始日FROM
			// FROMの値をTOへコピー
