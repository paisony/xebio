...
using com.xebio.bo.Tj070p01.Facade;
...
namespace com.xebio.bo.Tj070p01.Page
{
  /// <summary>
  /// Tj070f01のコードビハインドです。
  /// </summary>
  public partial class Tj070f01Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Tj070f01Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Tj070f01Facade().DoLoad(facadeContext);
								...
								ControlHeaderStoreCls.InitialSetHeaderStore<Tj070f01Form>(loginInfVO, tj070f01Form);
								// 一覧画面共通処理 ----------

								if (string.IsNullOrEmpty(tj070f01Form.Modeno))
								{
									// アコーディオンなし
									AccordionUtil.SetAccordionKbn(pageContext, BoSystemConstant.ACCORDION_ST_NONE);
									// モードNoを照会に設定
									tj070f01Form.Modeno = BoSystemConstant.MODE_REF.ToString();
				...
				if (base.GetPageContext() != null)
				{
					...
					Tj070f01Form formVO = (Tj070f01Form)fvm.GetProgramVO(base.GetPageContext().CommandInfo.ProgramId).GetFormVO(Tj070p01Constant.FORMID_01);
		...
		protected virtual void OnBTNSEARCH_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Tj070f01Facade().DoBTNSEARCH_FRM(facadeContext);
				...
				// モードを設定 
				ModeUtil.SetMode(pageContext, ModeCls.GetModeNm(((Tj070f01Form)pageContext.GetFormVO()).Stkmodeno));
		...
		protected virtual void OnBTNPRINT_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj070f01Facade().DoBTNPRINT_FRM(facadeContext);
				...
				// PDFファイル名を取得
				pdfNm = (string)facadeContext.GetUserObject(Tj070p01Constant.FCDUO_RRT_FLNM);					
			...
			string serverPath = string.Format("{0}{1}{2}",
												FilePathManager.GetOutFilePath(Tj070p01Constant.PGID),
												Path.DirectorySeparatorChar,
												pdfNm
												);
			// クライアントファイル名
			string clientNm = string.Format("{0}.{1}",
												BoSystemReport.CreateFileName(BoSystemConstant.REPORTNM_TANAOROSIJOKYOKAKUNINLIST_V, 2),
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj070f01Facade().DoPGR_PGN(facadeContext, pageindex);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					//{
						...
						//FormVOを取得する
						Tj070f01Form tj070f01Form = (Tj070f01Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(tj070f01Form);
			
						//明細部データを表示する
						RenderList(tj070f01Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(tj070f01Form, pageContext.FormInfo, formResource, lang);
					//}
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="tj070f01Form">画面FormVO</param>
		private void ShowListPageInfo(Tj070f01Form tj070f01Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(tj070f01Form.GetList("M1"));
		...
		private void RenderList(
			Tj070f01Form tj070f01Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(tj070f01Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(tj070f01Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = tj070f01Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Tj070f01M1Form tj070f01M1Form = (Tj070f01M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1rowno,formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tenpo_cd"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1tenpo_cd,formInfo["M1tenpo_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tenpo_nm"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1tenpo_nm,formInfo["M1tenpo_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hhtjissi_ymd"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1hhtjissi_ymd,formInfo["M1hhtjissi_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1sosin_kak_ymd"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1sosin_kak_ymd,formInfo["M1sosin_kak_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tenpo_kakutei_jyokyo"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1tenpo_kakutei_jyokyo,formInfo["M1tenpo_kakutei_jyokyo"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tenpo_kakutei_jotai_nm"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1tenpo_kakutei_jotai_nm,formInfo["M1tenpo_kakutei_jotai_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1md_sosin_jyokyo"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1md_sosin_jyokyo,formInfo["M1md_sosin_jyokyo"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1sosin_jotai_nm"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1sosin_jotai_nm,formInfo["M1sosin_jotai_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(tj070f01M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
			...
			//M1明細部標題を設定する。
				...
				// (M1.HeaderRow.FindControl("M1hhtjissi_ymd") as Label).Text = 
					// DataFormatUtil.GetFormatListCaption(formResource.GetString("M1hhtjissi_ymd", lang), base.GetPageContext().FormInfo["M1hhtjissi_ymd"]);
				...
				// (M1.HeaderRow.FindControl("M1tenpo_kakutei_jotai_nm") as Label).Text = 
					// DataFormatUtil.GetFormatListCaption(formResource.GetString("M1tenpo_kakutei_jotai_nm", lang), base.GetPageContext().FormInfo["M1tenpo_kakutei_jotai_nm"]);
				...
				// (M1.HeaderRow.FindControl("M1sosin_jotai_nm") as Label).Text = 
					// DataFormatUtil.GetFormatListCaption(formResource.GetString("M1sosin_jotai_nm", lang), base.GetPageContext().FormInfo["M1sosin_jotai_nm"]);
		...
		/// </summary>
		/// <param name="tj070f01Form">画面FormVO</param>
		private void RenderM1Pager(Tj070f01Form tj070f01Form)
		{
			Pgr.VirtualItemCount = tj070f01Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = tj070f01Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = tj070f01Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(tj070f01Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(tj070f01Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Modeno,
				DataFormatUtil.GetFormatItem(tj070f01Form.Modeno,formInfo["Modeno"]));
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(tj070f01Form.Stkmodeno,formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Tenpo_cd_from,
				DataFormatUtil.GetFormatItem(tj070f01Form.Tenpo_cd_from,formInfo["Tenpo_cd_from"]));
			ControlUtil.SetControlValue(Tenpo_nm_from,
				DataFormatUtil.GetFormatItem(tj070f01Form.Tenpo_nm_from,formInfo["Tenpo_nm_from"]));
			ControlUtil.SetControlValue(Tenpo_cd_to,
				DataFormatUtil.GetFormatItem(tj070f01Form.Tenpo_cd_to,formInfo["Tenpo_cd_to"]));
			ControlUtil.SetControlValue(Tenpo_nm_to,
				DataFormatUtil.GetFormatItem(tj070f01Form.Tenpo_nm_to,formInfo["Tenpo_nm_to"]));
			ControlUtil.SetControlValue(Tenpo_kakutei_jyokyo,
				DataFormatUtil.GetFormatItem(tj070f01Form.Tenpo_kakutei_jyokyo,formInfo["Tenpo_kakutei_jyokyo"]));
			ControlUtil.SetControlValue(Sosin_jyotai,
				DataFormatUtil.GetFormatItem(tj070f01Form.Sosin_jyotai,formInfo["Sosin_jyotai"]));
			ControlUtil.SetControlValue(Hhtjissi_ymd_from,
				DataFormatUtil.GetFormatItem(tj070f01Form.Hhtjissi_ymd_from,formInfo["Hhtjissi_ymd_from"]));
			ControlUtil.SetControlValue(Hhtjissi_ymd_to,
				DataFormatUtil.GetFormatItem(tj070f01Form.Hhtjissi_ymd_to,formInfo["Hhtjissi_ymd_to"]));
			ControlUtil.SetControlValue(Sosin_kak_ymd_from,
				DataFormatUtil.GetFormatItem(tj070f01Form.Sosin_kak_ymd_from,formInfo["Sosin_kak_ymd_from"]));
			ControlUtil.SetControlValue(Sosin_kak_ymd_to,
				DataFormatUtil.GetFormatItem(tj070f01Form.Sosin_kak_ymd_to,formInfo["Sosin_kak_ymd_to"]));
			ControlUtil.SetControlValue(Konkai_flg,
				DataFormatUtil.GetFormatItem(tj070f01Form.Konkai_flg,formInfo["Konkai_flg"]));
			Konkai_flg.Text = formResource.GetString("Konkai_flg", lang);
			ControlUtil.SetControlValue(Meisai_sort,
				DataFormatUtil.GetFormatItem(tj070f01Form.Meisai_sort,formInfo["Meisai_sort"]));
			ControlUtil.SetControlValue(Searchcnt,
				DataFormatUtil.GetFormatItem(tj070f01Form.Searchcnt,formInfo["Searchcnt"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Tj070f01Form)base.GetPageContext().GetFormVO());
			...
			Tj070f01Form formVO = (Tj070f01Form)base.GetPageContext().GetFormVO();
			...
			{
				ControlCls.Disable(Sosin_kak_ymd_from, false);
				ControlCls.Disable(Sosin_kak_ymd_to, false);

			}
			else
			{
				ControlCls.Disable(Sosin_kak_ymd_from, true);
				ControlCls.Disable(Sosin_kak_ymd_to, true);
			}
			...
			for (int i = 0; i < m1List.Count; i++)
			{
				...
				// 店舗確定状況
				if ("1".Equals(m1formVO.M1tenpo_kakutei_jyokyo)
					|| "5".Equals(m1formVO.M1tenpo_kakutei_jyokyo)
					)
				{	
					// 店舗確定状況
					((TextBox)M1.Items[i].FindControl("M1tenpo_kakutei_jyokyo")).ForeColor = Color.Black;
					// 店舗確定状況名称
					((TextBox)M1.Items[i].FindControl("M1tenpo_kakutei_jotai_nm")).ForeColor = Color.Black;
				}
				else
				{
					// 店舗確定状況
					...
					// 店舗確定状況名称
					((TextBox)M1.Items[i].FindControl("M1tenpo_kakutei_jotai_nm")).ForeColor = Color.Red;
				}

				// MD送信状況
				if ("1".Equals(m1formVO.M1md_sosin_jyokyo))
				{	
					// MD送信状況
					((TextBox)M1.Items[i].FindControl("M1md_sosin_jyokyo")).ForeColor = Color.Black;
					// MD送信状況名称
					((TextBox)M1.Items[i].FindControl("M1sosin_jotai_nm")).ForeColor = Color.Black;
}
				else{
					// MD送信状況
					((TextBox)M1.Items[i].FindControl("M1md_sosin_jyokyo")).ForeColor = Color.Red;
					// MD送信状況名称
					((TextBox)M1.Items[i].FindControl("M1sosin_jotai_nm")).ForeColor = Color.Red;
				}
			}
		...
		private void SetCardCaption(FormResource formResource, string lang)
		{
			...
			//ControlUtil.SetControlValue(Hhtjissi_ymd_from_lbl, 
			//	DataFormatUtil.GetFormatCaption(formResource.GetString("Hhtjissi_ymd_from", lang), base.GetPageContext().FormInfo["Hhtjissi_ymd_from"]));
				ControlUtil.SetControlValue(Hhtjissi_ymd_from_lbl, "HHT実施日");
				DataFormatUtil.SetMustColorCaption(Hhtjissi_ymd_from_lbl, base.GetPageContext().FormInfo["Hhtjissi_ymd_from"]);
			ControlUtil.SetControlValue(Hhtjissi_ymd_to_lbl, 
				DataFormatUtil.GetFormatCaption(formResource.GetString("Hhtjissi_ymd_to", lang), base.GetPageContext().FormInfo["Hhtjissi_ymd_to"]));
				DataFormatUtil.SetMustColorCaption(Hhtjissi_ymd_to_lbl, base.GetPageContext().FormInfo["Hhtjissi_ymd_to"]);
			//ControlUtil.SetControlValue(Sosin_kak_ymd_from_lbl, 
			//	DataFormatUtil.GetFormatCaption(formResource.GetString("Sosin_kak_ymd_from", lang), base.GetPageContext().FormInfo["Sosin_kak_ymd_from"]));
				ControlUtil.SetControlValue(Sosin_kak_ymd_from_lbl, "送信日");
				DataFormatUtil.SetMustColorCaption(Sosin_kak_ymd_from_lbl, base.GetPageContext().FormInfo["Sosin_kak_ymd_from"]);
			ControlUtil.SetControlValue(Sosin_kak_ymd_to_lbl, 
				DataFormatUtil.GetFormatCaption(formResource.GetString("Sosin_kak_ymd_to", lang), base.GetPageContext().FormInfo["Sosin_kak_ymd_to"]));
				DataFormatUtil.SetMustColorCaption(Sosin_kak_ymd_to_lbl, base.GetPageContext().FormInfo["Sosin_kak_ymd_to"]);
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Tj070f01_Titlebar", lang);
				header.FormName = formResource.GetString("Tj070f01_FormCaption", lang);
			}
		}
		#endregion
		#endregion
