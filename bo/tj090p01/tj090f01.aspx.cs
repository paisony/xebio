...
using com.xebio.bo.Tj090p01.Facade;
...
namespace com.xebio.bo.Tj090p01.Page
{
  /// <summary>
  /// Tj090f01のコードビハインドです。
  /// </summary>
  public partial class Tj090f01Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Tj090f01Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Tj090f01Facade().DoLoad(facadeContext);
                                ...
                                ControlHeaderStoreCls.InitialSetHeaderStore<Tj090f01Form>(loginInfVO, tj090f01Form);
                                // 一覧画面共通処理 ----------

                                if (string.IsNullOrEmpty(tj090f01Form.Modeno))
                                {
                                    // アコーディオンなし
                                    AccordionUtil.SetAccordionKbn(pageContext, BoSystemConstant.ACCORDION_ST_NONE);
                                    // モードNoを返品確定に設定
                                    tj090f01Form.Modeno = BoSystemConstant.MODE_REF.ToString();
                ...
                if (base.GetPageContext() != null)
                {
                    ...
                    Tj090f01Form formVO = (Tj090f01Form)fvm.GetProgramVO(base.GetPageContext().CommandInfo.ProgramId).GetFormVO(Tj090p01Constant.FORMID_01);
		...
		protected virtual void OnBTNSEARCH_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Tj090f01Facade().DoBTNSEARCH_FRM(facadeContext);
				...
				// モードを設定 
				ModeUtil.SetMode(pageContext, ModeCls.GetModeNm(((Tj090f01Form)pageContext.GetFormVO()).Stkmodeno));
		...
		protected virtual void OnBTNZENSTK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj090f01Facade().DoBTNZENSTK_FRM(facadeContext);
		...
		protected virtual void OnBTNZENKJO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj090f01Facade().DoBTNZENKJO_FRM(facadeContext);
		...
		protected virtual void OnBTNPRINT_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj090f01Facade().DoBTNPRINT_FRM(facadeContext);
                ...
                // PDFファイル名を取得
                pdfNm = (string)facadeContext.GetUserObject(Tj090p01Constant.FCDUO_RRT_FLNM);					
            ...
            string serverPath = string.Format("{0}{1}{2}",
                                            FilePathManager.GetOutFilePath(Tj090p01Constant.PGID),
                                            Path.DirectorySeparatorChar,
                                            pdfNm
                                            );
            // クライアントファイル名
            string clientNm = string.Format("{0}.{1}",
											BoSystemReport.CreateFileName(BoSystemConstant.REPORTNM_TANAOROSIUNMATCHLIST_X,2),
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Tj090f01Facade().DoPGR_PGN(facadeContext, pageindex);
		...
		protected virtual void OnM1ROWNO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
                ...
                // 次画面のフォームVOをファサードに設定
                FormVOManager fvm = new FormVOManager(Session);
                facadeContext.SetUserObject(Tj090p01Constant.FCDUO_NEXTVO, fvm.GetProgramVO(facadeContext.CommandInfo.ProgramId).GetFormVO(Tj090p01Constant.FORMID_02));
                
                new Tj090f01Facade().DoM1ROWNO_FRM(facadeContext);

                // 次画面のフォームビーンを設定
                fvm.SetFormVO(Tj090p01Constant.PGID, Tj090p01Constant.FORMID_02, (Tj090f02Form)facadeContext.GetUserObject(Tj090p01Constant.FCDUO_NEXTVO));
		...
		protected virtual void OnBTNENTER_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				new Tj090f01Facade().DoBTNENTER_FRM(facadeContext);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					//{
						...
						//FormVOを取得する
						Tj090f01Form tj090f01Form = (Tj090f01Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(tj090f01Form);
			
						//明細部データを表示する
						RenderList(tj090f01Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(tj090f01Form, pageContext.FormInfo, formResource, lang);
					//}
						// 明細エラー背景色対応 ----- START
						//エラー判定
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="tj090f01Form">画面FormVO</param>
		private void ShowListPageInfo(Tj090f01Form tj090f01Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(tj090f01Form.GetList("M1"));
		...
		private void RenderList(
			Tj090f01Form tj090f01Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(tj090f01Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(tj090f01Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = tj090f01Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Tj090f01M1Form tj090f01M1Form = (Tj090f01M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1face_no"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1face_no,formInfo["M1face_no"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tana_dan"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1tana_dan,formInfo["M1tana_dan"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kai_su"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1kai_su,formInfo["M1kai_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tensutanaorosinyuryoku_su"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1tensutanaorosinyuryoku_su,formInfo["M1tensutanaorosinyuryoku_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tensutanaorositeisei_su"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1tensutanaorositeisei_su,formInfo["M1tensutanaorositeisei_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tensutanaorositeisei_su_hdn"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1tensutanaorositeisei_su_hdn,formInfo["M1tensutanaorositeisei_su_hdn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1tensutanaorosigokei_su"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1tensutanaorosigokei_su,formInfo["M1tensutanaorosigokei_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_su"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1scan_su,formInfo["M1scan_su"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1teisei_suryo"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1teisei_suryo,formInfo["M1teisei_suryo"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1gokei_suryo"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1gokei_suryo,formInfo["M1gokei_suryo"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1nyuryokutan_nm"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1nyuryokutan_nm,formInfo["M1nyuryokutan_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1riyucomment_nm"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1riyucomment_nm,formInfo["M1riyucomment_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1nyuryoku_ymd"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1nyuryoku_ymd,formInfo["M1nyuryoku_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(tj090f01M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));

				if(!base.CheckUseSelfCustomize()){
                    //((System.Web.UI.HtmlControls.HtmlInputButton)M1.Items[index].FindControl("M1rowno")).Value =
                    //    formResource.GetString("M1rowno", lang);
                    ((System.Web.UI.HtmlControls.HtmlInputButton)M1.Items[index].FindControl("M1rowno")).Value =
                        tj090f01M1Form.Dictionary[Tj090p01Constant.DIC_M1ROWNO].ToString();
			...
			//M1明細部標題を設定する。
				...
				// (M1.HeaderRow.FindControl("M1tensutanaorositeisei_su_hdn") as Label).Text = 
					// DataFormatUtil.GetFormatListCaption(formResource.GetString("M1tensutanaorositeisei_su_hdn", lang), base.GetPageContext().FormInfo["M1tensutanaorositeisei_su_hdn"]);
		...
		/// </summary>
		/// <param name="tj090f01Form">画面FormVO</param>
		private void RenderM1Pager(Tj090f01Form tj090f01Form)
		{
			Pgr.VirtualItemCount = tj090f01Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = tj090f01Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = tj090f01Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(tj090f01Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(tj090f01Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Modeno,
				DataFormatUtil.GetFormatItem(tj090f01Form.Modeno,formInfo["Modeno"]));
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(tj090f01Form.Stkmodeno,formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Face_no_from,
				DataFormatUtil.GetFormatItem(tj090f01Form.Face_no_from,formInfo["Face_no_from"]));
			ControlUtil.SetControlValue(Face_no_to,
				DataFormatUtil.GetFormatItem(tj090f01Form.Face_no_to,formInfo["Face_no_to"]));
			ControlUtil.SetControlValue(Tana_dan_from,
				DataFormatUtil.GetFormatItem(tj090f01Form.Tana_dan_from,formInfo["Tana_dan_from"]));
			ControlUtil.SetControlValue(Tana_dan_to,
				DataFormatUtil.GetFormatItem(tj090f01Form.Tana_dan_to,formInfo["Tana_dan_to"]));
			ControlUtil.SetControlValue(Nyuryokutan_cd,
				DataFormatUtil.GetFormatItem(tj090f01Form.Nyuryokutan_cd,formInfo["Nyuryokutan_cd"]));
			ControlUtil.SetControlValue(Nyuryokutan_nm,
				DataFormatUtil.GetFormatItem(tj090f01Form.Nyuryokutan_nm,formInfo["Nyuryokutan_nm"]));
			ControlUtil.SetControlValue(Nyuryoku_ymd_from,
				DataFormatUtil.GetFormatItem(tj090f01Form.Nyuryoku_ymd_from,formInfo["Nyuryoku_ymd_from"]));
			ControlUtil.SetControlValue(Nyuryoku_ymd_to,
				DataFormatUtil.GetFormatItem(tj090f01Form.Nyuryoku_ymd_to,formInfo["Nyuryoku_ymd_to"]));
			ControlUtil.SetControlValue(Searchcnt,
				DataFormatUtil.GetFormatItem(tj090f01Form.Searchcnt,formInfo["Searchcnt"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Tj090f01Form)base.GetPageContext().GetFormVO());
            ...
            Tj090f01Form formVO = (Tj090f01Form)base.GetPageContext().GetFormVO();
            ...
            for (int i = 0; i < M1.Items.Count; i++)
            {
                ...
                {
                    //Ｍ１フェイスＮｏ
                    ControlCls.Disable(((TextBox)M1.Items[i].FindControl("M1face_no")), true);
                    //Ｍ１棚段
                    ControlCls.Disable(((TextBox)M1.Items[i].FindControl("M1tana_dan")), true);
                    //Ｍ１点数棚卸訂正数
                    ControlCls.Disable(((TextBox)M1.Items[i].FindControl("M1tensutanaorositeisei_su")), true);
                }
                else
                {
                    //Ｍ１フェイスＮｏ
                    ControlCls.Disable(((TextBox)M1.Items[i].FindControl("M1face_no")), false);
                    //Ｍ１棚段
                    ControlCls.Disable(((TextBox)M1.Items[i].FindControl("M1tana_dan")), false);
                    //Ｍ１点数棚卸訂正数
                    ControlCls.Disable(((TextBox)M1.Items[i].FindControl("M1tensutanaorositeisei_su")), false);
                }
            }            
		...
		private void SetCardCaption(FormResource formResource, string lang)
		{
			...
			ControlUtil.SetControlValue(Tana_dan_from_lbl,
				DataFormatUtil.GetFormatCaption("棚段", base.GetPageContext().FormInfo["Tana_dan_from"]));
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Tj090f01_Titlebar", lang);
				header.FormName = formResource.GetString("Tj090f01_FormCaption", lang);
			}
		}
		#endregion
		#endregion
