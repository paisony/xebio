...
function onSubmit() {
	...
	switch (AdvGB_LastClickItemNm.toUpperCase()) {
	    ...
	    case "Btnenter".toUpperCase():
	        ...
	        //// 確認メッセージを表示
	        if (getAdvControlFromItemID("Stkmodeno") == "11")
            {
	            //var yes = function () {
	            //	$("#Btnenter")[0].click();
	            //}
...
function onChange(ev) {
	...
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
	    ...
	    // 数量
	    // -------------------
	    case "M1tensutanaorositeisei_su".toUpperCase():
	        // 明細行番号を取得する
...
function calcRow(lineNo) {
    ...
    // Ｍ１点数棚卸訂正数
    var su = getAdvControlFromItemID("M1tensutanaorositeisei_su", lineNo);
    // Ｍ１点数棚卸訂正数(隠し)
    var suHid = getAdvControlFromItemID("M1tensutanaorositeisei_su_hdn", lineNo);

    // Ｍ１点数棚卸入力数
    var nyuryoku_su = getAdvControlFromItemID("M1tensutanaorosinyuryoku_su", lineNo);
    // Ｍ１点数棚卸合計数
    var gokei_su = getAdvControlFromItemID("M1tensutanaorosigokei_su", lineNo);

    // Ｍ１点数棚卸入力数＋Ｍ１点数棚卸訂正数をＭ１点数棚卸合計数に設定する。
    gokei_su.value = formatComma(ToNumber(unFormatComma(nyuryoku_su.value)) + ToNumber(unFormatComma(su.value)));
