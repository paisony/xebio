...
function onLoad() {
	...
	if (getAdvControlFromItemID("Shuturyoku_kbn")[0].checked == true) {
		getAdvControlFromItemID("Change_ymd_from").value = "";
		getAdvControlFromItemID("Change_ymd_to").value = "";
		itemDisabled(getAdvControlFromItemID("Change_ymd_from"), true);
		itemDisabled(getAdvControlFromItemID("Change_ymd_to"), true);
	}
	// 出力区分が「日付指定」の場合、変更日に前日を設定し入力可能にする。
	else if (getAdvControlFromItemID("Shuturyoku_kbn")[1].checked == true) {

		if (getAdvControlFromItemID("Change_ymd_from").value.length == 0 && getAdvControlFromItemID("Change_ymd_to").value.length == 0)
		{	
			//営業日-1の値を設定する。	
			getAdvControlFromItemID("Change_ymd_from").value = getAdvControlFromItemID("Change_ymd_hdn").value;
			getAdvControlFromItemID("Change_ymd_to").value = getAdvControlFromItemID("Change_ymd_hdn").value;
		}
		itemDisabled(getAdvControlFromItemID("Change_ymd_from"), false);
		itemDisabled(getAdvControlFromItemID("Change_ymd_to"), false);
	}
	
	//md共通処理ロード処理
	md_td060f01_register();
...
function onBlur(ev) {
	...
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
		...
		case "Change_ymd_from".toUpperCase():	// 変更日FROM
			// FROMの値をTOへコピー
			fromToCopy("Change_ymd");
			break;
