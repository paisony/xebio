...
namespace com.xebio.bo.Ta030p01.Page
{
  ...
  /// Ta030f02のコードビハインドです。
  /// </summary>
  public partial class Ta030f02Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if (!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Ta030f02Form());
						switch (commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Ta030f02Facade().DoLoad(facadeContext);
								break;
						}
					}
				}
		...
		protected virtual void OnBTNBACK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				InitFacadeContext(facadeContext);
				new Ta030f02Facade().DoBTNBACK_FRM(facadeContext);

				//// [モードNO]が「照会単品別」の場合
				//Ta030f01Form ta030f01Form = new Ta030f01Form();
			...
			modeNo = (string)((Ta030f02Form)pageContext.GetFormVO(Ta030p01Constant.PGID, Ta030p01Constant.FORMID_02)).Dictionary[Ta030p01Constant.DIC_MODENO];

			// [モードNO]が「照会部門別」の場合
			if (modeNo.Equals(BoSystemConstant.MODE_REF_BUMON))
			{
				// 選択されたM1部門リンクにフォーカス設定
				focusItem = "M1bumon_cd_bo";
				focusMno = (string)((Ta030f02Form)pageContext.GetFormVO(Ta030p01Constant.PGID, Ta030p01Constant.FORMID_02)).Dictionary[Ta030p01Constant.DIC_M1SELCETROWIDX];
			}
			// [モードNO]が「照会単品別」の場合
			else
			{
				LoginInfoVO loginInfVO = LoginInfoUtil.GetLoginInfo();
				decimal KengenConst = 1;

				//本部担当者の場合
				if (loginInfVO.Kengenkbn == KengenConst)
				{
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta030f02Facade().DoPGR_PGN(facadeContext, pageindex);
			...
			if (!base.IsPostBack || base.IsPageValid)
			{
				...
				if (pageContext != null && pageContext.GetFormVO() != null)
				{
					if (!MessageDisplayUtil.HasError(pageContext))
					{
						...
						//FormVOを取得する
						Ta030f02Form ta030f02Form = (Ta030f02Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(ta030f02Form);

						//明細部データを表示する
						RenderList(ta030f02Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(ta030f02Form, pageContext.FormInfo, formResource, lang);
					}
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="ta030f02Form">画面FormVO</param>
		private void ShowListPageInfo(Ta030f02Form ta030f02Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(ta030f02Form.GetList("M1"));
		...
		private void RenderList(
			Ta030f02Form ta030f02Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(ta030f02Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(ta030f02Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = ta030f02Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Ta030f02M1Form ta030f02M1Form = (Ta030f02M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1rowno, formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hojuirai_kbn_nm"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1hojuirai_kbn_nm, formInfo["M1hojuirai_kbn_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1sinsei_jotainm"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1sinsei_jotainm, formInfo["M1sinsei_jotainm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumon_cd_bo"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1bumon_cd_bo, formInfo["M1bumon_cd_bo"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1bumonkana_nm"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1bumonkana_nm, formInfo["M1bumonkana_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hinsyu_ryaku_nm"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1hinsyu_ryaku_nm, formInfo["M1hinsyu_ryaku_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1burando_nm"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1burando_nm, formInfo["M1burando_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1jisya_hbn"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1jisya_hbn, formInfo["M1jisya_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1syohin_zokusei"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1syohin_zokusei, formInfo["M1syohin_zokusei"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1maker_hbn"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1maker_hbn, formInfo["M1maker_hbn"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1syonmk"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1syonmk, formInfo["M1syonmk"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1iro_nm"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1iro_nm, formInfo["M1iro_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1size_nm"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1size_nm, formInfo["M1size_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1scan_cd"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1scan_cd, formInfo["M1scan_cd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1itemsu"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1itemsu, formInfo["M1itemsu"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1kingaku"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1kingaku, formInfo["M1kingaku"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hattyu_ymd"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1hattyu_ymd, formInfo["M1hattyu_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hanbaiin_nm"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1hanbaiin_nm, formInfo["M1hanbaiin_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1selectorcheckbox, formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1entersyoriflg, formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(ta030f02M1Form.M1dtlirokbn, formInfo["M1dtlirokbn"]));
		...
		/// </summary>
		/// <param name="ta030f02Form">画面FormVO</param>
		private void RenderM1Pager(Ta030f02Form ta030f02Form)
		{
			Pgr.VirtualItemCount = ta030f02Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = ta030f02Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = ta030f02Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(ta030f02Form.Head_tenpo_cd, formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(ta030f02Form.Head_tenpo_nm, formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(ta030f02Form.Stkmodeno, formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Gokei_itemsu,
				DataFormatUtil.GetFormatItem(ta030f02Form.Gokei_itemsu, formInfo["Gokei_itemsu"]));
			ControlUtil.SetControlValue(Gokei_kingaku,
				DataFormatUtil.GetFormatItem(ta030f02Form.Gokei_kingaku, formInfo["Gokei_kingaku"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Ta030f02Form)base.GetPageContext().GetFormVO());
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Ta030f02_Titlebar", lang);
				header.FormName = formResource.GetString("Ta030f02_FormCaption", lang);
			}
		}
		#endregion
		#endregion
