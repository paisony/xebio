...
// 業務ロジック↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
		// タブ処理
		case "Btnmodesiirekakutei".toUpperCase():
...
// 業務ロジック↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
		...
		case "Scan_cd".toUpperCase():	// スキャンコード
			...
			// 名称取得部品
			V02004(condition, result, getAdvControlFromItemID("Scan_cd"), false);

			break;
		case "M1nyuka_kakutei_check".toUpperCase():	// Ｍ１入荷確定チェック
...
function itemControl_mode(pModeno) {
	...
	switch (String(modeno)) {
		case c_modesiirekakutei:	// 仕入確定の場合
			// 仕入確定日From 使用不可
			...
			// 確定状態 使用不可
			itemDisabled(getAdvControlFromItemID("Kakutei_jyotai"), true);
			break;
		...
		case c_moderef:		// 照会の場合
			...
			// 確定状態 使用可
			itemDisabled(getAdvControlFromItemID("Kakutei_jyotai"), false);
			break;
