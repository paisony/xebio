...
namespace com.xebio.bo.Ta010p01.Page
{
  ...
  /// Ta010f01のコードビハインドです。
  /// </summary>
  public partial class Ta010f01Page : StandardBasePage
	{
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if(!IsPostBack)
				{
					...
					{
						pageContext.SetFormVO(new Ta010f01Form());
						switch(commandInfo.ActionMode)
						{
							...
							case "UPD":
								...
								new Ta010f01Facade().DoLoad(facadeContext);
								#region 共通ヘッダ処理

								// 一覧画面共通処理	----------
								LoginInfoVO	loginInfVO = LoginInfoUtil.GetLoginInfo();
								Ta010f01Form ta010f01Form =	(Ta010f01Form)facadeContext.FormVO;
								ControlHeaderStoreCls.InitialSetHeaderStore<Ta010f01Form>(loginInfVO, ta010f01Form);
								// 一覧画面共通処理	----------

								if (string.IsNullOrEmpty(ta010f01Form.Modeno))
								{
									// アコーディオンなし
									...
									{ 
										// モードNoを申請に設定
										ta010f01Form.Modeno = BoSystemConstant.MODE_APPLY.ToString();
									...
									{
										// モードNoを照会に設定
										ta010f01Form.Modeno = BoSystemConstant.MODE_REF.ToString();
				...
				if (base.GetPageContext() != null)
				{
					...
					Ta010f01Form f01VO = (Ta010f01Form)fvm.GetProgramVO(base.GetPageContext().CommandInfo.ProgramId).GetFormVO(Ta010p01Constant.FORMID_01);
		...
		protected virtual void OnBTNINSERT_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				// 次画面のフォームVOをファサードに設定
				FormVOManager fvm = new FormVOManager(Session);
				facadeContext.SetUserObject(Ta010p01Constant.FCDUO_NEXTVO, fvm.GetProgramVO(facadeContext.CommandInfo.ProgramId).GetFormVO(Ta010p01Constant.FORMID_02));

				new Ta010f01Facade().DoBTNINSERT_FRM(facadeContext);
				...
				// 次画面のフォームビーンを設定
				fvm.SetFormVO(Ta010p01Constant.PGID, Ta010p01Constant.FORMID_02, (Ta010f02Form)facadeContext.GetUserObject(Ta010p01Constant.FCDUO_NEXTVO));

				// モードを設定 
				ModeUtil.SetMode(pageContext, ModeCls.GetModeNm(((Ta010f01Form)pageContext.GetFormVO()).Stkmodeno));
		...
		protected virtual void OnBTNSEARCH_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta010f01Facade().DoBTNSEARCH_FRM(facadeContext);
		...
		protected virtual void OnBTNZENSTK_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta010f01Facade().DoBTNZENSTK_FRM(facadeContext);
		...
		protected virtual void OnBTNZENKJO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta010f01Facade().DoBTNZENKJO_FRM(facadeContext);
		...
		protected virtual void OnPGR_PGN(object sender, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta010f01Facade().DoPGR_PGN(facadeContext, pageindex);
		...
		protected virtual void OnM1KANRI_NO_FRM(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				new Ta010f01Facade().DoM1KANRI_NO_FRM(facadeContext);
		...
		protected virtual void OnBTNENTER_DBU(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				IFacadeContext facadeContext = FacadeContextFactory.GetFacadeContext(pageContext);
				InitFacadeContext(facadeContext);
				new Ta010f01Facade().DoBTNENTER_DBU(facadeContext);
			...
			if(!base.IsPostBack || base.IsPageValid)
			{
				...
				if(pageContext != null && pageContext.GetFormVO()!=null)
				{
					...
					// エラー時の画面表示対応 --------------------------------  end
						...
						//FormVOを取得する
						Ta010f01Form ta010f01Form = (Ta010f01Form)pageContext.GetFormVO();

						//明細ページ情報を表示する
						ShowListPageInfo(ta010f01Form);
			
						//明細部データを表示する
						RenderList(ta010f01Form, pageContext.FormInfo, formResource, lang);

						//カード部データを表示する
						RenderCard(ta010f01Form, pageContext.FormInfo, formResource, lang);
					// エラー時の画面表示対応 --------------------------------  str
					//}
					// エラー時の画面表示対応 --------------------------------  end
		...
		/// 明細ページ情報を表示する。
		/// </summary>
		/// <param name="ta010f01Form">画面FormVO</param>
		private void ShowListPageInfo(Ta010f01Form ta010f01Form)
		{
			//M1明細ページ情報を表示する。
			ShowM1ListPageInfo(ta010f01Form.GetList("M1"));
		...
		private void RenderList(
			Ta010f01Form ta010f01Form,
			IFormInfo formInfo,
			FormResource formResource,
			string lang)
		{
			//画面のM1明細部データを設定する。
			RenderM1List(ta010f01Form, formInfo, formResource, lang);
			//M1明細部のページャーにページ情報を設定する。
			RenderM1Pager(ta010f01Form);
		...
		{
			//M1明細部データソースを設定する。
			M1.DataSource = ta010f01Form.GetPageViewList("M1");
			M1.DataBind();
			...
			{
				Ta010f01M1Form ta010f01M1Form = (Ta010f01M1Form)m1DataList[index];
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1rowno"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1rowno,formInfo["M1rowno"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hojuirai_kbn_nm"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1hojuirai_kbn_nm,formInfo["M1hojuirai_kbn_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hattyu_ymd"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1hattyu_ymd,formInfo["M1hattyu_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1itemsu"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1itemsu,formInfo["M1itemsu"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1genkakin"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1genkakin,formInfo["M1genkakin"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1hanbaiin_nm"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1hanbaiin_nm,formInfo["M1hanbaiin_nm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1irai_riyu"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1irai_riyu,formInfo["M1irai_riyu"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1sinsei_jotainm"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1sinsei_jotainm,formInfo["M1sinsei_jotainm"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1apply_ymd"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1apply_ymd,formInfo["M1apply_ymd"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1selectorcheckbox"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1selectorcheckbox,formInfo["M1selectorcheckbox"]));
				((System.Web.UI.WebControls.CheckBox)M1.Items[index].FindControl("M1selectorcheckbox")).Text =
					formResource.GetString("M1selectorcheckbox", lang);
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1entersyoriflg"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1entersyoriflg,formInfo["M1entersyoriflg"]));
				ControlUtil.SetControlValue(M1.Items[index].FindControl("M1dtlirokbn"),
					DataFormatUtil.GetFormatItem(ta010f01M1Form.M1dtlirokbn,formInfo["M1dtlirokbn"]));

				if(!base.CheckUseSelfCustomize()){
					// ボタンのValueに管理Noを設定
					((System.Web.UI.HtmlControls.HtmlInputButton)M1.Items[index].FindControl("M1kanri_no")).Value =
						ta010f01M1Form.Dictionary[Ta010p01Constant.DIC_M1KANRI_NO].ToString();
					// 明細背景色の設定
		...
		/// </summary>
		/// <param name="ta010f01Form">画面FormVO</param>
		private void RenderM1Pager(Ta010f01Form ta010f01Form)
		{
			Pgr.VirtualItemCount = ta010f01Form.GetList("M1").RecordCount;
			Pgr.CurrentPageIndex = ta010f01Form.GetList("M1").PageNo - 1;
			Pgr.PageSize = ta010f01Form.GetList("M1").DispRow;
		}
		#endregion
		#endregion
			...
			string lang)
		{
			...
			ControlUtil.SetControlValue(Head_tenpo_cd,
				DataFormatUtil.GetFormatItem(ta010f01Form.Head_tenpo_cd,formInfo["Head_tenpo_cd"]));
			ControlUtil.SetControlValue(Head_tenpo_nm,
				DataFormatUtil.GetFormatItem(ta010f01Form.Head_tenpo_nm,formInfo["Head_tenpo_nm"]));
			ControlUtil.SetControlValue(Modeno,
				DataFormatUtil.GetFormatItem(ta010f01Form.Modeno,formInfo["Modeno"]));
			ControlUtil.SetControlValue(Stkmodeno,
				DataFormatUtil.GetFormatItem(ta010f01Form.Stkmodeno,formInfo["Stkmodeno"]));
			ControlUtil.SetControlValue(Kbn_cd,
				DataFormatUtil.GetFormatItem(ta010f01Form.Kbn_cd,formInfo["Kbn_cd"]));
			ControlUtil.SetControlValue(Hattyu_ymd_from,
				DataFormatUtil.GetFormatItem(ta010f01Form.Hattyu_ymd_from,formInfo["Hattyu_ymd_from"]));
			ControlUtil.SetControlValue(Hattyu_ymd_to,
				DataFormatUtil.GetFormatItem(ta010f01Form.Hattyu_ymd_to,formInfo["Hattyu_ymd_to"]));
			ControlUtil.SetControlValue(Tantosya_cd,
				DataFormatUtil.GetFormatItem(ta010f01Form.Tantosya_cd,formInfo["Tantosya_cd"]));
			ControlUtil.SetControlValue(Hanbaiin_nm,
				DataFormatUtil.GetFormatItem(ta010f01Form.Hanbaiin_nm,formInfo["Hanbaiin_nm"]));
			ControlUtil.SetControlValue(Irairiyu_cd1,
				DataFormatUtil.GetFormatItem(ta010f01Form.Irairiyu_cd1,formInfo["Irairiyu_cd1"]));
			ControlUtil.SetControlValue(Irairiyu_cd2,
				DataFormatUtil.GetFormatItem(ta010f01Form.Irairiyu_cd2, formInfo["Irairiyu_cd2"]));
			ControlUtil.SetControlValue(Shinsei_flg,
				DataFormatUtil.GetFormatItem(ta010f01Form.Shinsei_flg,formInfo["Shinsei_flg"]));
			ControlUtil.SetControlValue(Searchcnt,
				DataFormatUtil.GetFormatItem(ta010f01Form.Searchcnt,formInfo["Searchcnt"]));
		...
		protected override void SetAttribute()
		{
			...
			// UIScreenController controller = new UIScreenController((Ta010f01Form)base.GetPageContext().GetFormVO());
			...
			#region 画面表示制御
			Ta010f01Form formVo = (Ta010f01Form)base.GetPageContext().GetFormVO();
			...
			else
			{
				...
				if (BoSystemString.Nvl(formVo.Modeno).Equals(BoSystemConstant.MODE_APPLY))
				{
					...
					{
						Ta010f01M1Form ta010f01M1Form = (Ta010f01M1Form)m1DataList[index];
						if (ta010f01M1Form.Dictionary[Ta010p01Constant.DIC_M1SHINSEI_FLG].Equals(ConditionSinsei_jotai.VALUE_SINSEI_JOTAI2))
						{
							// 申請済みの場合
							// 選択を使用不可とする。
		...
		private void SetOtherCaption(FormResource formResource, string lang)
		{
			...
			{
				Windowtitle.InnerText = formResource.GetString("Ta010f01_Titlebar", lang);
				header.FormName = formResource.GetString("Ta010f01_FormCaption", lang);
			}
		}
		#endregion
		#endregion
