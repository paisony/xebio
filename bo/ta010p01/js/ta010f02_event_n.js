	...
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
		...
		case "M1scan_cd".toUpperCase():
			...
			var result = {
				...
				,"HOJU_SU": getAdvControlFromItemID("M1irai_syukei", lineNo)				// 依頼集計
				,"SYOHIN_ZOKUSEI": getAdvControlFromItemID("M1syohin_zokusei", lineNo)		// 商品属性
				//,"LOT_SU": getAdvControlFromItemID("M1lot_su", lineNo)						// ロット数
		...
		case "M1irai_su".toUpperCase():
			// 明細行番号を取得する
			var lineNo = getItemMNofromCtrl(eventTarget);
			responseHandle_onAfter(lineNo);
			break;
		// -------------------
		// 区分コード
		// -------------------
		case "Kbn_cd".toUpperCase():
			// 依頼理由のドロップダウン表示制御
			IraiRiyuControl();
			break;
	default:
...
function responseHandle_onAfter(lineNo) {
	...
	if (Number(Nvl(getAdvControlFromItemID("M1jido_su", lineNo).value, "0").replace(/,/g, '')) > 0) {
		...
		&& Number(Nvl(getAdvControlFromItemID("M1uriage_su", lineNo).value, "0").replace(/,/g, '')) <= 0) {
		// 確認メッセージを表示
		var yes = function () {
		...
		// Ｍ１依頼数量＜＝（入荷予定数）の場合、警告表示(W112)する。(メッセージ：入荷予定あり)
			...
			&& Nvl(getAdvControlFromItemID("M1irai_su", lineNo).value, "") != ""
...
function clearRow(lineNo) {
	...
	getAdvControlFromItemID("M1irai_syukei", lineNo).value = '';		// 依頼集計
