...
namespace com.xebio.bo.Tm050p01.Page
{
  /// <summary>
  /// Tm050p01の初期化ページです。
  /// </summary>
  public partial class Tm050p01InitPage : StandardBasePage
	{
		#region メソッド
		...
		protected new void Page_Load(object sender, System.EventArgs e)
		{
			if(!IsPostBack)
			{
				...
				fm.SetProgramVO(pgId, new Tm050p01PgForm());
				...
				if(base.CheckUseSelfCustomize()){
					...
					//して下さい。
					//セルフカスタマイズを使用する場合も下の行を有効にしてください。
					//efm.SetItemInfoFlg(pgId, "Tm050f01", true);
				}
				...
				// 業務ロジック↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
				// プログラムVOを取得
				Tm050p01PgForm pgForm = (Tm050p01PgForm)pageContext.GetParent();

				// 起動パラメータをリクエストから取得
				pgForm.Dictionary[Tm050p01Constant.DIC_FORM_ID] = Request.QueryString[OpenTm050p01Cls.PARAM_FORM_ID];			// 呼出元画面ID
				pgForm.Dictionary[Tm050p01Constant.DIC_CUR_ROW_CNT] = Request.QueryString[OpenTm050p01Cls.PARAM_CUR_ROW_CNT];	// 現在行数
				pgForm.Dictionary[Tm050p01Constant.DIC_MAX_ROW_CNT] = Request.QueryString[OpenTm050p01Cls.PARAM_MAX_ROW_CNT];	// 最大行数
				pgForm.Dictionary[Tm050p01Constant.DIC_CSV_NAME] = Request.QueryString[OpenTm050p01Cls.PARAM_CSV_NAME];			// CSV名称

				// CSVチェック情報をセッションから取得
				pgForm.Dictionary[Tm050p01Constant.DIC_CSV_CHECK_INFO] = Session[OpenTm050p01Cls.KEY_CSV_CHECK_INFO];
				Session.Remove(OpenTm050p01Cls.KEY_CSV_CHECK_INFO);
				// 業務ロジック↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
