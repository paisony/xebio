...
function onSubmit() {
	...
	switch (AdvGB_LastClickItemNm.toUpperCase()) {
		...
		case "Btnseal".toUpperCase():
			...
			// ラベル発行機の必須チェック
			if (getAdvControlFromItemID("Label_ip").value == null
				|| getAdvControlFromItemID("Label_ip").value == "") {
				var msg = getMessage("E119", ["ラベル発行機"]);
				boOpenErrorDialog(msg);
				return false;
			}
