		...
		public void DoBTNPRINT_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				pdfFileNm = string.Format("{0}.{1}",
											BoSystemReport.CreateFileName(BoSystemConstant.REPORTID_KIGYOKANSIIREDENPYO),
				...
				for (int i = 0; i < m1List.Count; i++)
				{
					...
					{
						//入荷会社コード
						inputData.AddScreenParameter(1, f01m1VO.Dictionary[Te130p01Constant.DIC_M1NYUKAKAI_CD].ToString());
						//入荷店コード
						inputData.AddScreenParameter(2, BoSystemFormat.formatTenpoCd(f01m1VO.M1jyuryoten_cd));
						//伝票番号
						inputData.AddScreenParameter(3, f01m1VO.Dictionary[Te130p01Constant.DIC_M1DENPYOBANGO].ToString());
						//入荷日
						inputData.AddScreenParameter(4, BoSystemFormat.formatDate(f01m1VO.M1jyuryo_ymd));
						//履歴
						inputData.AddScreenParameter(5, f01m1VO.Dictionary[Te130p01Constant.DIC_M1RIREKI_NO].ToString());
						//赤黒
						inputData.AddScreenParameter(6, f01m1VO.Dictionary[Te130p01Constant.DIC_M1AKAKURO_KBN].ToString());
					}
				}

				// 帳票を出力
				output = reportCls.MdGeneratePDF(inputData,
												BoSystemConstant.REPORTID_KIGYOKANSIIREDENPYO,
												Te130p01Constant.FORMID_01,
												Te130p01Constant.PGID,
												pdfFileNm
												);
				...
				// PDFをファイルをユーザマップに設定
				facadeContext.UserMap.Add(Te130p01Constant.FCDUO_RRT_FLNM, pdfFileNm);
