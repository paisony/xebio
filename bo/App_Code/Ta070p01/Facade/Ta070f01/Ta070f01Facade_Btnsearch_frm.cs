  ...
  public partial class Ta070f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNSEARCH_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				#region 業務チェック
				// 検索入力チェック
				ChkSelInput(facadeContext, f01VO, Ta070p01Constant.CHECK_MODE_BTNSEARCH);
				// ------------------------------------------------------------------------------------
				...
				// マスタチェック
				ChkSelSingleItem(facadeContext, f01VO, Ta070p01Constant.CHECK_MODE_BTNSEARCH);
				// ------------------------------------------------------------------------------------
				//エラーが発生した場合、その時点でチェックを中止しクライアント側へエラー内容を返却する。
				// ------------------------------------------------------------------------------------
				if (MessageDisplayUtil.HasError(facadeContext))
				...
				// 関連項目チェック
				ChkSelRelatedItem(facadeContext, f01VO, Ta070p01Constant.CHECK_MODE_BTNSEARCH);
				// ------------------------------------------------------------------------------------
				//エラーが発生した場合、その時点でチェックを中止しクライアント側へエラー内容を返却する。
				// ------------------------------------------------------------------------------------
				if (MessageDisplayUtil.HasError(facadeContext))
