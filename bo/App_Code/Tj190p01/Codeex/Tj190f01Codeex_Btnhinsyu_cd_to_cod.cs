...
{
  /// <summary>
  /// フォーム：Tj190f01 項目：Btnhinsyu_cd_to の拡張コード参照クラスです。
  /// </summary>
  [Serializable]
	public class Tj190f01Codeex_Btnhinsyu_cd_to : StandardBaseCodeex
	{
		public Tj190f01Codeex_Btnhinsyu_cd_to()
		{
		}
		...
		public override void Initialize(ICodeContext context)
		{
			...
			// 項目追加
			context.Condition.AddExtItemID("Bumon_cd_to");
		...
		public override void InitSearch(ICodeContext context)
		{
			...
			String sBumonCd = exeValues.InputValues["Bumon_cd_to"] as string;

			if (!string.IsNullOrEmpty(sBumonCd))
			{
				// 検索条件の設定
				context.Condition.AddFindKey("Bumon_cd_to", "BUMON_CD", SearchItemType.Equal);
			}
