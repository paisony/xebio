...
{
  /// <summary>
  /// Tg010f01Request の概要の説明です。
  /// </summary>
  public abstract class Tg010f01Request:StandardBaseRequest, IActionPreProcessor
	{
		/// <summary>
		/// 入力値を取得します。
		/// </summary>
		...
		public virtual void DoGetRequestValue(IPageContext pageContext)
		{
			...
			Tg010f01RequestHelper.GetRequestValue(pageContext);
		}
		...
		{
			Tg010f01RequestHelper.ExtItemInfo(pageContext);
		}
		...
		{
			Tg010f01RequestHelper.Unformat(pageContext);
		}
		...
		{
			Tg010f01RequestHelper.CopyForm(pageContext);
		}
		...
		public virtual void DoValidateInputValue(IPageContext pageContext)
		{
			...
			//カード部の入力項目
			Tg010f01RequestHelper.ValidateCardInputValue(formVO, checker);
			//明細部の入力項目
			if (BoSystemControl.IsM1CommonCheck(pageContext.ButtonInfo))
			{
				Tg010f01RequestHelper.ValidateM1InputValue(formVO, checker);
			}
			if (checker.HasError())
			{
		...
		public virtual void DoValidateCodeValue(IPageContext pageContext)
		{
			...
			//カード部コード存在チェック
			Tg010f01RequestHelper.ValidateCardCodeValue(formVO, codeChecker);
			
			//明細部コード存在チェック
			Tg010f01RequestHelper.ValidateM1CodeValue(formVO, codeChecker);
