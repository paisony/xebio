		...
		public void DoBTNPRINT_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				else
				{
					...
					for (int i = 0; i < m1List.Count; i++)
					{
						...
						if (f01m1VO.M1selectorcheckbox.Equals(BoSystemConstant.CHECKBOX_ON))
						{
							...
							if (   (   BoSystemConstant.KAKUTEI_SB_SHIIRE_MANUAL.Equals((string)f01m1VO.Dictionary[Tb010p01Constant.DIC_M1KAKUTEI_SB])
									|| BoSystemConstant.KAKUTEI_SB_SHIIRE_TSUJO_TEISEI.Equals((string)f01m1VO.Dictionary[Tb010p01Constant.DIC_M1KAKUTEI_SB])
									|| BoSystemConstant.KAKUTEI_SB_SHIIRE_MANUAL_TEISEI.Equals((string)f01m1VO.Dictionary[Tb010p01Constant.DIC_M1KAKUTEI_SB]))
								&& (   CheckKengenCls.CheckKengen(LoginInfoUtil.GetLoginInfo())
									|| BoSystemConstant.SOSINZUMI_FLG_MISOSIN.Equals(f01m1VO.Dictionary[Tb010p01Constant.DIC_M1SOSINZUMI_FLG].ToString()))
								)
							{
								continue;
							}
							...
							{
								// 選択された伝票は印刷できません。
								ErrMsgCls.AddErrMsg("E167", string.Empty, facadeContext, null, f01m1VO.M1rowno, i.ToString(), "M1");
								inputflg = 2;
							}
						}
					}
				...
				for (int i = 0; i < m1List.Count; i++)
				{
					...
					if (f01m1VO.M1selectorcheckbox.Equals(BoSystemConstant.CHECKBOX_ON))
					{
						...
						if(BoSystemConstant.KAKUTEI_SB_SHIIRE_MANUAL.Equals((string)f01m1VO.Dictionary[Tb010p01Constant.DIC_M1KAKUTEI_SB]))
						{
							...
							// 伝票番号
							inputDataManual.AddScreenParameter(3, BoSystemFormat.formatDenpyoNo(f01m1VO.Dictionary[Tb010p01Constant.DIC_M1DENPYO_BANGO].ToString()));
							// 指定納品日
						...
						else if(   BoSystemConstant.KAKUTEI_SB_SHIIRE_TSUJO_TEISEI.Equals((string)f01m1VO.Dictionary[Tb010p01Constant.DIC_M1KAKUTEI_SB])
								|| BoSystemConstant.KAKUTEI_SB_SHIIRE_MANUAL_TEISEI.Equals((string)f01m1VO.Dictionary[Tb010p01Constant.DIC_M1KAKUTEI_SB]))
						{
							// 伝票種別(1:伝票番号、2:元伝票番号)
							...
							// 確定種別
							inputDataTeisei.AddScreenParameter(3, f01m1VO.Dictionary[Tb010p01Constant.DIC_M1KAKUTEI_SB].ToString());
							// 仕入先コード
							inputDataTeisei.AddScreenParameter(4, BoSystemFormat.formatSiiresakiCd(f01m1VO.M1siiresaki_cd));
							// 伝票番号
							inputDataTeisei.AddScreenParameter(5, BoSystemFormat.formatDenpyoNo(f01m1VO.Dictionary[Tb010p01Constant.DIC_M1DENPYO_BANGO].ToString()));
							// 指定納品日
				...
				if (inputDataManual.GetScreenParameterCount() > 0
					&& inputDataTeisei.GetScreenParameterCount() > 0)
				{
					// 複数帳票(ﾏﾆｭｱﾙ仕入伝票＆仕入訂正伝票)
					reportKbn = Tb010p01Constant.REPORT_KBN_ALL;
				}
				else if (inputDataManual.GetScreenParameterCount() > 0)
				{
					// ﾏﾆｭｱﾙ仕入伝票
					reportKbn = Tb010p01Constant.REPORT_KBN_MANUAL;
				}
				else if (inputDataTeisei.GetScreenParameterCount() > 0)
				{
					// 仕入訂正伝票
					reportKbn = Tb010p01Constant.REPORT_KBN_TEISEI;
				}
				f01VO.Dictionary[Tb010p01Constant.DIC_REPORT_KBN] = reportKbn;
				...
				if (inputDataManual.GetScreenParameterCount() > 0)
				{
					...
					output = reportCls.MdGeneratePDF(inputDataManual,
													...
													Tb010p01Constant.FORMID_01,
													Tb010p01Constant.PGID,
													pdfFileNmNaibu
													);
...
//					// 複数帳票の場合
//					if (reportKbn.Equals(Tb010p01Constant.REPORT_KBN_ALL))
//					{
						// PDFファイル名（ダウンロード用）
...
//					{
//						// PDFをファイルをユーザマップに設定
//						facadeContext.UserMap.Add(Tb010p01Constant.FCDUO_RRT_FLNM, pdfFileNmNaibu);
//					}
				}
...
//					}
				...
				facadeContext.UserMap.Add(Tb010p01Constant.FCDUO_RRT_FLNM, pdfFileNmList);
