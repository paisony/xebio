...
using Common.Business.V01000.V01020;
  ...
  public partial class Te020f01Facade : StandardBaseFacade
	{
		...
		/// プログラムID。
		/// </summary>
		private const string PGID = "Te020p01";
		/// <summary>
		/// フォームID。
		/// </summary>
		private const string FORMID = "Te020f01";
		#endregion
		...
		/// インスタンスを生成します。
		/// </summary>
		public Te020f01Facade()
			: base()
		{
		}
		#endregion

		#region Te020f01画面データを作成する。
		/// <summary>
		/// Te020f01画面データを作成する。
		/// </summary>
		...
		public void DoLoad(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				// 出荷理由に"本部指示"を設定
				te020f01Form.Shukkariyu_kbn = ConditionShukkariyu_kbn.VALUE_SHUKKARIYU_KBN1;
				// 会社コード
				te020f01Form.Kaisya_cd = logininfo.CopCd;
				// 会社名称
				te020f01Form.Kaisya_nm = "";
				Hashtable resultHash = new Hashtable();
				resultHash = V01006Check.CheckKaisya(te020f01Form.Kaisya_cd, facadeContext);
				// 名称をラベルに設定
				if (resultHash != null)
				{
					te020f01Form.Kaisya_nm = (string)resultHash["MEISYO_NM"];
				}
				// 出荷日
				// システム日付取得
				SysDateVO sysDateVO = new SysDateVO();
				sysDateVO = SysdateCls.GetSysdateTime(facadeContext);
				te020f01Form.Syukka_ymd = BoSystemFormat.formatDate(sysDateVO.Sysdate);
		...
		public void DoM1ListLoad(IFacadeContext facadeContext)
		{
			...
			// FormVO取得
			// 画面より情報を取得する。
			Te020f01Form f01VO = (Te020f01Form)facadeContext.FormVO;
			...
			// ページを追加
			AddRowCls.AddEmptyRow<Te020f01M1Form>("M1", "M1rowno", (Te020f01Form)facadeContext.FormVO, m1List.DispRow, defVal);
		...
		/// <returns></returns>
		public void DoShohinCopy ( Te020f01M1Form f01m1VO, Hashtable syohinData, string TenpoCd, bool blSize , bool blTorikomi)
		{
			// 発注マスタ検索値をフォームビーン再設定
			...
			// 更新用
			f01m1VO.Dictionary[Te020p01Constant.DIC_M1TENPO_CD] = TenpoCd;												// 店舗コード
			f01m1VO.Dictionary[Te020p01Constant.DIC_M1HINSYU_CD] = syohinData["HINSYU_CD"].ToString();					// 品種コード
			f01m1VO.Dictionary[Te020p01Constant.DIC_M1BURANDO_CD] = syohinData["BURANDO_CD"].ToString();				// ブランドコード
			f01m1VO.Dictionary[Te020p01Constant.DIC_M1IRO_CD] = syohinData["MAKERCOLOR_CD"].ToString();					// 色コード
			f01m1VO.Dictionary[Te020p01Constant.DIC_M1SIZE_CD] = syohinData["SIZE_CD"].ToString();						// サイズコード
			f01m1VO.Dictionary[Te020p01Constant.DIC_M1SYOHIN_CD] = syohinData["SYOHIN_CD"].ToString();					// 商品コード
			f01m1VO.Dictionary[Te020p01Constant.DIC_M1SIIRESAKI_CD] = syohinData["SIIRESAKI_CD"].ToString();			// 仕入先コード
			f01m1VO.Dictionary[Te020p01Constant.DIC_M1SYOHINGUN1_CD] = syohinData["SYOHINGUN1_CD"].ToString();			// 商品群１コード
			f01m1VO.Dictionary[Te020p01Constant.DIC_M1HYOKA_TNK] = syohinData["HYOKA_TNK"].ToString();					// 評価単価
		...
		/// </summary>
		/// <param name="Te020f01Form">f02VO</param>
		/// <returns></returns>
		public void SumGoukeiDetail ( Te020f01Form f02VO )
		{
			decimal dSumSu = 0;		// 合計数量
			decimal dSumKin = 0;	// 合計金額

			foreach (Te020f01M1Form f01m1VO in f02VO.GetList("M1").ListData)
			{
		...
		{
			int inputflg = 0;
			foreach (Te020f01M1Form f01m1VO in m1List.ListData)
			{
				// スキャンコードが入力されている場合
		...
		/// </summary>
		/// <param name="IFacadeContext">facadeContext</param>
		/// <param name="Te020f01Form">f01VO</param>
		/// <param name="IDataList">m1List</param>
		/// <param name="SysDateVO">sysDateVO</param>
		/// <returns></returns>
		private void ChkUpdSingleItem ( IFacadeContext facadeContext, Te020f01Form f01VO, IDataList m1List, SysDateVO sysDateVO )
		{
			// ログイン情報取得
			LoginInfoVO logininfo = LoginInfoUtil.GetLoginInfo();
			...
			if (!string.IsNullOrEmpty(f01VO.Head_tenpo_cd))
			{
				...
				if (resultHash != null)
				{
					...
					if (Te020p01Constant.TENPOKEITAI_KB_LC.Equals(resultHash["TENPOKEITAI_KB"].ToString()))
					{
			...
			{
				if (f01VO.Siji_bango.Length != 16 && f01VO.Siji_bango.Length != 24)
				{
					ErrMsgCls.AddErrMsg("E222", new string[] { "指示番号" }, facadeContext, new[] { "Siji_bango" });
				}
				else
				{
					// 1-4 指示番号
					//     移動指示TBLを検索し、存在しない場合エラー
					if (!string.IsNullOrEmpty(f01VO.Siji_bango))
					{
						Hashtable resultHash = new Hashtable();
						resultHash = V01020Check.CheckIdouSiji(logininfo.CopCd
															, f01VO.Head_tenpo_cd
															, f01VO.Siji_bango
															, BoSystemFormat.IdoSijiNoGetNyukaKaisya(f01VO.Siji_bango)
															, BoSystemFormat.IdoSijiNoGetNyukaTen(f01VO.Siji_bango)
															, facadeContext, "指示番号", new[] { "Siji_bango" });
						// 名称をラベルに設定
						if (resultHash != null)
						{
							f01VO.Kaisya_cd = resultHash["JYURYOKAISYA_CD"].ToString();		// 入荷会社コード
							f01VO.Kaisya_nm = resultHash["JYURYOKAISYA_NM"].ToString();		// 入荷会社名
							f01VO.Jyuryoten_cd = resultHash["JYURYOTEN_CD"].ToString();		// 入荷店舗コード
							f01VO.Juryoten_nm = resultHash["JYURYOTEN_NM"].ToString();		// 入荷店舗名
						}
					}
			...
			// 1-3 指示番号
			//     出荷理由が「本部指示」で未入力の場合、エラー
			if (ConditionShukkariyu_kbn.VALUE_SHUKKARIYU_KBN1.Equals(f01VO.Shukkariyu_kbn))
			{
				if (string.IsNullOrEmpty(f01VO.Siji_bango))
				{
		...
		/// <param name="SysDateVO">sysDateVO</param>
		/// <returns></returns>
		private void ChkUpdSingleItemDetail ( IFacadeContext facadeContext, Te020f01Form f01VO, IDataList m1List, SysDateVO sysDateVO )
		{
			decimal dSumSu = 0;		// 合計数量
			decimal dSumKin = 0;	// 合計金額
			int iCnt = 0;
			...
			foreach (Te020f01M1Form f01m1VO in m1List.ListData)
			{
				...
				if (!string.IsNullOrEmpty(f01m1VO.M1scan_cd.Trim()))
				{
					...
					{
						// 発注マスタ情報設定
						DoShohinCopy(f01m1VO, syohinData, f01VO.Head_tenpo_cd, false, false);
					...
					{
						// 他社の発注MST取得
						FindSqlResultTable stmt = FindSqlUtil.CreateFindSqlResultTable(Te020p01Constant.SQL_ID_03, facadeContext.DBContext);
						...
						BoSystemSql.AddSql(stmt, Te020p01Constant.REP_TABLE_ID, repSql.ToString());
					...
					// 画面の出荷理由が「本部指示」の場合、移動指示TBL(B)に存在するJANコード以外はエラー
					if (!meisaiErr && ConditionShukkariyu_kbn.VALUE_SHUKKARIYU_KBN1.Equals(f01VO.Shukkariyu_kbn))
					{
						SearchHachuVO searchSijiConditionVO = new SearchHachuVO(
							...
							0,						// 依頼集計数(補充) 検索フラグ 0:検索しない 1:検索する
							0,						// 依頼集計数(単品) 検索フラグ 0:検索しない 1:検索する
							1,						// 指示検索 検索フラグ 0:検索しない 1:出荷指示、2:返品指示
							f01VO.Siji_bango,		// 指示NO（移動出荷マニュアル、返品マニュアル用）
							logininfo.CopCd,		// 出荷会社コード（移動出荷マニュアル)
							string.Empty,			// 入荷会社コード（移動出荷マニュアル)
		...
		private bool ChkUpdWarn ( IFacadeContext facadeContext, Te020f01Form f01VO, IDataList m1List )
		{
			bool blWorn = false;
			#region 単項目チェック
			// ログイン情報取得
				...
				if (!string.IsNullOrEmpty(f01m1VO.M1scan_cd.Trim()))
				{
					if (!string.IsNullOrEmpty(f01m1VO.M1syukka_su))
					{
						...
						if (!meisaiErr && !string.IsNullOrEmpty(f01VO.Siji_bango))
						{
							// Ｍ１出荷数量
							//     指示数<数量の場合、警告を行う。
							if (GetSijiSu(facadeContext, f01VO, f01m1VO, logininfo) < Convert.ToDecimal(f01m1VO.M1syukka_su))
							{
								InfoMsgCls.AddWarnMsg("W120", String.Empty, facadeContext, new[] { "M1syukka_su" }
													, f01m1VO.M1rowno, iCnt.ToString(), "M1", m1List.DispRow);
								blWorn = true;
							}
						}
		...
		#region 移動指示理由を取得する。(SQL_ID_01)
		/// <summary>
		/// 移動指示理由を取得する。
		/// </summary>
		/// <param name="facadeContext">ファサードコンテキスト</param>
		/// <returns>移動指示理由</returns>
		public static decimal GetSijiRiyu ( IFacadeContext facadeContext, Te020f01Form f01VO, LoginInfoVO loginInfo )
		{
			decimal dRiyu = Convert.ToDecimal(Te020p01Constant.IDOU_SIJI_RIYU_DEF);

			// [指示番号]が入力されている場合
			if (!string.IsNullOrEmpty(f01VO.Siji_bango))
			{
				// 移動指示理由取得
				FindSqlResultTable rtGet = FindSqlUtil.CreateFindSqlResultTable(Te020p01Constant.SQL_ID_01, facadeContext.DBContext);
				// 指示番号のバインド
				rtGet.BindValue("BIND_SIJI_BANGO", Te020p01Util.GetSijiBango10(f01VO.Siji_bango));
				// 出荷会社コードのバインド
				rtGet.BindValue("BIND_SYUKKAKAISYA_CD", Convert.ToDecimal(BoSystemString.Nvl(loginInfo.CopCd, "0")));
				// 出荷店舗コードのバインド
				rtGet.BindValue("BIND_SYUKKATEN_CD", BoSystemFormat.formatTenpoCd(f01VO.Head_tenpo_cd));
				// 入荷会社コードのバインド
				rtGet.BindValue("BIND_JYURYOKAISYA_CD", Convert.ToDecimal(BoSystemString.Nvl(f01VO.Kaisya_cd, "0")));
				// 入荷店舗コードのバインド
				rtGet.BindValue("BIND_JYURYOTEN_CD", BoSystemFormat.formatTenpoCd(f01VO.Jyuryoten_cd));

				//検索結果を取得します
				rtGet.CreateDbCommand();
				IList<Hashtable> tableListcnt = rtGet.Execute();
				Hashtable resultTbl = tableListcnt[0];
				if (resultTbl != null)
				{

					dRiyu = Convert.ToDecimal(BoSystemString.Nvl(resultTbl["IDORIYU_KB"].ToString(), Te020p01Constant.IDOU_SIJI_RIYU_DEF));
				}
			}
			// [指示番号]が入力されていない場合
			else
			{
				...
				{
					dRiyu = Convert.ToDecimal(Te020p01Constant.IDOU_SIJI_RIYU_CUSTORDER);
				}
				else
				{
					dRiyu = Convert.ToDecimal(Te020p01Constant.IDOU_SIJI_RIYU_DEF);
				}
		...
		#endregion
		#region 移動指示数を取得する。(SQL_ID_02)
		/// <summary>
		/// 移動指示数を取得する。
		/// </summary>
		/// <param name="facadeContext">ファサードコンテキスト</param>
		/// <returns>移動指示数</returns>
		public static decimal GetSijiSu ( IFacadeContext facadeContext, Te020f01Form f01VO, Te020f01M1Form f01M1VO, LoginInfoVO loginInfo )
		{
			decimal dSijiSu = 0;

			// 移動指示数取得
			FindSqlResultTable rtGet = FindSqlUtil.CreateFindSqlResultTable(Te020p01Constant.SQL_ID_02, facadeContext.DBContext);
			// 指示番号のバインド
			rtGet.BindValue("BIND_SIJI_BANGO", Te020p01Util.GetSijiBango10(f01VO.Siji_bango));
			// 出荷会社コードのバインド
			rtGet.BindValue("BIND_SYUKKAKAISYA_CD", Convert.ToDecimal(BoSystemString.Nvl(loginInfo.CopCd, "0")));
			...
			rtGet.BindValue("BIND_JYURYOTEN_CD", BoSystemFormat.formatTenpoCd(f01VO.Jyuryoten_cd));
			// ＪＡＮコードのバインド
			rtGet.BindValue("BIND_JAN_CD", BoSystemFormat.formatJanCd(f01M1VO.M1scan_cd));
			...
			if (resultTbl != null)
			{
				dSijiSu = Convert.ToDecimal(BoSystemString.Nvl(resultTbl["SIJI_SU"].ToString(), "0"));
			}
			return dSijiSu;
		}
		#endregion
		...
		private int Ins_TempTransfer ( IFacadeContext facadeContext, Te020f01Form f01VO, LoginInfoVO loginInfo, SysDateVO sysDateVO)
		{
			int iRownum = 0;
			IDataList m1List = f01VO.GetList("M1");
			...
			// 指示番号設定
			decimal dSijiNo = Te020p01Util.GetSijiBango10(f01VO.Siji_bango);
			// 移動指示理由取得
			decimal dSijiRiyu = GetSijiRiyu(facadeContext, f01VO, loginInfo);
			// 出荷日
			...
			foreach (Te020f01M1Form f01m1VO in f01VO.GetList("M1").ListData)
			{
				// スキャンコードが入力されている行が対象
				if (!string.IsNullOrEmpty(f01m1VO.M1scan_cd.Trim()))
				{
					...
					//  7.確定種別
					BoSystemDb.setInsertVal("KAKUTEI_SB", iRownum.ToString(), Te020p01Constant.KAKUTEI_SB_MANUAL, ref bindDic, ref command);
					//  8.登録日
					...
					// 11.商品群1コード
					BoSystemDb.setInsertVal("SYOHINGUN1_CD", iRownum.ToString(), Convert.ToDecimal(BoSystemString.Nvl(f01m1VO.Dictionary[Te020p01Constant.DIC_M1SYOHINGUN1_CD].ToString(),"0")), ref bindDic, ref command);
					// 12.部門コード
					...
					// 13.品種コード
					BoSystemDb.setInsertVal("HINSYU_CD", iRownum.ToString(), Convert.ToDecimal(BoSystemString.Nvl(f01m1VO.Dictionary[Te020p01Constant.DIC_M1HINSYU_CD].ToString(),"0")), ref bindDic, ref command);
					// 14.ブランドコード
					BoSystemDb.setInsertVal("BURANDO_CD", iRownum.ToString(), BoSystemFormat.formatBrandCd(f01m1VO.Dictionary[Te020p01Constant.DIC_M1BURANDO_CD].ToString()), ref bindDic, ref command);
					// 15.メーカー品番
					...
					// 18.色コード
					BoSystemDb.setInsertVal("IRO_CD", iRownum.ToString(), BoSystemFormat.formatIroCd((string)f01m1VO.Dictionary[Te020p01Constant.DIC_M1IRO_CD]), ref bindDic, ref command);
					// 19.サイズコード
					BoSystemDb.setInsertVal("SIZE_CD", iRownum.ToString(), BoSystemFormat.formatSizeCd((string)f01m1VO.Dictionary[Te020p01Constant.DIC_M1SIZE_CD]), ref bindDic, ref command);
					// 20.サイズ
					...
					// 22.商品コード
					BoSystemDb.setInsertVal("SYOHIN_CD", iRownum.ToString(), (string)f01m1VO.Dictionary[Te020p01Constant.DIC_M1SYOHIN_CD], ref bindDic, ref command);
					// 23.移動実績数
					...
					// 25.仕入先コード
					BoSystemDb.setInsertVal("SIIRESAKI_CD", iRownum.ToString(), BoSystemFormat.formatSiiresakiCd(f01m1VO.Dictionary[Te020p01Constant.DIC_M1SIIRESAKI_CD].ToString()), ref bindDic, ref command);
					// 26.移動理由
					BoSystemDb.setInsertVal("IDO_RIYU", iRownum.ToString(), f01VO.Shukkariyu_kbn , ref bindDic, ref command);
					// 27.出荷指示No
					BoSystemDb.setInsertVal("SYUKKA_SIJI_NO", iRownum.ToString(), dSijiNo , ref bindDic, ref command);
					// 28.防止期限
					BoSystemDb.setInsertVal("STOP_YMD", iRownum.ToString(), 0, ref bindDic, ref command);
					// 29.評価単価
					BoSystemDb.setInsertVal("HYOKA_TNK", iRownum.ToString(), Convert.ToDecimal(BoSystemString.Nvl(f01m1VO.Dictionary[Te020p01Constant.DIC_M1HYOKA_TNK].ToString(), "0")), ref bindDic, ref command);
					// 30.移動指示理由
