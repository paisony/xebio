  ...
  public partial class Te020f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNENTER_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				if (!Te020p01Constant.FLG_ON.Equals(warninngFlg))
				{
					#region 業務チェック
					// 行数チェック
				...
				output = reportCls.MdGeneratePDF(inputData,
												chohyoId,
												Te020p01Constant.FORMID_01,
												Te020p01Constant.PGID,
												pdfFileNm
												);
				...
				// PDFをファイルをユーザマップに設定
				facadeContext.UserMap.Add(Te020p01Constant.FCDUO_PRT_FLNM, pdfFileNm);
