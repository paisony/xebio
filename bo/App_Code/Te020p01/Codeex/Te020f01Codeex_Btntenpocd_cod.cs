...
namespace com.xebio.bo.Te020p01.Codeex
{
  /// <summary>
  /// フォーム：Te020f01 項目：Btntenpocd の拡張コード参照クラスです。
  /// </summary>
  [Serializable]
	public class Te020f01Codeex_Btntenpocd : StandardBaseCodeex
	{
		public Te020f01Codeex_Btntenpocd()
		{
		}
		...
		public override void Initialize(ICodeContext context)
		{
			...
			context.Condition.AddExtItemID("Kaisya_cd");
		...
		public override void InitSearch(ICodeContext context)
		{
			...
			// 拡張方法の詳細はマニュアルおよびサンプルを参照してください。
			//検索条件追加
			decimal dKaisya = decimal.Zero;
			if (context.ExeValue.InputValues.ContainsKey("Kaisya_cd")
				&& context.ExeValue.InputValues["Kaisya_cd"] != null)
			{
				// 親画面の値が取得できた場合
				dKaisya = Convert.ToDecimal(BoSystemString.Nvl(context.ExeValue.InputValues["Kaisya_cd"].ToString(), "0"));
			}
			context.Condition.AddFindKey("KAISYA_CD = " + dKaisya.ToString() + " AND SAKUJYO_FLG = 0");
