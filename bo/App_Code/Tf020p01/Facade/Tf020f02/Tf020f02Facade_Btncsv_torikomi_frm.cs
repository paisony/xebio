  ...
  public partial class Tf020f02Facade : StandardBaseFacade
	{
		...
		public void DoBTNCSV_TORIKOMI_FRM(IFacadeContext facadeContext)
		{
			...
			//	BeginTransactionWithConnect(facadeContext);
				...
				//以下に業務ロジックを記述する。
				// CSV取込にて取り込んだレコードを取得
				IList<Hashtable> csvInfo = (IList<Hashtable>)facadeContext.GetUserObject(Tf020p01Constant.DIC_CSV_IMPORT_RESULT);

				// フォームオブジェクト取得
				Tf020f02Form form = (Tf020f02Form)facadeContext.FormVO;
				IDataList m1List = form.GetList("M1");

				// CSVチェック情報取得
				CsvCheckInfoVO checkInfo = (CsvCheckInfoVO)form.Dictionary[Tf020p01Constant.DIC_CSV_CHECK_INFO];
				...
				{
					Tf020f02M1Form m1Form = (Tf020f02M1Form)m1List[index];
				...
				{
					// ページ追加処理
					AddRowCls.AddEmptyRow<Tf020f02M1Form>("M1", "M1rowno", form, m1List.DispRow);
				}
				m1List.SetPage(dispPageNo);
				// フォーカス行インデックス設定
				facadeContext.SetUserObject(Tf020p01Constant.DIC_FOCUS_INDEX, focusIndex.ToString());
		...
		private int addRequireRow(Tf020f02Form form, int addCnt)
		{
			// 有効現在行数
			int curRowCnt = 0;
			...
			if (requiredCnt > 0)
			{
				...
				AddRowCls.AddEmptyRow<Tf020f02M1Form>("M1", "M1rowno", form, addRowCnt, defVal);
			}
		...
		/// </summary>
		/// <param name="form">一覧画面のVO</param>
		private void setGokei(Tf020f02Form form)
		{
