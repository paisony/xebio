  ...
  public partial class Tf020f02Facade : StandardBaseFacade
	{
		...
		public void DoBTNROWINS_MADD(IFacadeContext facadeContext)
		{
			...
			IDataList m1List = tf020f02Form.GetList("M1");
			...
			// モードが「新規登録」の場合
			if (BoSystemConstant.MODE_INSERT.Equals(tf020f02Form.Stkmodeno))
			{
				edaban = "1";
			}
			// モードが「修正」の場合
			...
			// 行追加
			AddRowCls.AddEmptyRow<Tf020f02M1Form>("M1", "M1rowno", (Tf020f02Form)facadeContext.FormVO, addCount);

			////行オブジェクトを生成し、リストに追加する。
			//Tf020f02M1Form tf020f02M1Form = new Tf020f02M1Form();
			//tf020f02M1Form.Commode = DbuModeCode.INSERT;
	
			//m1List.Add(tf020f02M1Form, true);
			//m1List.SetBtmPage();
