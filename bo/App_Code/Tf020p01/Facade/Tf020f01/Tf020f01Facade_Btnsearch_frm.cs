  ...
  public partial class Tf020f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNSEARCH_FRM(IFacadeContext facadeContext)
		{
			...
			//	BeginTransactionWithConnect(facadeContext);
				...
				// 修正、取消モードの場合、
				if (f01VO.Modeno.Equals(BoSystemConstant.MODE_UPD) || f01VO.Modeno.Equals(BoSystemConstant.MODE_DEL))
				{
					// 承認状態、確定日の条件を初期化
				...
				// [選択モードNo]が「修正」、「取消」、または[選択モードNo]が「照会」、かつ[承認状態]が"未決裁"の場合
				// 経費振替申請テーブルから検索する。
				...
				{
					sSqlId_chk = Tf020p01Constant.SQL_ID_01;
					sSqlId_src = Tf020p01Constant.SQL_ID_03;
					sTblId1 = Tf020p01Constant.TABLE_ID_MDAT0020;	// 経費振替申請テーブル(H)
				}
				...
				{
					sSqlId_chk = Tf020p01Constant.SQL_ID_01;
					sSqlId_src = Tf020p01Constant.SQL_ID_04;
					sTblId1 = Tf020p01Constant.TABLE_ID_MDAT0030;	// 経費振替確定テーブル(H)
				}
				// [選択モードNo]が「照会」、かつ[承認状態]が空白の場合
				// 経費振替申請テーブル、経費振替確定テーブルから検索する。
				else
				{
					sSqlId_chk = Tf020p01Constant.SQL_ID_02;
					sSqlId_src = Tf020p01Constant.SQL_ID_05;
					sTblId1 = Tf020p01Constant.TABLE_ID_MDAT0020;	// 経費振替申請テーブル(H)
					sTblId2 = Tf020p01Constant.TABLE_ID_MDAT0030;	// 経費振替確定テーブル(H)
				}
				...
				BoSystemSql.AddSql(rtChk, Tf020p01Constant.SQL_ID_01_REP_TABLE_ID1, sTblId1 + " T1", bindList);

				// [選択モードNo]が「照会」、かつ[承認状態]が空白の場合
				if (!string.IsNullOrEmpty(sTblId2))
				{
					BoSystemSql.AddSql(rtChk, Tf020p01Constant.SQL_ID_01_REP_TABLE_ID2, sTblId2 + " T1", bindList);
				}

				#endregion

				// 検索条件設定
				this.AddWhere(f01VO, rtChk, sTblId1, Tf020p01Constant.SQL_ID_01_REP_ADD_WHERE1);

				// [選択モードNo]が「照会」、かつ[承認状態]が空白の場合
				if (!string.IsNullOrEmpty(sTblId2))
				{
					this.AddWhere(f01VO, rtChk, sTblId2, Tf020p01Constant.SQL_ID_01_REP_ADD_WHERE2);
				}
				...
				// テーブルIDを設定 -----------
				BoSystemSql.AddSql(rtSeach, Tf020p01Constant.SQL_ID_01_REP_TABLE_ID1, sTblId1 + " T1", bindList);

				// [選択モードNo]が「照会」、かつ[承認状態]が空白の場合
				if (!string.IsNullOrEmpty(sTblId2))
				{
					BoSystemSql.AddSql(rtSeach, Tf020p01Constant.SQL_ID_01_REP_TABLE_ID2, sTblId2 + " T1", bindList);
				}

				#endregion

				// 検索条件設定
				this.AddWhere(f01VO, rtSeach, sTblId1, Tf020p01Constant.SQL_ID_01_REP_ADD_WHERE1);

				// [選択モードNo]が「照会」、かつ[承認状態]が空白の場合
				if (!string.IsNullOrEmpty(sTblId2))
				{
					this.AddWhere(f01VO, rtSeach, sTblId2, Tf020p01Constant.SQL_ID_01_REP_ADD_WHERE2);
				}
				...
				{
					iCnt++;
					Tf020f01M1Form f01m1VO = new Tf020f01M1Form();
					...
					f01m1VO.Dictionary[Tf020p01Constant.DIC_M1DENPYO_BANGO] = string.Empty;
					if (!"0".Equals(rec["DENPYO_BANGO"].ToString()))
					{
						f01m1VO.Dictionary[Tf020p01Constant.DIC_M1DENPYO_BANGO] = BoSystemFormat.formatDenpyoNo(rec["DENPYO_BANGO"].ToString());

					}																									// Ｍ１伝票番号リンク
					f01m1VO.M1kamoku_cd = string.Empty;
					if (!"0".Equals(rec["KAMOKU_CD"].ToString()))
					{
						f01m1VO.M1kamoku_cd = rec["KAMOKU_CD"].ToString();
					}																									// Ｍ１科目コード
					...
					f01m1VO.M1baika_tnk = rec["BAIKAMONEY"].ToString();													// Ｍ１売価金額
					...
					// Dictionary
					f01m1VO.Dictionary.Add(Tf020p01Constant.DIC_M1TBLFLG, rec["TBLFLG"].ToString());					// Ｍ１テーブル区分
					f01m1VO.Dictionary.Add(Tf020p01Constant.DIC_M1SYORI_YMD, rec["SYORI_YMD"].ToString());				// Ｍ１処理日付
					f01m1VO.Dictionary.Add(Tf020p01Constant.DIC_M1SYORI_TM, rec["SYORI_TM"].ToString());				// Ｍ１処理時間
					f01m1VO.Dictionary.Add(Tf020p01Constant.DIC_M1SINSEIRIYU_KB, rec["SINSEIRIYU_KB"].ToString());		// Ｍ１申請理由区分
					f01m1VO.Dictionary.Add(Tf020p01Constant.DIC_M1SINSEIRIYU, rec["SINSEIRIYU"].ToString());			// Ｍ１申請理由
					f01m1VO.Dictionary.Add(Tf020p01Constant.DIC_M1UPD_YMD, rec["UPD_YMD"].ToString());					// Ｍ１更新日
					f01m1VO.Dictionary.Add(Tf020p01Constant.DIC_M1UPD_TM, rec["UPD_TM"].ToString());					// Ｍ１更新時間
					f01m1VO.Dictionary.Add(Tf020p01Constant.DIC_M1SYONINSTATUS, rec["SYONINSTATUS"].ToString());		// Ｍ１承認状態名称
		...
		/// <returns></returns>
		private void AddWhere(Tf020f01Form f01VO, FindSqlResultTable reader, String table_id, String add_where_id)
		{
			ArrayList bindList = new ArrayList();
			...
			// 経費振替申請テーブルの場合
			if (Tf020p01Constant.TABLE_ID_MDAT0020.Equals(table_id))
			{
			...
			// 経費振替確定テーブルの場合
			if (Tf020p01Constant.TABLE_ID_MDAT0030.Equals(table_id))
			{
				// 確定日FROMを設定
		...
		/// <param name="FindSqlResultTable">rtChk</param>
		/// <returns></returns>
		private void addOrderBy(Tf020f01Form f01VO, FindSqlResultTable reader)
		{
			...
			BoSystemSql.AddSql(reader, Tf020p01Constant.SQL_ID_01_REP_ADD_ORDER1, sRepSql.ToString());
