  ...
  public partial class Th010f01Facade : StandardBaseFacade
	{
		...
		#region フォームを呼び出します。(ボタンID : M1old_jisya_hbn)
		/// <summary>
		/// フォームを呼び出します。
		...
		public void DoM1OLD_JISYA_HBN_FRM(IFacadeContext facadeContext)
		{
			...
			//メソッドの開始処理を実行する。
			StartMethod(facadeContext, this.GetType().Name + ".DoM1OLD_JISYA_HBN_FRM");
			...
			try
			{
				...
				if (prevVo.Syohinmst_serchstk.Equals(ConditionSyohinmst_serchstk1.VALUE_SYOHINMST_SERCHSTK13))
				{
					...
					if (prevM1Vo.Dictionary[Th010p01Constant.DIC_M1TENKAI_KB].ToString().Equals("2"))
					{
						...
						#region 抽出条件設定
						sRepSql.Append(" AND	SYOHIN.OLD_XEBIO_CD = :BIND_XEBIO_CD ");
						...
						bindVO.Value = BoSystemFormat.formatJisyaHbn(prevM1Vo.Dictionary[Th010p01Constant.DIC_M1OLD_XEBIO_CD].ToString());
				...
				if (prevVo.Syohinmst_serchstk.Equals(ConditionSyohinmst_serchstk1.VALUE_SYOHINMST_SERCHSTK12))
				{
					...
					nextVo02.Dictionary[Th010p01Constant.DIC_M1ID] = "M1old_jisya_hbn";
				...
				if (prevVo.Syohinmst_serchstk.Equals(ConditionSyohinmst_serchstk1.VALUE_SYOHINMST_SERCHSTK13))
				{
					...
					nextVo03.Dictionary[Th010p01Constant.DIC_M1ID] = Th010p01Constant.M1OLD_JISYA_HBN;
			...
			//メソッドの終了処理を実行する。
			EndMethod(facadeContext, this.GetType().Name + ".DoM1OLD_JISYA_HBN_FRM");
