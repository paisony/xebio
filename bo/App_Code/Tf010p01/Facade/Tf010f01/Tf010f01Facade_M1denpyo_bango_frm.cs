  ...
  public partial class Tf010f01Facade : StandardBaseFacade
	{
		...
		public void DoM1DENPYO_BANGO_FRM(IFacadeContext facadeContext)
		{
			...
			//	BeginTransactionWithConnect(facadeContext);
				...
				// FormVO取得
				// 画面より情報を取得する。
				Tf010f01Form prevVo = (Tf010f01Form)facadeContext.FormVO;
				Tf010f02Form nextVo = (Tf010f02Form)facadeContext.GetUserObject(Tf010p01Constant.FCDUO_NEXTVO);

				IDataList prevM1List = prevVo.GetList("M1");
				IDataList nextM1List = nextVo.GetList("M1");

				// 選択行の情報を取得する。
				Tf010f01M1Form prevM1Vo = (Tf010f01M1Form)prevM1List.GetPageViewList()[facadeContext.CommandInfo.ListIndex];
				...
				if ("1".Equals((string)prevM1Vo.Dictionary[Tf010p01Constant.DIC_M1TBLFLG]))
				{
					sSqlId = Tf010p01Constant.SQL_ID_06;
				}
				else
				{
					sSqlId = Tf010p01Constant.SQL_ID_07;
				}
				...
				// バインド値の置き換え
				// 店舗コード
				rtSeach.BindValue(Tf010p01Constant.SQL_ID_06_REP_TENPO_CD, BoSystemFormat.formatTenpoCd(prevM1Vo.M1shinsei_tenpo_cd));
				// 処理日
				rtSeach.BindValue(Tf010p01Constant.SQL_ID_06_REP_SYORI_YMD, Convert.ToDecimal((string)prevM1Vo.Dictionary[Tf010p01Constant.DIC_M1SYORI_YMD]));
				// 伝票番号
				rtSeach.BindValue(Tf010p01Constant.SQL_ID_06_REP_DENPYO_BANGO, Convert.ToDecimal((string)prevM1Vo.Dictionary[Tf010p01Constant.DIC_M1DENPYO_BANGO]));
				...
				{
					Tf010f02M1Form f02m1VO = new Tf010f02M1Form();
					...
					f02m1VO.M1baika_kin = (Convert.ToDecimal(f02m1VO.M1genbaika_tnk) * Convert.ToDecimal(f02m1VO.M1suryo)).ToString();
																										// Ｍ１売価金額
					f02m1VO.M1suryo_hdn = f02m1VO.M1suryo;												// Ｍ１数量(隠し)
					f02m1VO.M1genka_kin_hdn = f02m1VO.M1genka_kin;										// Ｍ１原価金額(隠し)
					f02m1VO.M1baika_kin_hdn = f02m1VO.M1baika_kin;										// Ｍ１売価金額(隠し)
					...
					// ディクショナリ
					f02m1VO.Dictionary[Tf010p01Constant.DIC_M1HINSYU_CD] = BoSystemFormat.formatHinsyuCd(rec["HINSYU_CD"].ToString());		// 品種コード
					f02m1VO.Dictionary[Tf010p01Constant.DIC_M1IRO_CD] = BoSystemFormat.formatIroCd(rec["IRO_CD"].ToString());				// 色コード
					f02m1VO.Dictionary[Tf010p01Constant.DIC_M1SIZE_CD] = BoSystemFormat.formatSizeCd(rec["SIZE_CD"].ToString());			// サイズコード
					f02m1VO.Dictionary[Tf010p01Constant.DIC_M1SYOHIN_CD] = rec["SYOHIN_CD"].ToString();										// 商品コード
					f02m1VO.Dictionary[Tf010p01Constant.DIC_M1BURANDO_CD] = BoSystemFormat.formatBrandCd(rec["BURANDO_CD"].ToString());		// ブランドコード
				...
				nextVo.Genka_kin_gokei1 = dKinSum.ToString();
				...
				// 申請店舗コード
				nextVo.Shinsei_tenpo_cd = BoSystemFormat.formatTenpoCd(prevM1Vo.M1shinsei_tenpo_cd);
				// 申請店舗名
				nextVo.Shinsei_tenpo_nm = prevM1Vo.M1shinsei_tenpo_nm;
				// 申請担当者コード
				nextVo.Sinseitan_cd = BoSystemFormat.formatTantoCd((string)prevM1Vo.Dictionary[Tf010p01Constant.DIC_M1SINSEITAN_CD]);
				// 申請担当者名
				nextVo.Sinseitan_nm = prevM1Vo.M1sinseitan_nm;
				// 伝票番号
				nextVo.Denpyo_bango = (string)prevM1Vo.Dictionary[Tf010p01Constant.DIC_M1DENPYO_BANGO];
				// 申請理由区分
				nextVo.Sinseiriyu_kb = (string)prevM1Vo.Dictionary[Tf010p01Constant.DIC_M1SINSEIRIYU_KB];
				// 申請理由
				nextVo.Sinseiriyu = (string)prevM1Vo.Dictionary[Tf010p01Constant.DIC_M1SINSEIRIYU];
				// 確定担当者コード
				nextVo.Kakuteitan_cd = BoSystemFormat.formatTantoCd((string)prevM1Vo.Dictionary[Tf010p01Constant.DIC_M1KAKUTEITANTOCD]);
				// 確定担当者名称
				nextVo.Kakuteitan_nm = prevM1Vo.M1kakuteitan_nm;
				// 確定日
				nextVo.Kakutei_ymd = BoSystemFormat.formatDate(prevM1Vo.M1kakutei_ymd);
				// 科目コード
				...
				if (!string.IsNullOrEmpty(prevM1Vo.M1gyomuringi_no))
				{
					nextVo.Gyomuringi_no = prevM1Vo.M1gyomuringi_no.PadLeft(4, '0');
				}
				// 受理番号
				nextVo.Jyuri_no = prevM1Vo.M1jyuri_no;
				// 承認状態名称
				nextVo.Syonin_flg_nm = (string)prevM1Vo.Dictionary[Tf010p01Constant.DIC_M1SYONINSTATUS];

				// 選択明細のVO
				nextVo.Dictionary[Tf010p01Constant.DIC_M1SELECTVO] = prevM1Vo;
				// 選択行のインデックスを設定
				nextVo.Dictionary[Tf010p01Constant.DIC_M1SELECTROWIDX] = facadeContext.CommandInfo.ListIndex.ToString();
