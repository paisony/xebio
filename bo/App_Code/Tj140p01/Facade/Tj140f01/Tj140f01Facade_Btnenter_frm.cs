  ...
  public partial class Tj140f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNENTER_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				//カード部を取得します。
				Tj140f01Form f01VO = (Tj140f01Form)facadeContext.FormVO;
				...
				if (f01VO.Tanaorosisyuryo_kb == ConditionTanaorosi_syuryo_kbn.VALUE_TANAOROSI_SYURYO_KBN1)
				{
					...
					// XMLからSQLを取得する。
					FindSqlResultTable rtCheck = FindSqlUtil.CreateFindSqlResultTable("TJ140P01-01", facadeContext.DBContext);
					// 送信依頼フラグ
					...
					// XMLからSQLを取得する。
					FindSqlResultTable rtCheck2 = FindSqlUtil.CreateFindSqlResultTable("TJ140P01-02", facadeContext.DBContext);
					// 送信依頼フラグ
				...
				else if (f01VO.Tanaorosisyuryo_kb == ConditionTanaorosi_syuryo_kbn.VALUE_TANAOROSI_SYURYO_KBN2)
				{
					...
					// 棚卸終了フラグ取得
					// XMLからSQLを取得する。
					FindSqlResultTable rtCheck = FindSqlUtil.CreateFindSqlResultTable("TJ140P01-04", facadeContext.DBContext);
					// 店舗コードを設定
				...
				#region 印刷処理
				// [棚卸終了区分]が「棚卸終了確定」の場合、通常棚卸データ、業者棚卸データ毎に[棚卸集計表]を出力する。
				if (f01VO.Tanaorosisyuryo_kb == ConditionTanaorosi_syuryo_kbn.VALUE_TANAOROSI_SYURYO_KBN1)
				{
					///棚卸集計表(V)クエリプラグインです。
					/// ■パラメータ
					...
					// 送信済みフラグ
					inputData.AddScreenParameter(3, "");
					...
					output = reportCls.MdGeneratePDF(inputData,
													BoSystemConstant.REPORTID_TANAOROSISYUKEIHYO_V,
													Tj140p01Constant.FORMID_01,
													Tj140p01Constant.PGID,
													pdfFileNm
													);

					// PDFをファイルをユーザマップに設定
					facadeContext.UserMap.Add(Tj140p01Constant.FCDUO_RRT_FLNM, pdfFileNm);
		...
		/// </summary>
		/// <param name="facadeContext">ファサードコンテキスト</param>
		/// <param name="Tj140f01Form">カード部のVO</param>
		...
		{
			// XMLからSQLを取得する。
			FindSqlResultTable reader = FindSqlUtil.CreateFindSqlResultTable("TJ140P01-03", facadeContext.DBContext);
