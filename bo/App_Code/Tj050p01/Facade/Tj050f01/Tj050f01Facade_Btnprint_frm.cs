  ...
  public partial class Tj050f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNPRINT_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				//カード部を取得します。
				Tj050f01Form f01VO = (Tj050f01Form)facadeContext.FormVO;
				...
				if (f01VO.Tanaorosi_hokokusyo_kb == ConditionTanaorosi_hokokusyo_v.VALUE_TANAOROSI_HOKOKUSYO_X1)
				{	
					// 棚卸集計表
					// ■パラメータ
					// ① 店舗コード
					...
					pdfFileId = BoSystemConstant.REPORTID_TANAOROSISYUKEIHYO_V;
					pdfFileNm = string.Format("{0}.{1}",
											BoSystemReport.CreateFileName(BoSystemConstant.REPORTID_TANAOROSISYUKEIHYO_V),
											BoSystemConstant.RPT_PDF_EXTENSION
											);
				}
				else if (f01VO.Tanaorosi_hokokusyo_kb == ConditionTanaorosi_hokokusyo_v.VALUE_TANAOROSI_HOKOKUSYO_X2)
				{
					// 棚卸欠番報告書
					// ■パラメータ
					// ① 店舗コード
					// ② 棚卸日
					// ③～22	ﾌｪｲｽNoFT1～10 (取漏れ欠番検索画面から検索している場合) ：再出力画面からは送らないパラメータ

					pdfFileId = BoSystemConstant.REPORTID_TANAOROSIKETUBANHOKOKUSYO_V;
					pdfFileNm = string.Format("{0}.{1}",
											BoSystemReport.CreateFileName(BoSystemConstant.REPORTID_TANAOROSIKETUBANHOKOKUSYO_V),
											BoSystemConstant.RPT_PDF_EXTENSION
											);
				}
				else if (f01VO.Tanaorosi_hokokusyo_kb == ConditionTanaorosi_hokokusyo_v.VALUE_TANAOROSI_HOKOKUSYO_X4)
				{
					// 棚卸修正報告書
					//「棚卸修正報告書」については、送信回数が2回以上のデータを出力する。→plugin sql本文に記載済み
					// ■パラメータ
					// ①店舗コード
					// ②棚卸日
					// ③送信日			←実施日
				...
				output = reportCls.MdGeneratePDF(inputData,
												pdfFileId,
												Tj050p01Constant.FORMID_01,
												Tj050p01Constant.PGID,
												pdfFileNm
												);
				...
				// PDFをファイルをユーザマップに設定
				facadeContext.UserMap.Add(Tj050p01Constant.FCDUO_RRT_FLNM, pdfFileNm);
