  ...
  public partial class Ta060f01Facade : StandardBaseFacade
	{
		...
		public void DoM1BUMON_CD_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
                ...
                // FormVO取得
                // 画面より情報を取得する
                Ta060f01Form prevVo = (Ta060f01Form)facadeContext.FormVO;
                Ta060f02Form nextVo = (Ta060f02Form)facadeContext.GetUserObject(Ta060p01Constant.FCDUO_NEXTVO);

                IDataList prevM1List = prevVo.GetList("M1");
                IDataList nextM1List = nextVo.GetList("M1");

                // 選択行の情報を取得する。
                Ta060f01M1Form prevM1Vo = (Ta060f01M1Form)prevM1List.GetPageViewList()[facadeContext.CommandInfo.ListIndex];
                ...
                FindSqlResultTable rtSeach = FindSqlUtil.CreateFindSqlResultTable(Ta060p01Constant.SQL_ID_03, facadeContext.DBContext);

                rtSeach.BindValue(Ta060p01Constant.SQL_ID_02_REP_TENPO_CD, tenpoCd);
                //決裁日
                rtSeach.BindValue(Ta060p01Constant.SQL_ID_03_REP_KESSAI_YMD, Convert.ToDecimal(BoSystemFormat.formatDate(prevM1Vo.M1kessai_ymd, 0)));
                //変更区分
                rtSeach.BindValue(Ta060p01Constant.SQL_ID_04_REP_HENKO_KBN, Convert.ToDecimal((string)prevM1Vo.Dictionary[Ta060p01Constant.DIC_M1HENKO_KBN]));
                //部門コード
                rtSeach.BindValue(Ta060p01Constant.SQL_ID_05_REP_BUMON_CD, BoSystemFormat.formatBumonCd((string)prevM1Vo.Dictionary[Ta060p01Constant.DIC_M1BUMON_CD]));
                ...
                {
                    iCnt++;
                    Ta060f02M1Form f02m1VO = new Ta060f02M1Form();
                    ...
                    {
                        f02m1VO.M1comment_nm = dCommentstr.Substring(0, 20);        // Ｍ１コメント
                        f02m1VO.M1comment_nm2 = dCommentstr.Substring(20);          // Ｍ２コメント
                    }
                ...
                //部門コード
                nextVo.Bumon_cd = BoSystemFormat.formatBumonCd((string)prevM1Vo.Dictionary[Ta060p01Constant.DIC_M1BUMON_CD]);
                //部門名
                nextVo.Bumon_nm = (string)prevM1Vo.Dictionary[Ta060p01Constant.DIC_M1BUMON_NM];

                //選択明細のVO
                nextVo.Dictionary[Ta060p01Constant.DIC_M1SELCETVO] = prevM1Vo;
                // 選択行のインデックスを設定
                nextVo.Dictionary[Ta060p01Constant.DIC_M1SELCETROWIDX] = facadeContext.CommandInfo.ListIndex.ToString();
