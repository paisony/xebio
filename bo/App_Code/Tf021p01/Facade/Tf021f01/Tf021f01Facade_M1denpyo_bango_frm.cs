  ...
  public partial class Tf021f01Facade : StandardBaseFacade
	{
		...
		public void DoM1DENPYO_BANGO_FRM(IFacadeContext facadeContext)
		{
			...
			//	BeginTransactionWithConnect(facadeContext);
				...
				// 選択行の情報を取得する。
				Tf021f01M1Form prevM1Vo = (Tf021f01M1Form)prevM1List.GetPageViewList()[facadeContext.CommandInfo.ListIndex];
				...
				// [テーブル区分]が「0」（経費振替予定）の場合、経費振替予定（B)テーブルから検索する。
				if ("0".Equals((string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1TBLFLG].ToString()))
				{
					sSqlId = Tf021p01Constant.SQL_ID_07;
				}
				// [テーブル区分]が「1」（経費振替申請）の場合、経費振替申請（B)テーブルから検索する。
				else if ("1".Equals((string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1TBLFLG].ToString()))
				{
					sSqlId = Tf021p01Constant.SQL_ID_08;
				}
				else
				{
					sSqlId = Tf021p01Constant.SQL_ID_09;
				}
				...
				// バインド値の置き換え
				// 店舗コード
				rtSeach.BindValue(Tf021p01Constant.SQL_ID_08_REP_TENPO_CD, BoSystemFormat.formatTenpoCd(prevVo.Dictionary[SearchConditionSaveCls.PREFIX + "HEAD_TENPO_CD"].ToString()));
				// 処理日
				rtSeach.BindValue(Tf021p01Constant.SQL_ID_08_REP_SYORI_YMD, Convert.ToDecimal((string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1SYORI_YMD]));

				// [テーブル区分]が「0」（経費振替予定）の場合
				if ("0".Equals((string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1TBLFLG].ToString()))
				{
					// 管理No
					rtSeach.BindValue(Tf021p01Constant.SQL_ID_08_REP_KANRI_NO, Convert.ToDecimal((string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1KANRI_NO]));
				}
				else
				{
					// 伝票番号
					rtSeach.BindValue(Tf021p01Constant.SQL_ID_08_REP_DENPYO_BANGO, Convert.ToDecimal((string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1DENPYO_BANGO]));
				}
				...
				foreach (Hashtable rec in tableList)
				{
					...
					// ディクショナリ
					f02m1VO.Dictionary[Tf021p01Constant.DIC_M1HINSYU_CD] = BoSystemFormat.formatHinsyuCd(rec["HINSYU_CD"].ToString());		// 品種コード
					f02m1VO.Dictionary[Tf021p01Constant.DIC_M1BURANDO_CD] = BoSystemFormat.formatBrandCd(rec["BURANDO_CD"].ToString());		// ブランドコード
					f02m1VO.Dictionary[Tf021p01Constant.DIC_M1IRO_CD] = BoSystemFormat.formatIroCd(rec["IRO_CD"].ToString());				// 色コード
					f02m1VO.Dictionary[Tf021p01Constant.DIC_M1SIZE_CD] = BoSystemFormat.formatSizeCd(rec["SIZE_CD"].ToString());			// サイズコード
					f02m1VO.Dictionary[Tf021p01Constant.DIC_M1SYOHIN_CD] = rec["SYOHIN_CD"].ToString();										// 商品コード
				...
				// 登録日
				nextVo.Add_ymd = BoSystemFormat.formatDate(prevM1Vo.M1add_ymd);
				// 申請日
				nextVo.Apply_ymd = BoSystemFormat.formatDate(prevM1Vo.M1apply_ymd);
				// 申請理由区分
				nextVo.Sinseiriyu_kb = (string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1SINSEIRIYU_KB];
				// 申請理由
				nextVo.Sinseiriyu = (string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1SINSEIRIYU];
				// 伝票番号
				nextVo.Denpyo_bango = (string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1DENPYO_BANGO];
				// 管理No
				nextVo.Kanri_no = (string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1KANRI_NO];
				// 科目コード
				...
				// 承認状態名称
				nextVo.Syonin_flg_nm = (string)prevM1Vo.Dictionary[Tf021p01Constant.DIC_M1SYONINSTATUS];

				// 選択明細のVO
				nextVo.Dictionary[Tf021p01Constant.DIC_M1SELECTVO] = prevM1Vo;
				// 選択行のインデックスを設定
				nextVo.Dictionary[Tf021p01Constant.DIC_M1SELECTROWIDX] = facadeContext.CommandInfo.ListIndex.ToString();
