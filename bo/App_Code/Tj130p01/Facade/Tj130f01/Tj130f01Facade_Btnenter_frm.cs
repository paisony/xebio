  ...
  public partial class Tj130f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNENTER_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				String tenpocd = BoSystemFormat.formatTenpoCd((string)f01VO.Dictionary[SearchConditionSaveCls.PREFIX + Tj130p01Constant.DIC_TENPO_CD]);
				String tanaorosi_ymd = BoSystemFormat.formatDate((string)f01VO.Dictionary[Tj130p01Constant.DIC_TANAOROSIKIJUN_YMD]);
				...
				if (!warningflg.Equals("1"))
				{
					...
					if (f01VO.Stkmodeno == BoSystemConstant.MODE_REF)
					{
						#region 確定処理フラグチェック
						for (int i = 0; i < m1List.Count; i++)
						{
							Tj130f01M1Form f01m1VO = (Tj130f01M1Form)m1List[i];
					...
					for (int i = 0; i < m1List.Count; i++)
					{
						...
						if (tanadan < 1 || tanadan > BoSystemConstant.TANA_DAN_MAX_X)
						{
							ErrMsgCls.AddErrMsg("E173"
											...
											, facadeContext
											, new[] { "M1tana_dan" }
											, f01m1VO.Dictionary[Tj130p01Constant.DIC_M1ROWNO].ToString()
											, i.ToString()
											, "M1");
						}
					}
				...
				for (int i = 0; i < m1List.Count; i++)
				{
						...
						|| f01m1VO.M1entersyoriflg.Equals(ConditionKakuteisyori_flg.VALUE_ARI))
					{
						...
						// 明細部のDictionaryより取得
						bindVO.Value = (string)f01m1VO.Dictionary[Tj130p01Constant.DIC_M1FACE_NO];
						...
						// 明細部のDictionaryより取得
						bindVO.Value = (string)f01m1VO.Dictionary[Tj130p01Constant.DIC_M1TANA_DAN];
						bindVO.Type = BoSystemSql.BINDTYPE_NUMBER;	// 1:文字列、2:数値
						bindList.Add(bindVO);
						...
						// 明細部のDictionaryより取得
						bindVO.Value = BoSystemFormat.formatDate((string)f01m1VO.Dictionary[Tj130p01Constant.DIC_M1TANAOROSI_YMD]);
						bindVO.Type = BoSystemSql.BINDTYPE_NUMBER;	// 1:文字列、2:数値
						bindList.Add(bindVO);
						...
						// 明細部のDictionaryより取得
						bindVO.Value = (string)f01m1VO.Dictionary[Tj130p01Constant.DIC_M1SOSINKAI_SU];
						bindVO.Type = BoSystemSql.BINDTYPE_NUMBER;	// 1:文字列、2:数値
						bindList.Add(bindVO);

						// 排他チェック
						V03003Check.CheckHaitaMaxVal(
								Convert.ToDecimal((string)f01m1VO.Dictionary[Tj130p01Constant.DIC_M1UPD_YMD]),
								Convert.ToDecimal((string)f01m1VO.Dictionary[Tj130p01Constant.DIC_M1UPD_TM]),
								facadeContext,
								"MDIT0010",
								sRepSql.ToString(),
								bindList,
				...
				int iCnt = 0;
				//細単位で以下の処理を実施する。				
				foreach (Tj130f01M1Form check in m1List.ListData)
				{
					//	[選択モードNo]が「参照」の場合			
					if (f01VO.Stkmodeno == BoSystemConstant.MODE_REF)
					{
		...
		private Boolean chohukuCheck(IDataList m1List, IFacadeContext facadeContext, string tenpoCd, string tanaorosiYmd)
		{
			...
			{
				// 比較元明細行情報取得
				Tj130f01M1Form hikakumoto = (Tj130f01M1Form)m1List[i];
				int infoM1 = 0;
				...
				if (hikakumoto.M1entersyoriflg.Equals(ConditionKakuteisyori_flg.VALUE_ARI))
				{
					...
					for (int j = 0; j < m1List.Count; j++)
					{
						...
						Tj130f01M1Form hikakusaki = (Tj130f01M1Form)m1List[j];
					...
					#region DB重複チェック
					//DB重複チェック
					FindSqlResultTable reader = FindSqlUtil.CreateFindSqlResultTable(Tj130p01Constant.SQL_ID_04, facadeContext.DBContext);
		...
		#region  更新
		private Boolean upd(IFacadeContext facadeContext, Tj130f01Form f01Form, Tj130f01M1Form f01M1Form, LoginInfoVO loginInfo, SysDateVO sysDateVO, int iCnt)
		{
			Boolean retFlg = true;
			string tenpoCd = BoSystemFormat.formatTenpoCd((string)f01Form.Dictionary[SearchConditionSaveCls.PREFIX + Tj130p01Constant.DIC_TENPO_CD]);
			decimal faceNo_old = Convert.ToDecimal(BoSystemString.Nvl((string)f01M1Form.Dictionary[Tj130p01Constant.DIC_M1FACE_NO], "0"));
			decimal faceNo_new = Convert.ToDecimal(BoSystemString.Nvl(f01M1Form.M1face_no, "0"));
			decimal tanaDan_old = Convert.ToDecimal(BoSystemString.Nvl((string)f01M1Form.Dictionary[Tj130p01Constant.DIC_M1TANA_DAN], "0"));
			...
			decimal tanaorosi_ymd = Convert.ToDecimal(BoSystemString.Nvl((string)f01M1Form.Dictionary[Tj130p01Constant.DIC_M1TANAOROSI_YMD], "0"));
			decimal sosinkai_su = Convert.ToDecimal(BoSystemString.Nvl((string)f01M1Form.Dictionary[Tj130p01Constant.DIC_M1SOSINKAI_SU], "0"));
			...
			// XMLからSQLを取得する。
			FindSqlResultTable rtSearch = FindSqlUtil.CreateFindSqlResultTable(Tj130p01Constant.SQL_ID_05, facadeContext.DBContext);
			// バインド設定
			...
			// XMLからSQLを取得する。
			FindSqlResultTable reader = FindSqlUtil.CreateFindSqlResultTable(Tj130p01Constant.SQL_ID_07, facadeContext.DBContext);
			// バインド設定
			...
			// XMLからSQLを取得する。
			FindSqlResultTable reader2 = FindSqlUtil.CreateFindSqlResultTable(Tj130p01Constant.SQL_ID_08, facadeContext.DBContext);
			...
			// XMLからSQLを取得する。
			FindSqlResultTable reader3 = FindSqlUtil.CreateFindSqlResultTable(Tj130p01Constant.SQL_ID_06, facadeContext.DBContext);
			...
			// XMLからSQLを取得する。
			FindSqlResultTable reader4 = FindSqlUtil.CreateFindSqlResultTable(Tj130p01Constant.SQL_ID_09, facadeContext.DBContext);
		...
		#region 削除
		private Boolean dlt(IFacadeContext facadeContext, Tj130f01Form f01Form, Tj130f01M1Form f01M1Form)
		{
			Boolean retFlg = true;
			string tenpoCd = BoSystemFormat.formatTenpoCd((string)f01Form.Dictionary[SearchConditionSaveCls.PREFIX + Tj130p01Constant.DIC_TENPO_CD]);
			...
			// XMLからSQLを取得する。
			FindSqlResultTable reader = FindSqlUtil.CreateFindSqlResultTable(Tj130p01Constant.SQL_ID_10, facadeContext.DBContext);
			...
			//	[棚卸確定TBL(B)]を削除する。	
			FindSqlResultTable reader2 = FindSqlUtil.CreateFindSqlResultTable(Tj130p01Constant.SQL_ID_11, facadeContext.DBContext);
