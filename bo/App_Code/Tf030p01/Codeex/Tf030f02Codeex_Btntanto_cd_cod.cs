...
namespace com.xebio.bo.Tf030p01.Codeex
{
  /// <summary>
  /// フォーム：Tf030f02 項目：Btntanto_cd の拡張コード参照クラスです。
  /// </summary>
  [Serializable]
	public class Tf030f02Codeex_Btntanto_cd : StandardBaseCodeex
	{
		public Tf030f02Codeex_Btntanto_cd()
		{
		}
		...
		public override void Initialize(ICodeContext context)
		{
			...
			// 業務ロジック↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
			// 店舗コードをパラメータに追加
			context.Condition.AddExtItemID("Head_tenpo_cd");
			// 明細画面の店舗コードをパラメータに追加
			context.Condition.AddExtItemID("Tenpo_cd");
			// 業務ロジック↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
		...
		public override void InitSearch(ICodeContext context)
		{
			...
			// 親画面の店舗コードを取得する。
			String tenpo_cd = context.ExeValue.InputValues["Tenpo_cd"] as string;
			//明細画面の店舗コードが入っている場合は、明細画面の店舗コードを参照する
			if (string.IsNullOrEmpty(tenpo_cd)) 
			{
				tenpo_cd = context.ExeValue.InputValues["Head_tenpo_cd"] as string;
			}
			// ログイン店舗情報を条件に設定
			context.Condition.AddFindKey("HANBAIINTENPO_CD = '" + tenpo_cd + "' AND  SAKUJYO_FLG = 0");
