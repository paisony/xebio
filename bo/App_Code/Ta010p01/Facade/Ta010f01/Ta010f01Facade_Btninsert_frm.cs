  ...
  public partial class Ta010f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNINSERT_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				#region 業務チェック
				// 単項目チェック
				ChkSelSingleItem(facadeContext, prevVo, Ta010p01Constant.CHECK_MODE_BTNINSERT);
				// ------------------------------------------------------------------------------------
				...
				// 関連項目チェック
				ChkSelRelatedItem(facadeContext, prevVo, Ta010p01Constant.CHECK_MODE_BTNINSERT);
				// ------------------------------------------------------------------------------------
				//エラーが発生した場合、その時点でチェックを中止しクライアント側へエラー内容を返却する。
				// ------------------------------------------------------------------------------------
				if (MessageDisplayUtil.HasError(facadeContext))
				...
				// 明細をページ分追加
				AddRowCls.AddEmptyPage<Ta010f02M1Form>("M1", "M1rowno", nextVo);
				#region カード部設定
				// ヘッダ店舗コード
				...
				// 依頼日
				nextVo.Hattyu_ymd = sysDateVO.Sysdate.ToString();
				// 担当者コード
				...
				// 依頼理由コード
				//nextVo.Irairiyu_cd = "";
				Ta010p01Util.SetIrairiyu_cd<Ta010f02Form>(nextVo, string.Empty);
				// 合計欄の設定
				...
				// 選択明細のVO
				nextVo.Dictionary[Ta010p01Constant.DIC_M1SELCETVO] = string.Empty;
				// 選択行のインデックスを設定
				nextVo.Dictionary[Ta010p01Constant.DIC_M1SELCETROWIDX] = string.Empty;
