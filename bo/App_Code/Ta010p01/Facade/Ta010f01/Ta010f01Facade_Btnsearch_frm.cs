  ...
  public partial class Ta010f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNSEARCH_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				// 照会モード以外の場合、申請状態：未選択
				if (!f01VO.Modeno.Equals(BoSystemConstant.MODE_REF))
				{
					f01VO.Shinsei_flg = BoSystemConstant.DROPDOWNLIST_MISENTAKU;	
				}
				#endregion
				#region 業務チェック
				// 単項目チェック
				ChkSelSingleItem(facadeContext, f01VO, Ta010p01Constant.CHECK_MODE_BTNSEARCH);
				// ------------------------------------------------------------------------------------
				...
				// 関連項目チェック
				ChkSelRelatedItem(facadeContext, f01VO, Ta010p01Constant.CHECK_MODE_BTNSEARCH);
				// ------------------------------------------------------------------------------------
				//エラーが発生した場合、その時点でチェックを中止しクライアント側へエラー内容を返却する。
				// ------------------------------------------------------------------------------------
				if (MessageDisplayUtil.HasError(facadeContext))
