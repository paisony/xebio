  ...
  public partial class Te040f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNSIZSTK_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				//	//以下に業務ロジックを記述する。
				// サイズ検索にて選択したレコードを取得
				IList<Hashtable> hachuMstInfo = (IList<Hashtable>)facadeContext.GetUserObject(Te040p01Constant.KEY_SIZE_SEARCH_RESULT);

				// フォームオブジェクト取得
				Te040f01Form form = (Te040f01Form)facadeContext.FormVO;
				// 明細オブジェクト取得
				IDataList m1List = form.GetList("M1");

				// 有効現在行数
				int curRowCnt = Te040p01Util.GetRowCnt(m1List);
				// 設定店舗(ヘッダ情報より設定)
				...
				if (requiredCnt > 0)
				{
					...
					// 行追加
					AddRowCls.AddEmptyRow<Te040f01M1Form>("M1", "M1rowno", form, addRowCnt);
				}
				...
				{
					// 発注マスタ情報設定
					DoShohinCopy((Te040f01M1Form)m1List[index], hachuMstRow, setTenpoCd, true);
					Decimal wkSuryo = Convert.ToDecimal(BoSystemString.Nvl(((Te040f01M1Form)m1List[index]).M1syukka_su, "0"));
					...
					{
						((Te040f01M1Form)m1List[index]).M1genka_kin = genkakin.ToString();							// 原価金額
						((Te040f01M1Form)m1List[index]).M1genka_kin_hdn = genkakin.ToString();						// 原価金額（隠し）
					}
					index++;
				}
				...
				{
					// ページ追加処理
					AddRowCls.AddEmptyRow<Te040f01M1Form>("M1", "M1rowno", form, m1List.DispRow);
				}
				m1List.SetPage(dispPageNo);
				// フォーカス行インデックス設定
				facadeContext.SetUserObject(Te040p01Constant.KEY_SIZE_FOCUS_INDEX, focusIndex.ToString());
