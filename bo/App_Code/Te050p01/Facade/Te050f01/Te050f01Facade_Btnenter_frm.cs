			...
			try
			{
				...
				foreach (Te050f01M1Form f01m1VO in m1List.ListData)
				{
					...
					if (f01m1VO.M1entersyoriflg.Equals(ConditionKakuteisyori_flg.VALUE_ARI))
					{
						...
						BoSystemLog.logOut("[再入荷防止TBL]を更新(MERGE) START");
						int mergeCnt = Merge_SaiNyukaBousi(facadeContext, f01VO, f01m1VO, logininfo, sysDateVO);
						BoSystemLog.logOut("[再入荷防止TBL]を更新(MERGE) END");
					}
				}
				#endregion
				//トランザクションをコミットする。
