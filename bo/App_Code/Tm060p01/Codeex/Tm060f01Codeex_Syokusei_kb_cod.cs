...
{
  /// <summary>
  /// フォーム：Tm060f01 項目：Syokusei_kb の拡張コード参照クラスです。
  /// </summary>
  [Serializable]
	public class Tm060f01Codeex_Syokusei_kb : StandardBaseCodeex
	{
		public Tm060f01Codeex_Syokusei_kb()
		{
		}
		...
		public override void Initialize(ICodeContext context)
		{
			...
			// 業務区分の条件追加
			context.Condition.AddFindKey("SIKIBETSU_CD = 'SSKB'");
			if (CheckCompanyCls.IsXebio())
			{
				context.Condition.AddFindKey("MEISYO_CD IN ('2','5','10','15','18','20','30','60','90')");
			}
			else
			{
				context.Condition.AddFindKey("MEISYO_CD IN ('1','2','3','5','10')");
			}

			context.Condition.AddFindKey(" SAKUJYO_FLG = 0");
			//ソート条件の指定
			SortKey sortKey1 = new SortKey("TO_NUMBER(MEISYO_CD)", false, 1);
