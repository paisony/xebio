            ...
            try
            {
				...
				for (int i = 0; i < m1List.Count; i++)
				{
					...
					if (m1formVO.M1selectorcheckbox.Equals(BoSystemConstant.CHECKBOX_ON))
					{
						// パラメタを配列形式で格納します
						// 店舗／業者区分
						inputData.AddScreenParameter(1, (string)m1formVO.Dictionary[Tj040p01Constant.DIC_M1TENPO_GYOSYA_KB]);
						// 店舗コード
						inputData.AddScreenParameter(2, BoSystemFormat.formatTenpoCd(tenpocd));
						// フェイスNo
						inputData.AddScreenParameter(3, (string)m1formVO.Dictionary[Tj040p01Constant.DIC_M1FACE_NO]);
						// 棚段
						inputData.AddScreenParameter(4, (string)m1formVO.M1tana_dan);
						// 回数
						inputData.AddScreenParameter(5, (string)m1formVO.M1kai_su);
						// 棚卸日
						inputData.AddScreenParameter(6, (string)m1formVO.Dictionary[Tj040p01Constant.DIC_M1TANAOROSI_YMD]);
						// 送信回数/処理日付
						inputData.AddScreenParameter(7, (string)m1formVO.Dictionary[Tj040p01Constant.DIC_M1SOSINKAI_SU]);
					}
				}
