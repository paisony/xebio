  ...
  public partial class Tl010f01Facade : StandardBaseFacade
	{
		...
		public void DoBTNSEAL_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				String sTenpo_cd = BoSystemFormat.formatTenpoCd((String)formVO.Dictionary[SearchConditionSaveCls.PREFIX + "Head_tenpo_cd".ToUpper()]);
		...
		/// <returns></returns>
		private void ReplaceWhereSeal(Tl010f01Form formVO, FindSqlResultTable reader)
		{
			// 店舗コードを設定
			String sTenpo_cd = BoSystemFormat.formatTenpoCd((String)formVO.Dictionary[SearchConditionSaveCls.PREFIX + "Head_tenpo_cd".ToUpper()]);
			...
			int iCnt = 1;
			sRepSql.Append(" AND (MDCT0010.TENPO_CD, MDCT0010.BAIHENKAISI_YMD, MDCT0010.BUMON_CD,MDCT0010.BAIHEN_NO) IN (");
			sRepSqlMax.Append(" AND (MDCT0010_MAX.TENPO_CD, MDCT0010_MAX.BAIHENKAISI_YMD, MDCT0010_MAX.BUMON_CD,MDCT0010_MAX.BAIHEN_NO) IN (");
			...
			for (int i = 0; i < m1List.Count; i++)
			{
				...
				if (BoSystemConstant.CHECKBOX_ON.Equals(m1formVO.M1selectorcheckbox))
				{
					...
					bindListSql.Add(new BindInfoVO("BIND5_BUMON_CD" + iCnt.ToString("000")
												, BoSystemFormat.formatBumonCd(m1formVO.Dictionary[Tl010p01Constant.DIC_M1BUMON_CD].ToString())
												, BoSystemSql.BINDTYPE_STRING));

					// 売変No
					bindListSql.Add(new BindInfoVO("BIND5_BAIHEN_NO" + iCnt.ToString("000")
												, BoSystemFormat.formatBaihen_shiji_no(m1formVO.Dictionary[Tl010p01Constant.DIC_M1BAIHEN_NO].ToString())
												, BoSystemSql.BINDTYPE_STRING));
