  ...
  public partial class Ta020f01Facade : StandardBaseFacade
	{
		...
		public void DoM1KANRI_NO_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				// 選択行の情報を取得する。
				Ta020f01M1Form prevM1Vo = (Ta020f01M1Form)prevM1List.GetPageViewList()[facadeContext.CommandInfo.ListIndex];
				...
				// -------------------------------------------
				// 店舗コード
				rtSearch.BindValue("BIND_TENPO_CD", BoSystemFormat.formatTenpoCd(prevM1Vo.Dictionary[Ta020p01Constant.DIC_M1TENPO_CD].ToString()));
				// 管理番号
				rtSearch.BindValue("BIND_KANRI_NO", Convert.ToDecimal((string)prevM1Vo.Dictionary[Ta020p01Constant.DIC_M1KANRI_NO]));
				// 処理日付
				rtSearch.BindValue("BIND_SYORI_YMD", Convert.ToDecimal((string)prevM1Vo.Dictionary[Ta020p01Constant.DIC_M1SYORI_YMD]));
				...
				if (BoSystemConstant.MODE_APPLY.Equals(prevVo.Modeno)
				 && ConditionSinsei_jotai.VALUE_SINSEI_JOTAI2.Equals(prevM1Vo.Dictionary[Ta020p01Constant.DIC_M1SHINSEI_FLG]))
				{
				...
				// 依頼日
				nextVo.Irai_ymd = prevM1Vo.M1hattyu_ymd;
				// 担当者コード
				nextVo.Tantosya_cd = (string)prevM1Vo.Dictionary[Ta020p01Constant.DIC_M1TANTOSYA_CD];
				// 担当者名
				nextVo.Hanbaiin_nm = prevM1Vo.M1hanbaiin_nm;
				// 依頼理由コード
				nextVo.Irairiyu_cd = (string)prevM1Vo.Dictionary[Ta020p01Constant.DIC_M1IRAIRIYU_CD];

				// 選択明細のVO
				nextVo.Dictionary[Ta020p01Constant.DIC_M1SELCETVO] = prevM1Vo;
				// 選択行のインデックスを設定
				nextVo.Dictionary[Ta020p01Constant.DIC_M1SELCETROWIDX] = facadeContext.CommandInfo.ListIndex.ToString();
				#endregion
				#endregion
