  ...
  public partial class Ta020f01Facade : StandardBaseFacade
	{
		...
		/// フォームID。
		/// </summary>
		private const string FORMID = "Ta020f01";
		#endregion
		...
		/// インスタンスを生成します。
		/// </summary>
		public Ta020f01Facade()
			: base()
		{
		}
		#endregion

		#region Ta020f01画面データを作成する。
		/// <summary>
		/// Ta020f01画面データを作成する。
		/// </summary>
		...
		/// <param name="Ta020f01Form">f01VO</param>
		/// <param name="String">mode</param>
		/// <returns>Decimal</returns>
		private void ChkSelSingleItem ( IFacadeContext facadeContext, Ta020f01Form f01VO, String mode )
		{
			#region 単項目チェック
			// 1-1 ヘッダ店舗コード
			...
			// 検索時は、以下のチェックを行う
			if (Ta020p01Constant.CHECK_MODE_BTNSEARCH.Equals(mode))
			{
		...
		/// <param name="String">mode</param>
		/// <returns>Decimal</returns>
		private void ChkSelRelatedItem ( IFacadeContext facadeContext, Ta020f01Form f01VO, String mode )
		{
			#region 関連項目チェック
			// 検索時は、以下のチェックを行う
			if (Ta020p01Constant.CHECK_MODE_BTNSEARCH.Equals(mode))
			{
				// 2-1 依頼日FROM、発注日TO
				//       依頼日ＦＲＯＭ > 依頼日ＴＯの場合エラー
				if (!string.IsNullOrEmpty(f01VO.Irai_ymd_from) && !string.IsNullOrEmpty(f01VO.Irai_ymd_to))
				{
		...
		private IList<Hashtable> DoSelect ( Ta020f01Form f01VO, IDBContext DBContext, bool blCount )
		{
			FindSqlResultTable rtSearch = null;
			if (blCount)
			{
				// 件数取得
				rtSearch = FindSqlUtil.CreateFindSqlResultTable(Ta020p01Constant.SQL_ID_01, DBContext);
			}
			else
			{
				// 検索結果取得
				rtSearch = FindSqlUtil.CreateFindSqlResultTable(Ta020p01Constant.SQL_ID_02, DBContext);
			}
		...
		/// <param name="blCount">blCount</param>
		/// <returns></returns>
		private void SetBind ( Ta020f01Form f01VO, FindSqlResultTable reader, bool blCount )
		{
			...
			// 依頼日FROMを設定
			if (!string.IsNullOrEmpty(f01VO.Irai_ymd_from))
			{
				sRepSql1.Append(" AND T1.UPD_YMD >= :BIND_UPD_YMD_FROM1").AppendLine();
				bindVO = new BindInfoVO();
				bindVO.BindId = "BIND_UPD_YMD_FROM1";
				bindVO.Value = BoSystemFormat.formatDate(f01VO.Irai_ymd_from);
			...
			// 依頼日TOを設定
			if (!string.IsNullOrEmpty(f01VO.Irai_ymd_to))
			{
				sRepSql1.Append(" AND T1.UPD_YMD <= :BIND_UPD_YMD_TO1").AppendLine();
				bindVO = new BindInfoVO();
				bindVO.BindId = "BIND_UPD_YMD_TO1";
				bindVO.Value = BoSystemFormat.formatDate(f01VO.Irai_ymd_to);
				bindVO.Type = BoSystemSql.BINDTYPE_NUMBER;	// 1:文字列、2:数値
				bindList.Add(bindVO);
			...
			{
				// [選択モードNo]が「申請」、「修正」の場合、出荷要望申請テーブルから検索する。（①のSQLを実行する）
				if (BoSystemConstant.MODE_APPLY.Equals(f01VO.Modeno) || BoSystemConstant.MODE_UPD.Equals(f01VO.Modeno))
				{
					// 申請テーブル検索を可
					BoSystemSql.AddSql(reader, Ta020p01Constant.SQL_ID_01_REP_ADD_WHERE_KAHI_SHIN, Ta020p01Constant.SERCH_YUKO_KA);
					// 申請テーブル検索条件を設定
					BoSystemSql.AddSql(reader, Ta020p01Constant.SQL_ID_01_REP_ADD_WHERE_SHIN, sRepSql1.ToString(), bindList);
					// 確定テーブル検索を否
					BoSystemSql.AddSql(reader, Ta020p01Constant.SQL_ID_01_REP_ADD_WHERE_KAHI_KAKU, Ta020p01Constant.SERCH_YUKO_HI);
					// 申請テーブル検索条件を未設定
					BoSystemSql.AddSql(reader, Ta020p01Constant.SQL_ID_01_REP_ADD_WHERE_KAKU, "");

					// [選択モードNo]が「取消」の場合、出荷要望確定テーブルから検索する。（②のSQLを実行する）
				}
				else if (BoSystemConstant.MODE_DEL.Equals(f01VO.Modeno))
				{
					// 申請テーブル検索を否
					BoSystemSql.AddSql(reader, Ta020p01Constant.SQL_ID_01_REP_ADD_WHERE_KAHI_SHIN, Ta020p01Constant.SERCH_YUKO_HI);
					// 申請テーブル検索条件を未設定
					BoSystemSql.AddSql(reader, Ta020p01Constant.SQL_ID_01_REP_ADD_WHERE_SHIN, "");
					// 確定テーブル検索を可
					BoSystemSql.AddSql(reader, Ta020p01Constant.SQL_ID_01_REP_ADD_WHERE_KAHI_KAKU, Ta020p01Constant.SERCH_YUKO_KA);
					// 申請テーブル検索条件を設定
					BoSystemSql.AddSql(reader, Ta020p01Constant.SQL_ID_01_REP_ADD_WHERE_KAKU, sRepSql2.ToString(), bindList2);
				...
				else if (BoSystemConstant.MODE_REF.Equals(f01VO.Modeno))
				{
					...
					// [状態]が空白、出荷要望申請テーブルと出荷要望確定テーブルから検索する。（① UNION ALL ②のSQLを実行する。）
					...
					// [状態]が未申請、出荷要望申請テーブルから検索する。（①のSQLを実行する）
					...
					// [状態]が申請済、出荷要望確定テーブルから検索する。（②のSQLを実行する）
		...
		/// <returns></returns>
		public void DoCopy ( Ta020f01Form f01VO, IDataList m1List, IList<Hashtable> tableList )
		{
			int iCnt = 0;
			foreach (Hashtable rec in tableList)
			{
				iCnt++;
				Ta020f01M1Form f01m1VO = new Ta020f01M1Form();

				#region 明細転記
				f01m1VO.M1rowno = iCnt.ToString();																	// Ｍ１行NO
				f01m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1KANRI_NO, BoSystemFormat.formatDenpyoNo(rec["KANRI_NO"].ToString()));    // Ｍ１管理NOリンク
				...
				// Dictionary
				f01m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1TENPO_CD, rec["TENPO_CD"].ToString());				// 店舗コード
				f01m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1UPD_YMD, rec["UPD_YMD"].ToString());					// 更新日
				f01m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1UPD_TM, rec["UPD_TM"].ToString());					// 更新時間
				f01m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1SYORI_YMD, rec["SYORI_YMD"].ToString());				// 処理日付
				f01m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1SYORI_TM, rec["SYORI_TM"].ToString());				// 処理時間
				f01m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1IRAIRIYU_CD, rec["IRAIRIYU_CD"].ToString());			// 依頼理由コード
				f01m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1SHINSEI_FLG, rec["SHINSEI_FLG"].ToString());			// 申請状態
				f01m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1TANTOSYA_CD, rec["UPD_TANCD"].ToString());			// 更新担当者コード
				#endregion
		...
		/// DoCopy 明細転記処理
		/// </summary>
		/// <param name="Ta020f01M1Form">prevM1Vo</param>
		/// <param name="IDataList">nextM1List</param>
		/// <param name="tableList">取得情報</param>
		/// <returns>合計集計情報</returns>
		public decimal[] DoDetailCopy ( Ta020f01M1Form prevM1Vo, IDataList nextM1List, IList<Hashtable> tableList )
		{
			...
			foreach (Hashtable rec in tableList)
			{
				Ta020f02M1Form f02m1VO = new Ta020f02M1Form();
				...
				// 出荷要望の場合は、発注メッセージは、常に空白を設定
				//// 未申請の場合
				//if (ConditionSinsei_jotai.VALUE_SINSEI_JOTAI1.Equals(prevM1Vo.Dictionary[Ta020p01Constant.DIC_M1SHINSEI_FLG]))
				//{
				//	f02m1VO.M1hatchu_msg = Ta020p01Util.GetHtms(f02m1VO);			// Ｍ１発注メッセージ
				//	// 申請済みの場合
				//}
				//else if (ConditionSinsei_jotai.VALUE_SINSEI_JOTAI2.Equals(prevM1Vo.Dictionary[Ta020p01Constant.DIC_M1SHINSEI_FLG]))
				//{
				//	f02m1VO.M1hatchu_msg = rec["MESSEGE_NM"].ToString();			// Ｍ１発注メッセージ
				//}
				f02m1VO.M1hatchu_msg = "";										// Ｍ１発注メッセージ
				...
				// Dictionary
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1BUMON_CD, rec["BUMON_CD"].ToString());							// 部門コード
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1HINSYU_CD, rec["HINSYU_CD"].ToString());							// 品種コード
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1BURANDO_CD, rec["BURANDO_CD"].ToString());						// ブランドコード
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1IRO_CD, rec["IRO_CD"].ToString());								// 色コード
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1SIZE_CD, rec["SIZE_CD"].ToString());								// サイズコード
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1SYOHIN_CD, rec["SYOHIN_CD"].ToString());							// 商品コード
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1HANBAIKANRYO_YMD, rec["HANBAIKANRYO_YMD"].ToString());			// 販売完了日
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1HATYUTAISYO_KB, rec["HATYUTAISYO_KB"].ToString());				// 補充区分
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1HYOKA_KB, rec["HYOKA_KB"].ToString());							// 評価区分
				f02m1VO.Dictionary.Add(Ta020p01Constant.DIC_M1MESSEGE_KB, rec["MESSEGE_KB"].ToString());						// メッセージ区分
		...
		/// <returns>Decimal</returns>
		private void ChkUpdSingleItem ( IFacadeContext facadeContext, Ta020f01Form f01VO, SysDateVO sysDateVO )
		{
			#region 単項目チェック
			#endregion
		}
		...
		private void ChkUpdHaita ( IFacadeContext facadeContext, IDataList m1List, string sModeno )
		{
			...
			foreach (Ta020f01M1Form f01m1VO in m1List.ListData)
			{
				...
				if (f01m1VO.M1selectorcheckbox.Equals(BoSystemConstant.CHECKBOX_ON))
				{
					...
					bindVO.Value = BoSystemFormat.formatTenpoCd((string)f01m1VO.Dictionary[Ta020p01Constant.DIC_M1TENPO_CD]);
					bindVO.Type = BoSystemSql.BINDTYPE_STRING;	// 1:文字列、2:数値
					bindList.Add(bindVO);
					...
					bindVO.Value = BoSystemFormat.formatDenpyoNo((string)f01m1VO.Dictionary[Ta020p01Constant.DIC_M1KANRI_NO]);
					bindVO.Type = BoSystemSql.BINDTYPE_NUMBER;	// 1:文字列、2:数値
					bindList.Add(bindVO);
					...
					bindVO.Value = (string)f01m1VO.Dictionary[Ta020p01Constant.DIC_M1SYORI_YMD];
					bindVO.Type = BoSystemSql.BINDTYPE_NUMBER;	// 1:文字列、2:数値
					bindList.Add(bindVO);

					// 排他チェック
					V03003Check.CheckHaitaMaxVal(
							Convert.ToDecimal((string)f01m1VO.Dictionary[Ta020p01Constant.DIC_M1UPD_YMD]),
							Convert.ToDecimal((string)f01m1VO.Dictionary[Ta020p01Constant.DIC_M1UPD_TM]),
							facadeContext,
							sHaitaTable,
							sRepSql.ToString(),
							bindList,
		...
		#endregion
		#region [出荷要望申請TBL(B)]を検索し、[出荷要望確定TBL(B)]を登録する。(SQL_ID_06)
		/// <summary>
		/// [出荷要望申請TBL(B)]を検索し、[出荷要望確定TBL(B)]を登録
		/// </summary>
		/// <param name="facadeContext">ファサードコンテキスト</param>
		...
		{
			FindSqlResultTable reader = FindSqlUtil.CreateFindSqlResultTable(Ta020p01Constant.SQL_ID_06, facadeContext.DBContext);
