  ...
  public partial class Ta020f02Facade : StandardBaseFacade
	{
		...
		public void DoBTNROWDEL_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				// 行追加フラグ
				int addRowFlg = Convert.ToInt32(Ta020p01Constant.FLG_OFF);
				// 新規作成時は、行追加フラグをＯＮ
				if (BoSystemConstant.MODE_INSERT.Equals(f02Vo.Stkmodeno))
				{
					addRowFlg = Convert.ToInt32(Ta020p01Constant.FLG_ON);
				}
				// 選択行を削除
				decimal lastRow = DelRowCls.DeleteSelectRow<Ta020f02Form, Ta020f02M1Form>("M1", "M1rowno", f02Vo, gokeiCalcList, addRowFlg);
				...
				// フォーカス行をコードビハインドに戻す
				facadeContext.SetUserObject(Ta020p01Constant.FCDUO_FOCUSROW, lastRow.ToString());
