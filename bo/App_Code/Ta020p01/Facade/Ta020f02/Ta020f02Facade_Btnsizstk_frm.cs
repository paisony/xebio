  ...
  public partial class Ta020f02Facade : StandardBaseFacade
	{
		...
		public void DoBTNSIZSTK_FRM(IFacadeContext facadeContext)
		{
			...
			try
			{
				...
				//	//以下に業務ロジックを記述する。
				// サイズ検索にて選択したレコードを取得
				IList<Hashtable> hachuMstInfo = (IList<Hashtable>)facadeContext.GetUserObject(Ta020p01Constant.KEY_SIZE_SEARCH_RESULT);

				// フォームオブジェクト取得
				Ta020f02Form form = (Ta020f02Form)facadeContext.FormVO;
				// 明細オブジェクト取得
				IDataList m1List = form.GetList("M1");

				// 有効現在行数
				int curRowCnt = Ta020p01Util.GetRowCnt(m1List);
				// 設定店舗(ヘッダ情報より設定)
				...
				if (requiredCnt > 0)
				{
					...
					// 行追加
					AddRowCls.AddEmptyRow<Ta020f02M1Form>("M1", "M1rowno", form, addRowCnt);

					// 新規作成時は、ページ追加
					if (BoSystemConstant.MODE_INSERT.Equals(form.Stkmodeno))
					{
						// ページ追加
						AddRowCls.AddEmptyPage<Ta020f02M1Form>("M1", "M1rowno", form);
					}
				...
				{
					// 発注マスタ情報設定
					DoShohinCopy((Ta020f02M1Form)m1List[index], hachuMstRow, true);
					Decimal wkSuryo = Convert.ToDecimal(BoSystemString.Nvl(((Ta020f02M1Form)m1List[index]).M1irai_su, "0"));
					...
					{
						((Ta020f02M1Form)m1List[index]).M1genkakin = genkakin.ToString();							// 原価金額
						((Ta020f02M1Form)m1List[index]).M1genkakin_hdn = genkakin.ToString();						// 原価金額（隠し）
					}
					index++;
				}
				...
				if (BoSystemConstant.MODE_INSERT.Equals(form.Stkmodeno))
				{
					...
					{
						// ページ追加処理
						AddRowCls.AddEmptyRow<Ta020f02M1Form>("M1", "M1rowno", form, m1List.DispRow);
					}
				}
				else
				{
				...
				// フォーカス行インデックス設定
				facadeContext.SetUserObject(Ta020p01Constant.KEY_SIZE_FOCUS_INDEX, focusIndex.ToString());
