...
function onLoad() {
	...
	if (urlPara["StdFocusedItemID"] == "Tyotatsu_kb") {
		for (i = 0; i < getAdvControlFromItemID("Tyotatsu_kb").length; i++) {
			if (getAdvControlFromItemID("Tyotatsu_kb")[i].checked == true) {
				getAdvControlFromItemID("Tyotatsu_kb")[i].focus();
				break;
			}
		}
	}
...
function onBlur(ev) {
	...
	switch (eventTargetName.toUpperCase()) {
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
		case "ymd_from".toUpperCase():	// 日付けFROM
			// FROMの値をTOへコピー
			fromToCopy("ymd");
			break;
