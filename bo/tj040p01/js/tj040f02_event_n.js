...
function onChange(ev) {
	...
	//  ここに項目IDのcase文を追加し、固有処理を記述します。
		...
		// 訂正数量
		// -------------------
		case "Teisei_suryo".toUpperCase():

			// 訂正数量
			var su = getAdvControlFromItemID("Teisei_suryo");
			// 訂正数量(隠し)
			var suHid = getAdvControlFromItemID("Teisei_suryo_hdn");

			// 点数棚卸数量
			var tana_su = getAdvControlFromItemID("Tensutanaorosi_su");
			// 合計数量
			var gokei_su = getAdvControlFromItemID("Gokei_suryo");

			// 点数棚卸数量＋訂正数量を合計数量に設定する。
			gokei_su.value = formatComma(Number(unFormatComma(tana_su.value)) + Number(unFormatComma(su.value)));
